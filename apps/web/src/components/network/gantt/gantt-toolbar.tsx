...
export function GanttToolbar({
  ...
  useEffect(() => {
    if (!optimizerOpen && !compareOpen) loadCompareCount()
  }, [optimizerOpen, compareOpen, loadCompareCount])
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      ...
      if (
        utilBtnRef.current &&
        !utilBtnRef.current.contains(t) &&
        utilDropRef.current &&
        !utilDropRef.current.contains(t)
      )
        setUtilOpen(false)
    }
          ...
          {toolbarItems.map((item, i) => (
            <div key={i} className="relative">
              ...
              {'badge' in item && item.badge && (
                <div
                  className="absolute top-0.5 right-0 min-w-[14px] h-[14px] rounded-full flex items-center justify-center px-0.5"
                  ...
                  return (
                    <div key={t.icaoType}>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <span className="text-[12px] font-semibold w-[32px] text-right" style={{ color: '#06C270' }}>
                          {val}h
                        </span>
                      </div>
                    </div>
