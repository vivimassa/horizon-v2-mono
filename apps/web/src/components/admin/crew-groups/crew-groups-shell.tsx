...
export function CrewGroupsShell() {
  ...
  const fetchData = useCallback(() => {
    setLoading(true)
    api
      .getCrewGroups(getOperatorId(), true)
      .then((data) => {
        setGroups(data)
        setSelected((prev) => {
          if (prev) {
            const f = data.find((g) => g._id === prev._id)
            if (f) return f
          }
  ...
  const handleCreate = useCallback(
    async (data: Partial<CrewGroupRef>) => {
  ...
  const handleSave = useCallback(
    async (id: string, data: Partial<CrewGroupRef>) => {
    ...
    <MasterDetailLayout
      ...
      center={
        showCreate ? (
          ...
          <CrewGroupDetail group={selected} onSave={handleSave} onDelete={handleDelete} />
    ...
    <div className="flex flex-col items-center justify-center h-full gap-4 px-6">
      ...
      <div className="text-center">
        ...
        <p className="text-[13px] text-hz-text-secondary max-w-sm">
          Crew groups classify crew members for scheduling, reporting, and bulk operations. Seed defaults for a quick
          start, or add groups manually.
        </p>
      </div>
