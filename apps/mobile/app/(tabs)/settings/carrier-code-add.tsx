...
export default function CarrierCodeAddScreen() {
  ...
  const handleCreate = useCallback(async () => {
    if (!form.iataCode.trim() || !form.name.trim()) { setError('IATA code and name are required'); return }
    ...
    try {
      ...
      try { const m = msg.match(/API (\d+): (.+)/); if (m) { const p = JSON.parse(m[2]); msg = Number(m[1]) === 409 ? 'This carrier code already exists.' : p.error || msg } } catch {}
      setError(msg)
    ...
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={['top']}>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 100 }} keyboardShouldPersistTaps="handled">
        <View className={isTablet ? 'flex-row' : ''} style={{ gap: 12, marginBottom: 12 }}>
          <FormField label="IATA Code *" value={form.iataCode} flex={isTablet ? 0.3 : 1}
            onChangeText={(v) => setForm(p => ({ ...p, iataCode: v.toUpperCase() }))}
            palette={palette} mono maxLength={2} placeholder="e.g. VJ" />
          <FormField label="ICAO Code" value={form.icaoCode} flex={isTablet ? 0.3 : 1}
            onChangeText={(v) => setForm(p => ({ ...p, icaoCode: v.toUpperCase() }))}
            palette={palette} mono maxLength={3} placeholder="e.g. VJC" />
          <FormField label="Name *" value={form.name} flex={1}
            onChangeText={(v) => setForm(p => ({ ...p, name: v }))}
            palette={palette} placeholder="e.g. VietJet Air" />
        </View>
        ...
        <View className="flex-row" style={{ gap: 8, marginBottom: 16 }}>
          {CATEGORIES.map(cat => {
            ...
            return (
              <Pressable key={cat} onPress={() => setForm(p => ({ ...p, category: cat }))}
                ...
                <Text style={{ fontSize: 14, fontWeight: active ? '600' : '400', color: active ? accent : palette.text }}>{cat}</Text>
              </Pressable>
            )
          })}
        </View>
        ...
        <Pressable onPress={handleCreate} disabled={creating}
          ...
          <Text style={{ fontSize: 15, fontWeight: '600', color: '#fff' }}>{creating ? 'Creating...' : 'Add Carrier'}</Text>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
  )
