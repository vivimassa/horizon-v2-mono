...
import { Plus, Pencil, Save, X, Trash2, Shield, ArrowDownUp, Hash, Palette, AlertTriangle } from 'lucide-react'
...
export function CrewPositionDetail({ position, onCreate, onSave, onDelete, onDeactivate, onCancel }: DetailProps) {
  ...
  return <ViewEditPanel position={position} onSave={onSave!} onDelete={onDelete!} onDeactivate={onDeactivate!} />
}
...
function CreatePanel({
  ...
  onCreate: (d: Partial<CrewPositionRef>) => Promise<void>
  onCancel: () => void
}) {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
            {form.category === 'cockpit' && (
              <ToggleField
                ...
                value={form.isPic}
                onChange={(v) => set('isPic', v)}
                description="This position is responsible as Pilot in Command"
              />
            )}
            <ToggleField
              label="Can Downrank"
              value={form.canDownrank}
              onChange={(v) => set('canDownrank', v)}
...
function ViewEditPanel({
  ...
  const handleSave = useCallback(async () => {
    ...
    setErrorMsg('')
    try {
      await onSave(position._id, payload)
      setEditing(false)
      setDraft({})
    } catch (err: any) {
      setErrorMsg(friendlyError(err))
  ...
  const handleDeactivate = useCallback(async () => {
    ...
    setErrorMsg('')
    try {
      await onDeactivate(position._id)
      setConfirmDelete(false)
    } catch (err: any) {
      setErrorMsg(friendlyError(err))
    } finally {
  ...
  const getVal = (key: keyof CrewPositionRef) => (key in draft ? (draft as any)[key] : position[key])
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span
              ...
              style={{
                backgroundColor: accentTint(position.color ?? ACCENT, isDark ? 0.15 : 0.1),
                color: position.color ?? ACCENT,
              }}
            >
              {position.code}
            </span>
            ...
            <div>
              ...
              <p className="text-[12px] text-hz-text-secondary">
                Rank {position.rankOrder}
                {position.isPic ? ' \u00b7 Pilot in Command' : ''}
      ...
      <div className="flex-1 overflow-y-auto px-6 pt-4 pb-6">
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[12px] font-bold uppercase tracking-wider text-hz-text-secondary">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
            <FieldRow
              label="Position Code"
              value={<span className="font-mono font-bold">{position.code}</span>}
              editing={editing}
              fieldKey="code"
            ...
            <FieldRow
              label="Rank Order"
              value={<span className="font-mono">{position.rankOrder}</span>}
              editing={editing}
              fieldKey="rankOrder"
              editValue={getVal('rankOrder')}
              onChange={handleFieldChange}
              inputType="number"
            />
          </div>
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
            ...
            <FieldRow
              label="Description"
              value={position.description || <span className="text-hz-text-secondary">No description</span>}
              editing={editing}
        ...
        <div className="rounded-2xl p-5 border border-red-200 dark:border-red-500/20">
          ...
          <p className="text-[13px] text-hz-text-secondary mb-3">
            Deleting a position is only possible if it is not referenced by any expiry codes, crew members, or
            complements. If referenced, you can deactivate it instead.
          </p>
          {confirmDelete ? (
            totalRefs > 0 ? (
              <div className="space-y-3">
                <div className="flex items-start gap-2 px-3 py-2.5 rounded-lg border border-amber-200 bg-amber-50 dark:border-amber-500/20 dark:bg-amber-500/10">
                  ...
                  <div className="text-[13px] text-amber-700 dark:text-amber-400">
                    ...
                    {(refs?.expiryCodes ?? 0) > 0 && (
                      <>
                        <strong>{refs?.expiryCodes}</strong> expiry code{(refs?.expiryCodes ?? 0) !== 1 ? 's' : ''}
                      </>
                    )}
                    {(refs?.expiryCodes ?? 0) > 0 && (refs?.crewComplements ?? 0) > 0 && ' and '}
                    {(refs?.crewComplements ?? 0) > 0 && (
                      <>
                        <strong>{refs?.crewComplements}</strong> crew complement
                        {(refs?.crewComplements ?? 0) !== 1 ? 's' : ''}
                      </>
                    )}
