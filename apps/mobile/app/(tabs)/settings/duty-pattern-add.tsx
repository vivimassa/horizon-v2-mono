...
export default function DutyPatternAddScreen() {
  ...
  const handleCreate = useCallback(async () => {
    ...
    if (form.sequence.length < 2 || form.sequence.length % 2 !== 0) { setError('Sequence must have even number of segments (ON/OFF pairs)'); return }
    if (form.sequence.some(n => n < 1)) { setError('Each segment must be at least 1 day'); return }
    ...
    try {
      ...
      try { const m = msg.match(/API (\d+): (.+)/); if (m) { const p = JSON.parse(m[2]); msg = Number(m[1]) === 409 ? 'This pattern code already exists.' : p.error || msg } } catch {}
      setError(msg)
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 100 }} keyboardShouldPersistTaps="handled">
        ...
        <View className={isTablet ? 'flex-row' : ''} style={{ gap: 12, marginBottom: 16 }}>
          <FormField label="Code *" value={form.code} flex={isTablet ? 0.4 : 1}
            ...
            palette={palette} mono maxLength={20} placeholder="e.g. 52" />
          <FormField label="Description" value={form.description} flex={1}
            onChangeText={(v) => setForm(p => ({ ...p, description: v }))}
            palette={palette} placeholder="e.g. Standard 5-on 2-off" />
        </View>
        ...
        <View className="flex-row flex-wrap mb-4" style={{ gap: 8 }}>
          ...
          <View className="flex-row items-center" style={{ gap: 6 }}>
            <Pressable onPress={() => setForm(p => ({ ...p, sequence: [...p.sequence, 1, 1] }))}
            ...
            {form.sequence.length > 2 && (
              <Pressable onPress={() => setForm(p => ({ ...p, sequence: p.sequence.slice(0, -2) }))}
                className="items-center justify-center rounded-lg active:opacity-60"
        ...
        <View className="flex-row flex-wrap" style={{ gap: 6, marginBottom: 16 }}>
          {OFF_CODES.map(code => {
            ...
            return (
              <Pressable key={code} onPress={() => setForm(p => ({ ...p, offCode: code }))}
        ...
        <Pressable onPress={handleCreate} disabled={creating}
          ...
          <Text style={{ fontSize: 15, fontWeight: '600', color: '#fff' }}>{creating ? 'Creating...' : 'Add Pattern'}</Text>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
  )
...
function FormField({ label, value, onChangeText, palette, flex = 1, mono, maxLength, placeholder }: {
  label: string; value: string; onChangeText: (v: string) => void; palette: any; flex?: number;
  mono?: boolean; maxLength?: number; placeholder?: string
}) {
  return (
