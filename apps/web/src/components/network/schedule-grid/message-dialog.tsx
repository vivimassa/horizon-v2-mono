...
import { MessageSquare, X, RefreshCw, Copy, Check, FlaskConical, AlertTriangle } from 'lucide-react'
...
interface MessageDialogProps {
  ...
  /** True when the grid has unsaved changes (dirty cells / new rows / deleted rows). */
  hasDirty?: boolean
  onClose: () => void
}
...
export function MessageDialog({ dateFrom, dateTo, targetScenarioId, hasDirty = false, onClose }: MessageDialogProps) {
          ...
          !targetScenarioId ? (
            <div className="py-8 text-center space-y-4">
              <div
                ...
                style={{ backgroundColor: 'rgba(253,221,72,0.18)' }}
              >
                <FlaskConical size={22} style={{ color: '#E67A00' }} />
              </div>
              <div>
                <p className="text-[14px] font-semibold text-hz-text">You&apos;re viewing Production</p>
                <p className="text-[13px] text-hz-text-secondary mt-1">
                  ASM/SSM messages are generated by comparing a scenario against the live schedule. Open the Scenario
                  panel and switch to a scenario to see the diff.
                </p>
              </div>
            </div>
          ) : (
            <div className="py-8 text-center space-y-4">
              {hasDirty && (
                <div
                  className="flex items-start gap-2.5 rounded-xl p-3 text-left"
                  style={{
                    background: 'rgba(255,136,0,0.08)',
                    border: '1px solid rgba(255,136,0,0.25)',
                  }}
                >
                  <AlertTriangle size={16} style={{ color: '#FF8800' }} className="shrink-0 mt-0.5" />
                  <div>
                    <p className="text-[13px] font-semibold text-hz-text">Unsaved changes won&apos;t appear</p>
                    <p className="text-[13px] text-hz-text-secondary mt-0.5">
                      ASM/SSM messages are generated by comparing the database record of the scenario against
                      production. Save your current edits (Ctrl+S) first, then generate.
                    </p>
                  </div>
                </div>
              )}
              <p className="text-[14px] text-hz-text-secondary">
                Generate ASM/SSM messages by comparing the active scenario against the production schedule.
              </p>
              <button
