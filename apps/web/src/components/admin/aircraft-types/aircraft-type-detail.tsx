...
export function AircraftTypeDetail({ aircraftType, onSave, onDelete, onCreate, onCancelCreate }: AircraftTypeDetailProps) {
  ...
  const handleCreate = useCallback(async () => {
    if (!onCreate) return;
    if (!createForm.icaoType || !createForm.name) { setCreateError("ICAO type and name are required"); return; }
  ...
  const handleSave = useCallback(async () => {
    if (!onSave || !aircraftType || Object.keys(draft).length === 0) { setEditing(false); return; }
    setSaving(true); setErrorMsg("");
    try { await onSave(aircraftType._id, draft as Partial<AircraftTypeRef>); setEditing(false); setDraft({}); }
  ...
  const handleDelete = useCallback(async () => {
    ...
    setSaving(true); setErrorMsg("");
    try { await onDelete(aircraftType._id); }
    catch (err: any) { setErrorMsg(friendlyError(err)); }
      ...
      <div className="flex flex-col h-full overflow-hidden">
        ...
        <div className="px-6 py-4 space-y-3 overflow-y-auto">
          <div className="flex gap-3">
            <MiniInput label="ICAO Type *" value={createForm.icaoType} maxLength={4}
              onChange={(v) => setCreateForm(p => ({ ...p, icaoType: v.toUpperCase() }))} mono />
        ...
        <div className="px-6 pt-3 pb-6">
          {activeTab === "basic" && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8">
              <FieldRow label="ICAO Type" value={<span className="font-bold font-mono">{aircraftType.icaoType}</span>}
                editing={editing} fieldKey="icaoType" editValue={getVal("icaoType")} onChange={handleFieldChange} />
              <FieldRow label="IATA Type" value={aircraftType.iataType}
                editing={editing} fieldKey="iataType" editValue={getVal("iataType")} onChange={handleFieldChange} />
              <FieldRow label="Name" value={aircraftType.name}
              ...
              <FieldRow label="Family" value={aircraftType.family}
              ...
              <div className="py-2.5 border-b border-hz-border/50">
                ...
                {editing ? (
                  ...
                  <div className="flex items-center gap-2">
                    {aircraftType.color && <span className="w-5 h-5 rounded-full border border-hz-border/50" style={{ backgroundColor: aircraftType.color }} />}
            ...
            <div className="space-y-6">
              <Section title="Weights">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8">
                  <NestedField label="MTOW (kg)" parent="performance" child="mtowKg" type={aircraftType} draft={draft} editing={editing} onChange={handleNestedChange} inputType="number" />
                  <NestedField label="MLW (kg)" parent="performance" child="mlwKg" type={aircraftType} draft={draft} editing={editing} onChange={handleNestedChange} inputType="number" />
                  <NestedField label="MZFW (kg)" parent="performance" child="mzfwKg" type={aircraftType} draft={draft} editing={editing} onChange={handleNestedChange} inputType="number" />
                  <NestedField label="OEW (kg)" parent="performance" child="oewKg" type={aircraftType} draft={draft} editing={editing} onChange={handleNestedChange} inputType="number" />
                </div>
              </Section>
              <Section title="Fuel">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
                  <NestedField label="Max Fuel Capacity (kg)" parent="performance" child="maxFuelCapacityKg" type={aircraftType} draft={draft} editing={editing} onChange={handleNestedChange} inputType="number" />
                  <FieldRow label="Fuel Burn Rate (kg/hr)" value={aircraftType.fuelBurnRateKgPerHour}
                    editing={editing} fieldKey="fuelBurnRateKgPerHour" editValue={getVal("fuelBurnRateKgPerHour")} onChange={handleFieldChange} inputType="number" />
                </div>
              </Section>
              <Section title="Speed & Range">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-x-8">
                  <NestedField label="Cruising Speed (kts)" parent="performance" child="cruisingSpeedKts" type={aircraftType} draft={draft} editing={editing} onChange={handleNestedChange} inputType="number" />
                  <NestedField label="Max Range (NM)" parent="performance" child="maxRangeNm" type={aircraftType} draft={draft} editing={editing} onChange={handleNestedChange} inputType="number" />
                  <NestedField label="Ceiling (FL)" parent="performance" child="ceilingFl" type={aircraftType} draft={draft} editing={editing} onChange={handleNestedChange} inputType="number" />
                </div>
              </Section>
              <Section title="ETOPS">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
              ...
              <Section title="Classifications">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
                  <FieldRow label="Noise Category" value={aircraftType.noiseCategory}
                    editing={editing} fieldKey="noiseCategory" editValue={getVal("noiseCategory")} onChange={handleFieldChange} />
                  <FieldRow label="Emissions Category" value={aircraftType.emissionsCategory}
                    editing={editing} fieldKey="emissionsCategory" editValue={getVal("emissionsCategory")} onChange={handleFieldChange} />
                </div>
              </Section>
            </div>
          )}
                  ...
                  <table className="w-full text-[13px]">
                    ...
                    <tbody>
                      ...
                      <tr className="border-t border-hz-border/50">
                        ...
                        <TATCell field="tat.minIi" type={aircraftType} draft={draft} editing={editing} onChange={handleNestedChange} />
                      </tr>
                    </tbody>
                  </table>
                </div>
          ...
          {activeTab === "cargo" && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
              <NestedField label="Max Cargo Weight (kg)" parent="cargo" child="maxCargoWeightKg" type={aircraftType} draft={draft} editing={editing} onChange={handleNestedChange} inputType="number" />
              <NestedField label="Cargo Positions (ULD)" parent="cargo" child="cargoPositions" type={aircraftType} draft={draft} editing={editing} onChange={handleNestedChange} inputType="number" />
                ...
                {editing ? (
                  <input type="text"
                    value={(() => {
                      ...
                      if (draftCargo && "uldTypesAccepted" in draftCargo) return (draftCargo.uldTypesAccepted as string[])?.join(", ") ?? "";
                  ...
                  <div className="text-[13px] font-medium">{aircraftType.cargo?.uldTypesAccepted?.join(", ") || "—"}</div>
                )}
              </div>
              <FieldRow label="Notes" value={aircraftType.notes}
                editing={editing} fieldKey="notes" editValue={getVal("notes")} onChange={handleFieldChange} />
            ...
            <div className="space-y-6">
                  ...
                  <div className="py-2.5 border-b border-hz-border/50">
                    ...
                    {editing ? (
                      <select
                        ...
                        className="w-full text-[13px] font-medium bg-transparent border-b border-hz-accent/30 outline-none focus:border-hz-accent py-0.5 text-hz-text">
                        <option value="">—</option>
                        {REST_CLASSES.map(c => <option key={c} value={c}>{c}</option>)}
                      </select>
                    ) : (
                      <div className="text-[13px] font-medium">{aircraftType.crewRest?.cockpitClass || "—"}</div>
                    )}
                  </div>
                  <NestedField label="Positions" parent="crewRest" child="cockpitPositions" type={aircraftType} draft={draft} editing={editing} onChange={handleNestedChange} inputType="number" />
                </div>
              </Section>
              <Section title="Cabin Rest Facility">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
                  ...
                  <NestedField label="Positions" parent="crewRest" child="cabinPositions" type={aircraftType} draft={draft} editing={editing} onChange={handleNestedChange} inputType="number" />
                </div>
              </Section>
            </div>
          )}
          ...
          {activeTab === "weather" && (
            <div className="space-y-6">
              <Section title="Weather Limitations">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8">
                  <NestedField label="Min Ceiling (ft)" parent="weather" child="minCeilingFt" type={aircraftType} draft={draft} editing={editing} onChange={handleNestedChange} inputType="number" />
                  <NestedField label="Min RVR (m)" parent="weather" child="minRvrM" type={aircraftType} draft={draft} editing={editing} onChange={handleNestedChange} inputType="number" />
                  <NestedField label="Min Visibility (m)" parent="weather" child="minVisibilityM" type={aircraftType} draft={draft} editing={editing} onChange={handleNestedChange} inputType="number" />
                  <NestedField label="Max Crosswind (kt)" parent="weather" child="maxCrosswindKt" type={aircraftType} draft={draft} editing={editing} onChange={handleNestedChange} inputType="number" />
                  <NestedField label="Max Wind (kt)" parent="weather" child="maxWindKt" type={aircraftType} draft={draft} editing={editing} onChange={handleNestedChange} inputType="number" />
                </div>
              </Section>
                  ...
                  <div className="py-2.5 border-b border-hz-border/50">
                    ...
                    {editing ? (
                      <select
                        ...
                        className="w-full text-[13px] font-medium bg-transparent border-b border-hz-accent/30 outline-none focus:border-hz-accent py-0.5 text-hz-text">
                        <option value="">—</option>
                        {ILS_CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                      </select>
                    ) : (
                  ...
                  <div className="py-2.5 border-b border-hz-border/50">
                    ...
                    {editing ? (
                      <button
                        ...
                        style={{
                          backgroundColor: getNestedVal("approach", "autolandCapable") ? "rgba(22,163,74,0.1)" : "rgba(220,38,38,0.1)",
...
function NestedField({ label, parent, child, type, draft, editing, onChange, inputType = "text" }: {
  label: string; parent: string; child: string; type: AircraftTypeRef; draft: Record<string, unknown>;
  editing: boolean; onChange: (path: string, value: string | number | boolean | null) => void; inputType?: "text" | "number";
}) {
  const draftParent = draft[parent] as Record<string, unknown> | undefined;
  const value = draftParent && child in draftParent
    ? draftParent[child]
...
function TATCell({ field, type, draft, editing, onChange }: {
  field: string; type: AircraftTypeRef; draft: Record<string, unknown>;
  editing: boolean; onChange: (path: string, value: string | number | boolean | null) => void;
}) {
  const [parent, child] = field.split(".");
  const draftParent = draft[parent] as Record<string, unknown> | undefined;
  const minutes = draftParent && child in draftParent
