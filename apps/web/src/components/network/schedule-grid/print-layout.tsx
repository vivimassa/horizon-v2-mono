    ...
    <div className="fixed inset-0 z-50 bg-white dark:bg-hz-bg overflow-auto">
      ...
      <div className="print:hidden flex items-center justify-between px-6 py-3 border-b border-hz-border bg-hz-card">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[13px] text-hz-text-secondary">
            {rows.length} flights — Season {seasonCode}
          </span>
        </div>
      ...
      <div className="p-6 max-w-[1200px] mx-auto">
        ...
        <table className="w-full text-[11px] border-collapse" style={{ fontFamily: 'Consolas, monospace' }}>
          <thead>
            <tr>
              <th className="px-1 py-1.5 text-left font-semibold border-b-2 border-hz-text text-[10px] uppercase tracking-wider">
                #
              </th>
              {GRID_COLUMNS.map((col) => (
                <th
                  key={col.key}
                  className="px-1 py-1.5 text-center font-semibold border-b-2 border-hz-text text-[10px] uppercase tracking-wider"
                >
                  {col.label}
                </th>
              ))}
            ...
            {rows.map((row, i) => {
              ...
              return (
                <tr key={row._id} className={i % 2 === 1 ? 'bg-gray-50 dark:bg-gray-900/20' : ''}>
                  ...
                  <td className="px-1 py-1 border-b border-hz-border/30 text-center">
                    {row.rotationLabel ?? row.aircraftTypeIcao ?? '—'}
                  </td>
                  ...
                  <td
                    className="px-1 py-1 border-b border-hz-border/30 text-center"
                    style={{ color: '#E63535', fontWeight: 600 }}
                  >
                    {row.airlineCode}
                    {row.flightNumber}
                  </td>
