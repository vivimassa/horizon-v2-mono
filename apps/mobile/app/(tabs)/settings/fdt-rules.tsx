...
import {
  ChevronLeft, ChevronRight, ShieldCheck, Sparkles, Check, RefreshCw,
  Clock, Globe, Timer, Users, Plane, BedDouble, Moon, Shield,
  AlertTriangle, Wrench, Radio, Settings,
...
export default function FdtRulesScreen() {
  ...
  const handleSeed = useCallback(async (code: string) => {
    setSeeding(true)
    setError(null)
    try {
      await api.seedFdtl(operatorId, code)
      await fetchData()
    } catch (err: any) { setError(err.message || 'Failed to seed framework') }
        ...
        <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={[]}>
          ...
          <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 100 }}>
            <Text style={{ fontSize: 15, color: palette.textSecondary, marginBottom: 16, lineHeight: 22 }}>
              Select the regulatory framework for your operation. This will populate all FDP tables, rest requirements, cumulative limits, and augmented crew rules.
            </Text>
            ...
            <View className="flex-row flex-wrap" style={{ gap: 10 }}>
              {frameworks.map(fw => {
                ...
                return (
                  <Pressable key={fw.code} onPress={() => setSelectedFw(fw.code)}
                    ...
                    <View className="flex-row items-center mb-1" style={{ gap: 6 }}>
                      ...
                      <Text style={{ fontSize: 14, fontWeight: '600', color: palette.text }} numberOfLines={1}>{fw.name}</Text>
                    </View>
                    <Text style={{ fontSize: 13, color: palette.textSecondary }}>{fw.region}</Text>
                    <Text style={{ fontSize: 12, color: palette.textTertiary, marginTop: 2 }} numberOfLines={1}>{fw.legalBasis}</Text>
                    {isSel && (
            ...
            <Pressable onPress={() => selectedFw && handleSeed(selectedFw)}
    ...
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={[]}>
      <View className="px-4 pt-2 pb-3" style={{ borderBottomWidth: 1, borderBottomColor: palette.border }}>
        <View className="flex-row items-center">
          ...
          <View className="items-center justify-center rounded-lg mr-3"
            style={{ width: 36, height: 36, backgroundColor: accentTint(activeFramework?.color ?? accent, isDark ? 0.15 : 0.1) }}>
            <ShieldCheck size={18} color={activeFramework?.color ?? accent} strokeWidth={1.8} />
          </View>
          <View className="flex-1">
            <Text style={{ fontSize: 20, fontWeight: '700', color: palette.text }}>FDT Rules</Text>
            <View className="flex-row items-center mt-1" style={{ gap: 6 }}>
              <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: activeFramework?.color ?? accent }} />
        ...
        <View className="flex-row items-center mt-3" style={{ gap: 16, marginLeft: 60 }}>
          <View className="flex-row items-center" style={{ gap: 4 }}>
            <View className="px-1.5 rounded" style={{ backgroundColor: isDark ? 'rgba(253,172,66,0.15)' : 'rgba(255,136,0,0.12)' }}>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 100 }}>
        {tabGroups.map(group => {
          ...
          const visibleTabs = group.tabs.filter(tab =>
            syntheticTabs.has(tab.key) || (ruleCountByTab.get(tab.key) ?? 0) + (tableCountByTab.get(tab.key) ?? 0) > 0
          )
          ...
          return (
            <View key={group.key} className="mb-5">
              ...
              <View className="flex-row items-center mb-2" style={{ gap: 6 }}>
                <View style={{ width: 3, height: 16, borderRadius: 2, backgroundColor: activeFramework?.color ?? accent }} />
                <Text style={{ fontSize: 13, fontWeight: '600', color: palette.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5 }}>
                  {group.label}
                </Text>
              </View>
                ...
                {visibleTabs.map(tab => {
                  ...
                  return (
                    <Pressable key={tab.key}
                      onPress={() => {
                        if (isReporting) {
                          router.push({ pathname: '/(tabs)/settings/fdt-scheme-settings' as any, params: { schemeId: scheme._id } })
                        } else {
                          router.push({ pathname: '/(tabs)/settings/fdt-rule-tab' as any, params: { tabKey: tab.key, frameworkCode: scheme.frameworkCode } })
                        }
                      }}
                      ...
                      <Text style={{ fontSize: 15, fontWeight: '500', color: palette.text, flex: 1, marginLeft: 10 }}>{tab.label}</Text>
                      {!isReporting && total > 0 && (
                        <Text style={{ fontSize: 13, color: palette.textTertiary, fontFamily: 'monospace', marginRight: 6 }}>{total}</Text>
                      )}
