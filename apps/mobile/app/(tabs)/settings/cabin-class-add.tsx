      ...
      >
        ...
        >
          {SEAT_PRESETS.map((preset) => {
            ...
            return (
              ...
              >
                <Text
                  style={{ fontSize: 12, fontWeight: active ? '700' : '500', color: active ? accent : palette.text }}
                >
                  {preset.label}
                </Text>
              </Pressable>
        ...
        <View className="flex-row flex-wrap" style={{ gap: 6, marginBottom: 12 }}>
          {SEAT_TYPES.map((st) => {
            ...
            return (
              <Pressable
                key={st}
                onPress={() => setForm((p) => ({ ...p, seatType: st }))}
        ...
        <View className="flex-row" style={{ gap: 12, marginBottom: 16 }}>
          <ToggleChip
            label="IFE Screen"
            value={form.hasIfe}
            onToggle={() => setForm((p) => ({ ...p, hasIfe: !p.hasIfe }))}
            palette={palette}
            isDark={isDark}
            accent={accent}
          />
          <ToggleChip
            label="Power Outlet"
            value={form.hasPower}
            onToggle={() => setForm((p) => ({ ...p, hasPower: !p.hasPower }))}
            palette={palette}
            isDark={isDark}
            accent={accent}
          />
