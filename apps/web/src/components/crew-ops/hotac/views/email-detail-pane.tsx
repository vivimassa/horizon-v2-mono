...
export function EmailDetailPane({ email, onChanged }: EmailDetailPaneProps) {
  ...
  const handleRelease = async () => {
    setBusy('release')
    try {
      await api.releaseHotelEmails([email._id])
      await onChanged()
    } catch (err) {
      console.warn('[hotac] release failed', err)
    } finally {
      setBusy(null)
    }
  }
  ...
  const handleDiscard = async () => {
    setBusy('discard')
    try {
      await api.discardHotelEmails([email._id])
      await onChanged()
    } catch (err) {
      console.warn('[hotac] discard failed', err)
    } finally {
      setBusy(null)
    }
  }
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="px-5 py-4 border-b border-hz-border flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[13px] text-hz-text-secondary truncate">
            ...
            {email.hotelName ? ` · ${email.hotelName}` : ''}
          </div>
        </div>
      </div>
      ...
      {email.status === 'held' && (
        <div className="mx-5 mt-3 px-3 py-2.5 rounded-xl bg-[#FF8800]/10 border border-[#FF8800]/25 flex items-start gap-2">
          ...
          <div className="text-[13px] text-hz-text leading-snug">
            This email has not been sent. You can still amend the subject, body, and recipients before clicking{' '}
            <span className="font-semibold">Release</span>.
          </div>
        </div>
      )}

      {/* Failed / partial banner */}
      ...
      {/* Footer actions — only when held/draft */}
      {editable && (
