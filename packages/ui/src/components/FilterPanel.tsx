...
export const FilterPanel = memo(function FilterPanel({
  children, onApply, applyDisabled, applyLabel = 'Go', loading,
  activeCount = 0, collapsible = true, initialCollapsed = false,
  width = EXPANDED_WIDTH, bottomInset = 0, accent, palette, isDark,
}: FilterPanelProps) {
      ...
      <Pressable onPress={() => setCollapsed(false)}
        ...
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
          <View style={{ width: 80, alignItems: 'center', transform: [{ rotate: '-90deg' }] }}>
            <Text style={{ fontSize: 12, fontWeight: '700', letterSpacing: 2, color: palette.textTertiary }} numberOfLines={1}>
              FILTERS
            </Text>
          </View>
        </View>
  ...
  return (
    <View style={{
      ...
      <View className="flex-row items-center px-4" style={{
        ...
        {activeCount > 0 && (
          <View className="items-center justify-center rounded-full mr-2"
            style={{ width: 20, height: 20, backgroundColor: accent }}>
            <Text style={{ fontSize: 11, fontWeight: '700', color: '#fff' }}>{activeCount}</Text>
          </View>
        )}
        {collapsible && (
          <Pressable onPress={() => setCollapsed(true)} className="items-center justify-center active:opacity-60"
      ...
      {/* Footer: Apply button */}
      <View style={{ paddingHorizontal: 16, paddingTop: 12, paddingBottom: Math.max(16, bottomInset), borderTopWidth: 1, borderTopColor: isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)' }}>
        {applyDisabled && (
          <Text style={{ fontSize: 13, color: isDark ? '#f87171' : '#dc2626', marginBottom: 8, textAlign: 'center' }}>
            Select the period to continue
          </Text>
        )}
        <Pressable onPress={handleApply} disabled={applyDisabled || loading}
