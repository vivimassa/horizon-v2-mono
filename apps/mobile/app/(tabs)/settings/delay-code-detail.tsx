...
export default function DelayCodeDetailScreen() {
  ...
  useEffect(() => {
    ...
    api
      .getDelayCodes()
      .then((codes) => setCode(codes.find((c) => c._id === id) ?? null))
      .catch(console.error)
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaving(true)
    try {
      const updated = await api.updateDelayCode(code._id, draft)
      setCode(updated)
      setDraft({})
      setEditing(false)
    } catch (err: any) {
  ...
  const handleDelete = useCallback(() => {
    ...
    Alert.alert('Delete Delay Code', `Delete ${code.code} — ${code.name}?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await api.deleteDelayCode(code._id)
            router.back()
          } catch (err: any) {
      ...
      <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={['top']}>
        <View className="flex-1 justify-center items-center">
          <Text style={{ fontSize: 15, color: palette.textTertiary }}>
            {loading ? 'Loading...' : 'Delay code not found'}
          </Text>
        </View>
      </SafeAreaView>
    )
      ...
      <View className="px-4 pt-4 pb-4" style={{ borderBottomWidth: 1, borderBottomColor: palette.border }}>
          ...
          <View className="flex-row items-center flex-1 mr-2">
            ...
            {code.alphaCode && (
              <View className="px-2 py-0.5 rounded mr-2" style={{ backgroundColor: `${catColor}20` }}>
                <Text style={{ fontSize: 13, fontWeight: '700', fontFamily: 'monospace', color: catColor }}>
                  {code.alphaCode}
                </Text>
              </View>
            )}
        ...
        <View className="flex-row items-center mt-2 flex-wrap" style={{ gap: 6, marginLeft: 36 }}>
          ...
          {code.isIataStandard && (
            ...
            >
              <Text style={{ fontSize: 11, fontWeight: '600', color: isDark ? '#818cf8' : '#4338ca' }}>
                IATA Standard
              </Text>
            </View>
          )}
      ...
      >
        ...
        >
          ...
          {editing ? (
            <View className="flex-row items-center justify-center" style={{ gap: 8 }}>
              <TripleBox
                label="Process"
                value={get('ahm732Process')}
                onChange={(v) => handleFieldChange('ahm732Process', v)}
                palette={palette}
                accent={accent}
                isDark={isDark}
              />
              ...
              <TripleBox
                label="Reason"
                value={get('ahm732Reason')}
                onChange={(v) => handleFieldChange('ahm732Reason', v)}
                palette={palette}
                accent={accent}
                isDark={isDark}
              />
              ...
              <TripleBox
                label="Stakeholder"
                value={get('ahm732Stakeholder')}
                onChange={(v) => handleFieldChange('ahm732Stakeholder', v)}
                palette={palette}
                accent={accent}
                isDark={isDark}
              />
        ...
        <View className={isTablet ? 'flex-row flex-wrap' : ''}>
          ...
          <Field
            ...
            value={code.alphaCode}
            editing={editing}
            fieldKey="alphaCode"
            editValue={get('alphaCode')}
            onChange={handleFieldChange}
            palette={palette}
            mono
            maxLength={2}
          ...
          <ToggleField
            label="IATA Standard"
            value={code.isIataStandard}
            editing={editing}
            fieldKey="isIataStandard"
            editValue={get('isIataStandard')}
            onChange={handleFieldChange}
            palette={palette}
            isDark={isDark}
            half={isTablet}
          ...
          {options.map((opt) => {
            ...
            return (
              <Pressable
                ...
                style={{
                  backgroundColor: active ? accentTint(optColor || accent, isDark ? 0.15 : 0.08) : 'transparent',
                  borderWidth: 1,
                  borderColor: active ? optColor || accent : palette.cardBorder,
                  gap: 4,
                }}
              >
                ...
                <Text
                  style={{
                    ...
                    color: active ? optColor || accent : palette.text,
                  }}
                >
                  {opt}
                </Text>
