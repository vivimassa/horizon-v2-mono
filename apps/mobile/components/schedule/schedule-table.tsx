...
const STATUS_COLORS: Record<string, string> = {
  draft: '#6b7280', active: '#16a34a', suspended: '#f59e0b', cancelled: '#dc2626',
}

interface ColDef {
  key: string; label: string; width: number; editable: boolean; mono?: boolean
}
...
export const ScheduleTable = memo(function ScheduleTable({
  flights, dirtyMap, newIds, deletedIds, selectedId,
  onSelect, onCellEdit, onLongPress,
  palette, accent, isDark, sortKey, sortDir, onSort,
}: {
  flights: ScheduledFlightRef[]
  dirtyMap: Map<string, Partial<ScheduledFlightRef>>
  newIds: Set<string>; deletedIds: Set<string>
    ...
    if (isEditing) {
      return (
        <TextInput value={editValue} onChangeText={setEditValue}
          ...
          keyboardType={col.key === 'departureDayOffset' ? 'numeric' : (col.key === 'stdUtc' || col.key === 'staUtc') ? 'numbers-and-punctuation' : 'default'}
          style={{ fontSize: 13, fontWeight: '600', fontFamily: col.mono ? 'monospace' : undefined,
            color: accent, textAlign: 'center', width: col.width - 8, height: ROW_HEIGHT - 8,
            borderWidth: 1, borderColor: accent, borderRadius: 4, backgroundColor: palette.card }} />
      )
    }

    return (
      <Pressable onPress={() => col.editable && !isDeleted ? startEdit(flight._id, col, val) : onSelect(flight._id)}>
        <Text style={{ fontSize: 13, fontWeight: '500', fontFamily: col.mono ? 'monospace' : undefined,
  ...
  const renderHeaderCells = (cols: ColDef[]) => (
    <View className="flex-row" style={{ height: HEADER_HEIGHT, backgroundColor: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.03)' }}>
      {cols.map(col => (
        <Pressable key={col.key} onPress={() => onSort(col.key)}
          style={{ width: col.width, justifyContent: 'center', alignItems: 'center', borderRightWidth: 1, borderBottomWidth: 1, borderColor: palette.cardBorder }}>
          <Text style={{ fontSize: 11, fontWeight: '700', color: sortKey === col.key ? accent : palette.textSecondary, textTransform: 'uppercase' }}>
  ...
  const renderRow = ({ item: flight }: { item: ScheduledFlightRef }) => {
    ...
    const rowBg = isSelected ? accentTint(accent, isDark ? 0.1 : 0.04)
      : isDeleted ? (isDark ? 'rgba(220,38,38,0.06)' : 'rgba(220,38,38,0.03)')
      : flight.status === 'suspended' ? (isDark ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.015)')
      : 'transparent'
    const rowOpacity = isDeleted ? 0.4 : flight.status === 'suspended' ? 0.6 : 1

    return (
      <View>
        <Pressable onPress={() => onSelect(flight._id)} onLongPress={() => onLongPress(flight._id)}
          className="flex-row" style={{
            height: ROW_HEIGHT, backgroundColor: rowBg, opacity: rowOpacity,
            borderLeftWidth: isDirty || isNew ? 3 : 0,
            borderLeftColor: isNew ? '#16a34a' : accent,
          }}>
          {/* Sticky columns */}
          {STICKY_COLS.map(col => (
            <View key={col.key} style={{ width: col.width, justifyContent: 'center', alignItems: 'center',
              borderRightWidth: 1, borderBottomWidth: 1, borderColor: palette.cardBorder }}>
        ...
        {hasSeparator && <View style={{ height: 8, backgroundColor: isDark ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.02)' }} />}
      </View>
    )
  }
  ...
  return (
    <ScrollView horizontal showsHorizontalScrollIndicator={true}>
      <View style={{ width: totalWidth }}>
        ...
        {/* Data rows */}
        <FlatList data={flights} keyExtractor={f => f._id} renderItem={renderRow}
