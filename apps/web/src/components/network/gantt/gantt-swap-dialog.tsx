  ...
  const analysis = useMemo(() => {
    ...
    if (sourceType && targetType && sourceType !== targetType) {
      warnings.push({
        ...
        type: 'type_mismatch',
        message:
          'Aircraft type mismatch: Seat capacity, crew qualifications, payload limits, and performance data may differ.',
      })
    }
    ...
    >
      ...
      >
        ...
        <div className="px-5 py-4 space-y-4 overflow-y-auto" style={{ maxHeight: 'calc(80vh - 130px)' }}>
          {/* Date range */}
          <div className="text-[12px] font-medium" style={{ color: textMuted }}>
            {analysis.dateRange}
          </div>
          ...
          {analysis.warnings.length > 0 && (
            ...
            >
              ...
              {analysis.warnings.map((w, i) => (
                <div
                  key={i}
                  className="flex items-start gap-1.5 text-[12px] pl-5"
                ...
                >
                  ...
                  <span>
                    {w.aircraft && (
                      <>
                        <span className="font-medium">{w.aircraft}</span>:{' '}
                      </>
                    )}
                    {w.message}
                  </span>
...
function AcPill({
  ...
  return (
    ...
    >
      <span className="text-[12px] font-bold" style={{ color: textColor }}>
        {reg}
      </span>
      {type && (
        <span
  ...
  return (
    <div className="rounded-xl p-3 space-y-3" style={{ background: cardBg, border: `1px solid ${border}` }}>
      ...
      <div className="flex flex-wrap gap-1.5">
        {flights.length === 0 ? (
          <span className="text-[12px] italic" style={{ color: textMuted }}>
            No flights
          </span>
        ) : (
          flights.map((f) => {
