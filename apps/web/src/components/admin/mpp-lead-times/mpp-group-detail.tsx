...
import { Pencil, Save, X, Trash2, Plus, Clock, FileText, PlaneTakeoff, UsersRound, Layers } from 'lucide-react'
...
export function MppGroupDetail({
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaving(true)
    try {
      await onSaveGroup(group._id, draft)
      setEditing(false)
      setDraft({})
    } catch {
  ...
  const handleCreateGroup = useCallback(async () => {
    if (!createForm.label || !createForm.code) {
      setCreateError('Label and code are required')
      return
    }
    try {
      ...
      <div className="px-6 py-4 border-b border-hz-border shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span
              className="text-[13px] font-bold px-2.5 py-1 rounded text-white"
        ...
        {group.description && <p className="text-[13px] text-hz-text-secondary mt-1">{group.description}</p>}
          ...
          {addingItem ? (
            <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-dashed border-hz-border">
              <input
                ...
                className="flex-1 text-[13px] px-2 py-1.5 rounded-lg border border-hz-border bg-hz-bg outline-none text-hz-text"
                autoFocus
              />
              <input
                type="text"
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-lg text-[13px] font-medium text-white bg-module-accent"
              >
                Add
              </button>
              <button
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          {editingMonths ? (
            <input
              ...
              onBlur={commitMonths}
              onKeyDown={(e) => {
                if (e.key === 'Enter') commitMonths()
                if (e.key === 'Escape') setEditingMonths(false)
              }}
            ...
            <button
              onClick={() => {
                setDraftMonths(item.valueMonths)
        ...
        {confirmDel ? (
          <div className="flex items-center gap-1">
            <button
              ...
              className="px-2 py-1 rounded-lg text-[11px] font-semibold text-white bg-red-500"
            >
              Yes
            </button>
            <button
              onClick={() => setConfirmDel(false)}
              className="px-2 py-1 rounded-lg text-[11px] font-medium text-hz-text-secondary"
            >
              No
            </button>
          </div>
