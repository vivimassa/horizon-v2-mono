...
export function CrewComplementTypeList({ aircraftTypes, byType, selectedType, onSelect, loading }: Props) {
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 py-3 border-b border-hz-border shrink-0">
        ...
        <p className="text-[11px] text-hz-text-tertiary mt-0.5">Select a type to configure crew complements</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2">
        {loading ? (
          ...
          aircraftTypes.map((t) => {
            ...
            const hasRest = t.crewRest?.cockpitClass != null || t.crewRest?.cabinClass != null
            const restPositions = (t.crewRest?.cockpitPositions ?? 0) + (t.crewRest?.cabinPositions ?? 0)
              ...
              >
                <span
                  className={`shrink-0 font-mono font-bold text-[11px] px-1.5 py-0.5 rounded tabular-nums ${
                    isSelected ? 'text-white' : 'bg-hz-border/40 text-hz-text-secondary'
                  }`}
                  style={isSelected ? { backgroundColor: ACCENT } : undefined}
                >
                  {t.icaoType}
                </span>
                <span className="flex-1 min-w-0 truncate text-[13px] text-hz-text">{t.name}</span>
                {hasData && hasRest && (
                ...
                {!hasData && <Plus className="h-3 w-3 shrink-0 text-hz-text-tertiary" />}
                {hasData && !hasRest && <span className="text-[10px] text-hz-text-tertiary">{rows.length}</span>}
              </button>
            )
          })
        )}
