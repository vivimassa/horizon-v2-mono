...
export async function scenarioRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/scenarios/:id/publish-merge', async (req, reply) => {
    ...
    const COMPARE_FIELDS = [
      'flightNumber', 'depStation', 'arrStation', 'stdUtc', 'staUtc',
      'daysOfWeek', 'effectiveFrom', 'effectiveUntil',
      'aircraftTypeIcao', 'blockMinutes', 'serviceType',
      'status', 'cockpitCrewRequired', 'cabinCrewRequired',
      'departureDayOffset', 'arrivalDayOffset',
    ] as const
  ...
  app.get('/scenarios/:id/diff-preview', async (req, reply) => {
    ...
    for (const sf of scenarioFlights) {
      const srcId = sf.sourceFlightId as string | null
      if (!srcId) { added++; continue }
      referencedProdIds.add(srcId)
      const prod = prodMap.get(srcId)
      if (!prod) { added++; continue }
      if (sf.status === 'cancelled') { deleted++; continue }

      let hasChange = false
      for (const field of COMPARE_FIELDS) {
        if (String(sf[field] ?? '') !== String(prod[field] ?? '')) { hasChange = true; break }
      }
