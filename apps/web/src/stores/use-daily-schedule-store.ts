...
export interface DailyScheduleState {
  ...
  /* ── Actions ── */
  setFlights: (flights: Flight[], airports: AirportRef[], regs?: AircraftRegistrationRef[], acTypes?: AircraftTypeRef[]) => void
  commitFilters: () => void
...
function deriveHomeBaseOffset(flights: Flight[], airportMap: Record<string, { utcOffset: number }>): number {
  ...
  let best = 7; let bestCount = 0
  counts.forEach((c, o) => { if (c > bestCount) { best = o; bestCount = c } })
  return best
}
...
export const useDailyScheduleStore = create<DailyScheduleState>((set, get) => ({
  ...
  reset: () => set({
    flights: [], airportMap: {}, homeBaseOffset: 7,
    dateFrom: "", dateTo: "", timeFrom: "", timeTo: "",
    acTypes: null, acRegs: null, domInt: "all", statuses: null, showCancelled: false,
    committed: null,
    timeModes: new Set<TimeMode>(["utc"]),
    sortSequence: "date-reg-std",
    columnOrder: [...DEFAULT_COLUMN_ORDER],
...
export function applyOffset(hhmmUtc: string, offsetHours: number): { time: string; dayShift: number } {
  ...
  else if (totalMin < 0) { totalMin += 1440; dayShift = -1 }
  return {
...
export function getSortedFlights(flights: Flight[], sortSequence: SortSequence): Flight[] {
  ...
  sorted.sort((a, b) => {
    switch (sortSequence) {
      ...
      case "date-reg-std":
        return a.operatingDate.localeCompare(b.operatingDate)
          || (a.tail.registration ?? "").localeCompare(b.tail.registration ?? "")
          || (a.schedule.stdUtc ?? 0) - (b.schedule.stdUtc ?? 0)
      case "reg-date-std":
        return (a.tail.registration ?? "zzz").localeCompare(b.tail.registration ?? "zzz")
...
export function getGroupKey(flight: Flight, sortSequence: SortSequence): string {
  switch (sortSequence) {
    case "date-std": return flight.operatingDate
    case "date-reg-std": return `${flight.operatingDate}__${flight.tail.registration ?? "__unassigned__"}`
    case "reg-date-std": return flight.tail.registration ?? "__unassigned__"
    case "flt-date": return flight.flightNumber
  }
}
