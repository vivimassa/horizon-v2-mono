...
export const GridCell = memo(function GridCell({
  flight, col, value, isEditing, editValue, onEditChange,
  isSelected, isInRange, isClipboard, clipboardMode,
  cellFormat, condFormat,
  onTap, onDoubleTap, onCommit, onCancel,
  palette, accent, isDark, isDeleted, isCancelled,
}: {
  ...
  if (col.key === 'status') {
    ...
    return (
      <View style={cellContainerStyle(col, isSelected, isInRange, isClipboard, clipboardMode, fmt, palette, accent, isDark)}>
  ...
  if (isEditing) {
    return (
      <View style={cellContainerStyle(col, true, false, false, null, fmt, palette, accent, isDark)}>
        <TextInput value={editValue} onChangeText={onEditChange}
          onBlur={onCommit} onSubmitEditing={onCommit}
          autoFocus autoCapitalize="characters"
          keyboardType={col.key === 'departureDayOffset' ? 'numeric'
            : (col.type === 'time') ? 'numbers-and-punctuation' : 'default'}
          maxLength={col.maxLength}
          style={{
            fontSize: 13, fontWeight: '600',
            fontFamily: col.mono ? 'monospace' : undefined,
            color: accent, textAlign: 'center',
            width: col.width - 6, height: 34,
...
function cellContainerStyle(
  col: GridColumn, isSelected: boolean, isInRange: boolean,
  isClipboard: boolean, clipboardMode: 'copy' | 'cut' | null,
  fmt: Partial<CellFormat>, palette: Palette, accent: string, isDark: boolean,
) {
  return {
    width: col.width,
    height: 44,
    ...
    borderColor: isSelected ? accent
      : isClipboard ? (clipboardMode === 'cut' ? '#dc2626' : accent)
      : palette.cardBorder,
    borderStyle: (isClipboard && clipboardMode === 'cut' ? 'dashed' : 'solid') as any,
    backgroundColor: fmt.bgColor
      ?? (isSelected ? accentTint(accent, isDark ? 0.12 : 0.06)
        : isInRange ? accentTint(accent, isDark ? 0.06 : 0.03)
        : undefined),
  }
}
