      ...
      <div className="flex flex-1 overflow-hidden gap-4 px-4 pb-4">
          ...
          <div className="px-5 py-5 flex-1">
            ...
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center gap-2">
                {isDark ? <Moon size={16} style={{ color: selectedAccent }} /> : <Sun size={16} style={{ color: selectedAccent }} />}
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: selectedAccent }}>
            ...
            <div className="flex flex-col gap-2">
              <PreviewPill label="Compact mode" on={compactMode} palette={palette} accent={selectedAccent} isDark={isDark} />
              <PreviewPill label="Animated backgrounds" on={animatedBg} palette={palette} accent={selectedAccent} isDark={isDark} />
              <PreviewPill label="Reduce motion" on={reduceMotion} palette={palette} accent={selectedAccent} isDark={isDark} />
            </div>
          </div>
        </aside>
        ...
        <section className="flex-1 overflow-y-auto">
          <div className="flex flex-col gap-4">
            ...
            <GlassCard title="Theme Mode" icon={isDark ? Moon : Sun} palette={palette} isDark={isDark} glass={glass} accent={selectedAccent}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <ThemeCard
                  ...
                  preview={
                    <div className="w-full h-16 rounded-lg mb-2 overflow-hidden" style={{ background: "linear-gradient(135deg, #f8fafc, #e2e8f0)" }}>
                ...
                <ThemeCard
                  ...
                  onClick={() => { if (!isDark) toggle(); }}
                  preview={
                    <div className="w-full h-16 rounded-lg mb-2 overflow-hidden" style={{ background: "linear-gradient(135deg, #1e1e2e, #0f0f1a)" }}>
                      <div className="flex gap-1.5 p-2">
                        <div className="w-6 h-1.5 rounded-full" style={{ backgroundColor: "rgba(255,255,255,0.15)" }} />
                        <div className="w-10 h-1.5 rounded-full" style={{ backgroundColor: "rgba(255,255,255,0.08)" }} />
                      </div>
                      <div className="flex gap-1 px-2">
                        <div className="flex-1 h-5 rounded" style={{ backgroundColor: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.08)" }} />
                        <div className="flex-1 h-5 rounded" style={{ backgroundColor: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.08)" }} />
                      </div>
                    </div>
                  }
                    ...
                    <button
                      ...
                      style={{
                        background: isSelected
                          ? accentTint(preset.hex, isDark ? 0.12 : 0.06)
                          : "transparent",
            ...
            {/* Interface */}
            <GlassCard title="Interface" icon={Layers} palette={palette} isDark={isDark} glass={glass} accent={selectedAccent}>
              <SettingToggle
                label="Compact mode"
...
function GlassCard({
  title, icon: Icon, palette, isDark, glass, accent, children,
}: {
  title: string; icon: typeof Sun; palette: PaletteType; isDark: boolean;
  glass: typeof GLASS.light; accent: string; children: React.ReactNode;
}) {
  return (
    <div
...
function ThemeCard({
  label, icon: Icon, isActive, palette, isDark, accent, onClick, preview,
}: {
  label: string; icon: typeof Sun; isActive: boolean; palette: PaletteType;
  isDark: boolean; accent: string; onClick: () => void; preview: React.ReactNode;
}) {
  return (
    <button
      onClick={onClick}
...
function SettingToggle({
  label, description, on, onToggle, palette, isDark, accent,
}: {
...
function PreviewPill({
  label, on, palette, accent, isDark,
}: {
  label: string; on: boolean; palette: PaletteType; accent: string; isDark: boolean;
}) {
  return (
    <div
    ...
    >
      ...
      <div
        ...
        style={{ backgroundColor: on ? accent : palette.textTertiary }}
      />
    </div>
  );
}
