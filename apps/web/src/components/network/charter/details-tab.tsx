    ...
    <div className="p-5 space-y-6">
      ...
      <Section title="Contract" border={sectionBorder} isDark={isDark}>
        <div className="grid grid-cols-4 gap-4">
          ...
          <Field
            label="Type"
            value={CONTRACT_TYPE_LABELS[contract.contractType as ContractType]}
      ...
      <Section title="Aircraft" border={sectionBorder} isDark={isDark}>
        <div className="grid grid-cols-4 gap-4">
          ...
          <Field
            label="Pax capacity"
            value={contract.paxCapacity?.toString() ?? '\u2014'}
            mono
            labelColor={labelColor}
          />
        </div>
      ...
      <Section title="Commercial" border={sectionBorder} isDark={isDark}>
        <div className="grid grid-cols-4 gap-4">
          <Field
            label="Rate/sector"
            value={fmtMoney(contract.ratePerSector, contract.currency)}
            mono
            labelColor={labelColor}
          />
          <Field
            label="Rate/block hour"
            value={fmtMoney(contract.ratePerBlockHour, contract.currency)}
            mono
            labelColor={labelColor}
          />
        ...
        <div className="grid grid-cols-4 gap-4 mt-4">
          <Field
            label="Catering"
            value={contract.catering === 'operator' ? 'Operator' : contract.catering === 'client' ? 'Client' : 'None'}
            labelColor={labelColor}
          />
...
function Field({
  ...
  return (
    <div>
      <div className="text-[13px] font-medium uppercase tracking-wide mb-1" style={{ color: labelColor }}>
        {label}
      </div>
