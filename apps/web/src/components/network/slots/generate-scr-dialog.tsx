...
export function GenerateSCRDialog({
  open, onOpenChange, series, airportIata, seasonCode, onGenerated, isDark,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.5)' }}>
      <div className="w-[760px] max-h-[85vh] rounded-2xl overflow-hidden flex flex-col"
        ...
        <div className="flex items-center justify-between px-5 py-4 shrink-0"
          ...
          <div className="flex items-center gap-3">
            <h2 className="text-[16px] font-semibold" style={{ color: palette.text }}>Generate SCR Message</h2>
            {/* Step indicator */}
            <div className="flex gap-1">
              {[1, 2, 3].map(s => (
                <div key={s} className="w-6 h-6 rounded-full text-[13px] font-semibold flex items-center justify-center"
                  style={{
                    background: step === s ? MODULE_THEMES.network.accent : isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)',
          ...
          {step === 1 && (
            <div className="space-y-1">
              ...
              {series.map(s => {
                ...
                return (
                  <div key={s._id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors"
                    style={{ background: isSelected ? (isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.02)') : 'transparent' }}>
                    <input type="checkbox" checked={isSelected} onChange={() => toggleSeries(s._id)}
                    ...
                    {isSelected && (
                      <select value={actionCode}
                        ...
                        {AIRLINE_ACTIONS.map(a => (
                          <option key={a} value={a}>{a} – {ACTION_CODE_LABELS[a]}</option>
                        ))}
                      </select>
                    )}
                  </div>
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea value={giText} onChange={e => setGiText(e.target.value)} rows={3}
          ...
          {step === 3 && (
            <div>
              <div className="flex items-center justify-between mb-3">
                <span className="text-[13px] font-medium" style={{ color: palette.text }}>Generated SCR</span>
