...
export function ColumnFilterDropdown({
  colKey, rows, activeFilters, activeColorFilter, onApply, onApplyColor, onClose,
    ...
    >
        ...
        <div className="flex border-b border-hz-border" style={{ fontFamily: "Inter, system-ui, sans-serif" }}>
          <button
            ...
            className={`flex-1 py-1.5 text-[12px] font-medium transition-colors ${
              tab === "values" ? "text-module-accent border-b-2 border-module-accent" : "text-hz-text-secondary hover:text-hz-text"
            }`}
          >
            Values
          </button>
          <button
            ...
            className={`flex-1 py-1.5 text-[12px] font-medium transition-colors flex items-center justify-center gap-1 ${
              tab === "color" ? "text-module-accent border-b-2 border-module-accent" : "text-hz-text-secondary hover:text-hz-text"
            }`}
          >
            <Paintbrush size={11} />
            Color
      ...
      {tab === "values" && (
        <>
          ...
          <div className="px-2 py-1 border-b border-hz-border/50">
            <label className="flex items-center gap-2 cursor-pointer text-[13px] text-hz-text-secondary hover:text-hz-text" style={{ fontFamily: "Inter, system-ui, sans-serif" }}>
              <input type="checkbox" checked={allSelected} onChange={toggleAll} className="accent-module-accent w-3.5 h-3.5" />
              <span className="font-medium">Select All</span> <span className="text-hz-text-tertiary">({filtered.length})</span>
            </label>
          </div>
          ...
          <div className="max-h-48 overflow-y-auto p-1">
            {filtered.map((v) => (
              ...
              >
                <input type="checkbox" checked={selected.has(v)} onChange={() => toggle(v)} className="accent-module-accent w-3.5 h-3.5" />
                <span className="truncate" style={{ fontFamily: valueFont }}>{displayValue(v)}</span>
              </label>
            ))}
            {filtered.length === 0 && (
              <p className="text-[13px] text-hz-text-tertiary px-2 py-2">No values</p>
            )}
          </div>

          {/* Actions */}
          <div className="flex items-center gap-1 p-2 border-t border-hz-border" style={{ fontFamily: "Inter, system-ui, sans-serif" }}>
            <button
      ...
      {tab === "color" && (
        <div className="p-2 space-y-3" style={{ fontFamily: "Inter, system-ui, sans-serif" }}>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {bgColors.map((color) => (
                  <button
                    key={color}
                    onClick={() => { onApplyColor?.(colKey, { type: "bg", color }); onClose(); }}
                    ...
                    style={{
                      ...
                      outline: activeColorFilter?.type === "bg" && activeColorFilter.color === color ? "2px solid var(--color-module-accent)" : "none",
                      outlineOffset: 1,
                    }}
                    title={color}
                  />
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {textColors.map((color) => (
                  <button
                    key={color}
                    onClick={() => { onApplyColor?.(colKey, { type: "text", color }); onClose(); }}
                    className="rounded-md transition-transform hover:scale-110"
                    style={{
                      width: 28,
                      height: 28,
                      backgroundColor: color,
                      border: `2px solid ${borderColor}`,
                      outline: activeColorFilter?.type === "text" && activeColorFilter.color === color ? "2px solid var(--color-module-accent)" : "none",
                      outlineOffset: 1,
                    }}
                    title={color}
                  />
          ...
          {activeColorFilter && (
            <button
              onClick={() => { onApplyColor?.(colKey, null); onClose(); }}
