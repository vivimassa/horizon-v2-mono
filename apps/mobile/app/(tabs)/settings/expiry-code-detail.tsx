...
export default function ExpiryCodeDetailScreen() {
  ...
  const handleDelete = useCallback(() => {
    ...
    Alert.alert('Delete Expiry Code', `Delete ${code.code} — ${code.name}?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            const u = await api.updateExpiryCode(code._id, { isActive: false })
            setCode(u)
          } catch (err: any) {
      ...
      },
      {
        ...
        onPress: async () => {
          try {
            await api.deleteExpiryCode(code._id)
            router.back()
          } catch (err: any) {
  ...
  const catColor = useMemo(
    () => categories.find((c) => c._id === code?.categoryId)?.color ?? accent,
    [categories, code, accent],
  )
    ...
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={['top']}>
        ...
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center flex-1 mr-2">
            ...
            <View className="px-2 py-0.5 rounded mr-2" style={{ backgroundColor: `${catColor}20` }}>
              <Text style={{ fontSize: 15, fontWeight: '700', fontFamily: 'monospace', color: catColor }}>
                {code.code}
              </Text>
            </View>
      ...
      >
        ...
        <View className={isTablet ? 'flex-row flex-wrap' : ''}>
          ...
          <Field
            label="Warning Days"
            value={code.warningDays}
            editing={editing}
            fieldKey="warningDays"
            editValue={get('warningDays')}
            onChange={handleFieldChange}
            palette={palette}
            numeric
            half={isTablet}
        ...
        {editing && (
          <PickerField
            label="Category"
            options={categories.map((c) => ({ key: c._id, label: c.label, color: c.color }))}
            editValue={get('categoryId')}
            fieldKey="categoryId"
            onChange={handleFieldChange}
            palette={palette}
            isDark={isDark}
        ...
        <PickerField
          label="Crew Category"
          options={CREW_CATS.map((c) => ({ key: c.key, label: c.label }))}
          editValue={editing ? get('crewCategory') : code.crewCategory}
          fieldKey="crewCategory"
          onChange={handleFieldChange}
          palette={palette}
          isDark={isDark}
          accent={accent}
        ...
        {editing ? (
          <PickerField
            label="Formula"
            options={EXPIRY_FORMULAS.map((f) => ({ key: f.id, label: f.label }))}
            editValue={get('formula')}
            fieldKey="formula"
            onChange={handleFieldChange}
            palette={palette}
            isDark={isDark}
          ...
          <Field
            label="Formula"
            value={formulaDef?.label ?? code.formula}
            editing={false}
            fieldKey=""
            editValue=""
            onChange={() => {}}
          ...
          <View className={isTablet ? 'flex-row flex-wrap' : ''}>
            {formulaDef.fields.map((f) => (
              <Field
                ...
                onChange={(k, v) => {
                  const params = {
                    ...(get('formulaParams') ?? code.formulaParams ?? {}),
                    [f.key]: f.type === 'number' ? (v === '' ? null : Number(v)) : v,
                  }
        ...
        <View className="flex-row flex-wrap" style={{ gap: 6, marginBottom: 8 }}>
          ...
          {filteredPositions.length === 0 && (
            <Text style={{ fontSize: 13, color: palette.textTertiary }}>
              No positions available for this crew category
            </Text>
          )}
        </View>
