...
import {
  Search, ChevronLeft, ChevronDown, Users, Plus, RefreshCw, Sparkles, Pencil,
    ...
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={[]}>
      <View className="px-4 pt-2 pb-3" style={{ borderBottomWidth: 1, borderBottomColor: palette.border }}>
        <View className="flex-row items-center mb-3">
          ...
          <View className="flex-1">
            <Text style={{ fontSize: 20, fontWeight: '700', color: palette.text }}>Crew Groups</Text>
          ...
          <Pressable onPress={() => router.push('/(tabs)/settings/crew-group-add' as any)}
        ...
        <View className="flex-row items-center rounded-xl" style={{
          ...
          <TextInput className="flex-1 py-2.5 ml-2" style={{ fontSize: 15, color: palette.text }}
            placeholder="Search name, description..." placeholderTextColor={palette.textTertiary}
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <GroupAccordion group={item} palette={palette} accent={accent} isDark={isDark}
              isExpanded={expanded.has(item._id)}
              onToggle={() => setExpanded(prev => { const n = new Set(prev); n.has(item._id) ? n.delete(item._id) : n.add(item._id); return n })}
              onEdit={() => router.push({ pathname: '/(tabs)/settings/crew-group-detail' as any, params: { id: item._id } })} />
          )}
        />
      )}
    </SafeAreaView>
...
const GroupAccordion = memo(function GroupAccordion({
  group, palette, accent, isDark, isExpanded, onToggle, onEdit,
}: {
  group: CrewGroupRef; palette: Palette; accent: string; isDark: boolean;
  ...
  return (
    <View style={{ borderBottomWidth: 1, borderBottomColor: palette.border, opacity: group.isActive ? 1 : 0.4 }}>
      ...
      <Pressable onPress={onToggle} className="flex-row items-center active:opacity-70"
        ...
        <View className="flex-1">
          <Text style={{ fontSize: 15, fontWeight: '500', color: palette.text }} numberOfLines={1}>{group.name}</Text>
        </View>
        <View className="flex-row items-center" style={{ gap: 6 }}>
          {!group.isActive && (
            <View className="px-1.5 py-0.5 rounded" style={{ backgroundColor: isDark ? 'rgba(220,38,38,0.15)' : '#fee2e2' }}>
      ...
      {isExpanded && (
        <View style={{ paddingHorizontal: 16, paddingBottom: 12 }}>
          <View className="rounded-lg p-3" style={{
            ...
            {group.description ? (
              ...
              <Text style={{ fontSize: 14, color: palette.textTertiary, fontStyle: 'italic', marginBottom: 8 }}>No description</Text>
            )}
            <View className="flex-row items-center justify-between">
              <View className="flex-row items-center" style={{ gap: 8 }}>
                <Text style={{ fontSize: 13, color: palette.textTertiary }}>Order: <Text style={{ fontWeight: '600', fontFamily: 'monospace', color: palette.text }}>{group.sortOrder}</Text></Text>
                {group.isActive ? (
                  <View className="px-1.5 py-0.5 rounded" style={{ backgroundColor: isDark ? 'rgba(22,163,74,0.15)' : '#dcfce7' }}>
              ...
              <Pressable onPress={onEdit} className="flex-row items-center px-3 py-1.5 rounded-lg active:opacity-70"
