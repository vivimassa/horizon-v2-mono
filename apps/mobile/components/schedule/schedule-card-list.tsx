...
export const ScheduleCardList = memo(function ScheduleCardList({
  flights, dirtyMap, newIds, deletedIds,
  onPress, palette, accent, isDark,
}: {
  ...
  return (
    <FlatList data={flights} keyExtractor={f => f._id}
        ...
        return (
          <Pressable onPress={() => onPress(flight._id)} className="active:opacity-70"
            ...
            <View className="flex-row items-center mb-1">
              <Text style={{ fontSize: 17, fontWeight: '700', fontFamily: 'monospace', color: accent, marginRight: 8 }}>
                {flight.airlineCode}{flight.flightNumber}
              </Text>
              {flight.aircraftTypeIcao && (
                <View className="px-1.5 py-0.5 rounded mr-2" style={{ backgroundColor: accentTint(accent, isDark ? 0.15 : 0.08) }}>
                  <Text style={{ fontSize: 12, fontWeight: '600', fontFamily: 'monospace', color: accent }}>{flight.aircraftTypeIcao}</Text>
                </View>
              )}
              <View className="flex-1" />
              <View className="px-2 py-0.5 rounded-full" style={{ backgroundColor: `${statusColor}20` }}>
                <Text style={{ fontSize: 11, fontWeight: '600', color: statusColor, textTransform: 'uppercase' }}>{flight.status}</Text>
              </View>
            </View>
            ...
            <View className="flex-row items-center mb-1">
              ...
              {flight.blockMinutes != null && flight.blockMinutes > 0 && (
                <Text style={{ fontSize: 12, color: palette.textTertiary, marginLeft: 6 }}>({fmtMinutes(flight.blockMinutes)})</Text>
              )}
            </View>
