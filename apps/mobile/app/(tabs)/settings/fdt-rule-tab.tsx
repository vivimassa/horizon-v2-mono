...
function formatRowLabel(label: string): string {
  return label.replace(
    /(\d{4})-(\d{4})/,
    (_, a, b) => `${a.slice(0, 2)}:${a.slice(2)}\u2013${b.slice(0, 2)}:${b.slice(2)}`,
  )
}

function isLessRestrictive(rule: FdtlRuleRef): boolean {
  if (rule.isTemplateDefault || !rule.templateValue) return false
  const cur = parseFloat(rule.value),
    tpl = parseFloat(rule.templateValue)
...
export default function FdtRuleTabScreen() {
  ...
  const handleCellSave = useCallback(
    async (tableId: string, rowKey: string, colKey: string) => {
  ...
  const handleResetTable = useCallback(async (tableId: string) => {
    try {
      ...
      Alert.alert('Error', 'Failed to reset table')
    }
  }, [])
  ...
  const handleRuleReset = useCallback(async (id: string) => {
    try {
      ...
      Alert.alert('Error', 'Failed to reset rule')
    }
  }, [])
          ...
          return (
            <View key={table._id} className="mb-6">
              <View className="flex-row items-center justify-between mb-2">
                ...
                {hasModified && (
                  <Pressable
                    onPress={() => handleResetTable(table._id)}
                    className="flex-row items-center px-2 py-1 rounded active:opacity-60"
                    style={{ gap: 4 }}
                  >
                ...
                >
                  ...
                  {table.rowKeys.map((rk, ri) => (
                    <View
                      ...
                      style={{
                        backgroundColor:
                          ri % 2 === 0 ? 'transparent' : isDark ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.015)',
                      }}
                    >
                      <View
                        style={{
                        ...
                        return (
                          <Pressable
                            ...
                            onPress={() => {
                              ...
                              setCellDraft(
                                cell?.valueMinutes != null && cell.valueMinutes >= 0
                                  ? minutesToHHMM(cell.valueMinutes)
                                  : '',
                              )
                            }}
                            style={{
                            ...
                            {isEditing ? (
                              <TextInput
                                ...
                                style={{
                                  ...
                                  borderRadius: 4,
                                  borderWidth: 1,
                                  borderColor: accentTint(accent, 0.4),
                                  backgroundColor: palette.card,
                                }}
                              />
                            ) : isProhibited ? (
                              <Text style={{ fontSize: 13, fontWeight: '700', color: isDark ? '#f87171' : '#dc2626' }}>
                                {'\u2014'}
                              </Text>
                            ) : isNA ? (
                              ...
                              <View className="flex-row items-center" style={{ gap: 2 }}>
                                ...
                                <Text
                                  style={{
                                    ...
                                    fontFamily: 'monospace',
                                    color: isModified ? accent : palette.text,
                                  }}
                                >
              ...
              <Pressable
                onPress={() =>
                  setCollapsed((prev) => {
                    ...
                    n.has(subcategory) ? n.delete(subcategory) : n.add(subcategory)
                    return n
                  })
                }
                  ...
                  return (
                    ...
                    >
                      <View className="flex-row items-center">
                        {/* Source badge */}
                        <View
                          className="px-1.5 rounded mr-2"
                          style={{
                            backgroundColor:
                              rule.source === 'company'
                                ? accentTint(accent, isDark ? 0.15 : 0.1)
                                : isDark
                                  ? 'rgba(253,172,66,0.15)'
                                  : 'rgba(255,136,0,0.12)',
                          }}
                        >
                          <Text
                        ...
                        <View className="flex-1 mr-2">
                          <View className="flex-row items-center flex-wrap" style={{ gap: 4 }}>
                            ...
                            {lessRestr && (
                              <AlertTriangle size={12} color={isDark ? '#fbbf24' : '#d97706'} strokeWidth={2} />
                            )}
                          </View>
                          {rule.legalReference && (
                            <Text style={{ fontSize: 11, color: palette.textTertiary, marginTop: 1 }}>
                              {rule.legalReference}
                            </Text>
                          )}
                        </View>
                        ...
                        {isBool ? (
                          <Pressable
                            onPress={() => handleRuleToggle(rule)}
                            className="px-3 py-1.5 rounded-lg"
                            style={{
                              backgroundColor: boolVal
                                ? isDark
                                  ? 'rgba(22,163,74,0.15)'
                                  : '#dcfce7'
                                : isDark
                          ...
                          >
                            <Text
                              style={{
                                fontSize: 13,
                                fontWeight: '600',
                                color: boolVal ? (isDark ? '#4ade80' : '#16a34a') : palette.textTertiary,
                              }}
                            >
                              {boolVal ? 'Yes' : 'No'}
                            </Text>
                          ...
                          <TextInput
                            ...
                            style={{
                              ...
                              width: 70,
                              borderBottomWidth: 1,
                              borderBottomColor: accentTint(accent, 0.4),
                              paddingVertical: 2,
                            }}
                          />
                        ) : (
