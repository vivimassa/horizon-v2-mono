    ...
    <div className="p-5 space-y-4">
      ...
      <div className="rounded-xl p-4" style={{ background: cardBg, border: `1px solid ${cardBorder}` }}>
        ...
        <div className="space-y-2">
          <Row
            label="Revenue flights"
            value={`${stats.revenueFlights} sectors`}
            color={isDark ? '#39D98A' : '#06C270'}
          />
          <Row
            label="Positioning legs"
            value={`${stats.positioningFlights} sectors`}
      ...
      <div className="rounded-xl p-4" style={{ background: cardBg, border: `1px solid ${cardBorder}` }}>
        ...
        {contract.ratePerSector ? (
          ...
          <div className="text-[13px] text-hz-text-secondary mb-2">
            {(stats.totalBlockMinutes / 60).toFixed(1)} BH &times;{' '}
            {fmtMoney(contract.ratePerBlockHour, contract.currency)}/BH
          </div>
        ) : (
...
function Row({ label, value, color, mono }: { label: string; value: string; color?: string; mono?: boolean }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <span className={`text-[13px] font-semibold ${mono ? 'font-mono' : ''}`} style={color ? { color } : undefined}>
        {value}
      </span>
    </div>
  )
