...
const ROUTE_TYPE_OPTIONS = ['domestic', 'regional', 'international', 'long-haul', 'ultra-long-haul', 'unknown']
...
const ROUTE_BADGE: Record<string, { bg: string; text: string; darkBg: string; darkText: string }> = {
  ...
  'ultra-long-haul': {
    bg: 'rgba(102,0,204,0.10)',
    text: '#6600CC',
    darkBg: 'rgba(172,93,217,0.15)',
    darkText: '#AC5DD9',
  },
...
function Alert({
  ...
  return (
    ...
    >
      ...
      <span className="text-[13px] flex-1" style={{ color: config.text }}>
        {message}
      </span>
      {onDismiss && (
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-[13px] text-hz-text-secondary mt-1">
              This will permanently remove the city pair and all associated block hour data. This action cannot be
              undone.
            </p>
          </div>
        </div>
...
export function CityPairDetail({ cityPair, onSave, onDelete, onCreate, onRefresh }: CityPairDetailProps) {
  ...
  const handleCreate = useCallback(async () => {
    if (!onCreate) return
    if (!createForm.station1.trim() || !createForm.station2.trim()) {
      setCreateError('Both ICAO codes are required')
      return
    }
    setCreating(true)
    setCreateError('')
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaving(true)
    try {
      await onSave(cityPair._id, draft)
      setEditing(false)
      setDraft({})
    } catch {
  ...
  const handleDelete = useCallback(async () => {
    ...
    setErrorMsg('')
    try {
      await onDelete(cityPair._id)
      setShowDeleteModal(false)
    } catch (err: any) {
      const msg = err.message || 'Delete failed'
      const match = msg.match(/API \d+: (.+)/)
      try {
        setErrorMsg(JSON.parse(match?.[1] ?? '{}').error || msg)
      } catch {
  ...
  const hasCoords =
    cityPair.station1Lat != null &&
    cityPair.station1Lon != null &&
    cityPair.station2Lat != null &&
    cityPair.station2Lon != null
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-hz-border shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span
              className="text-[13px] font-semibold px-2.5 py-0.5 rounded-full capitalize dark:hidden"
              style={{ backgroundColor: badge.bg, color: badge.text }}
            >
              {cityPair.routeType}
            </span>
            <span
              className="text-[13px] font-semibold px-2.5 py-0.5 rounded-full capitalize hidden dark:inline"
      ...
      {hasCoords && (
        <div className="shrink-0 border-b border-hz-border relative" style={{ height: mapHeight }}>
          <CityPairMap
            lat1={cityPair.station1Lat!}
            lon1={cityPair.station1Lon!}
            lat2={cityPair.station2Lat!}
            lon2={cityPair.station2Lon!}
            label1={label1}
            label2={label2}
            distanceNm={cityPair.distanceNm}
            distanceKm={cityPair.distanceKm}
          />
          <div
        ...
        {activeTab === 'general' && (
          <div className="px-6 pt-3 pb-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8">
              ...
              <FieldRow
                label="Standard Block (min)"
                value={cityPair.standardBlockMinutes}
                editing={editing}
                fieldKey="standardBlockMinutes"
                editValue={getVal('standardBlockMinutes')}
              ...
              <FieldRow
                label="Overwater"
                value={
                  cityPair.isOverwater ? (
                    <span className="font-semibold" style={{ color: '#0063F7' }}>
                      Yes
                    </span>
                  ) : (
                    <span className="text-hz-text-secondary">No</span>
                ...
                editing={editing}
                fieldKey="isOverwater"
                editValue={getVal('isOverwater')}
                onChange={handleFieldChange}
                inputType="toggle"
              />
            </div>
...
function BlockHoursTab({
  ...
  const handleAdd = useCallback(async () => {
    if (!addForm.dir1Block || !addForm.dir2Block) {
      setError('Block minutes for both directions are required')
      return
    }
    setSaving(true)
    setError('')
    try {
      await api.addBlockHour(cityPair._id, formToPayload(addForm))
      setShowAdd(false)
  ...
  const handleEditSave = useCallback(async () => {
    ...
    setError('')
    try {
      await api.updateBlockHour(cityPair._id, editId, formToPayload(editForm))
      setEditId(null)
      onRefresh?.()
    } catch (err: any) {
      setError(err.message || 'Failed')
  ...
  const handleDelete = useCallback(
    async (bhId: string) => {
      setSaving(true)
      try {
        await api.deleteBlockHour(cityPair._id, bhId)
        ...
        <div className="p-4 rounded-xl border border-hz-border bg-hz-card space-y-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-3 py-2 rounded-lg text-[13px] font-medium text-hz-text-secondary border border-hz-border hover:bg-hz-border/30 transition-colors"
            >
              Cancel
            </button>
          </div>
      ...
      {blockHours.length === 0 && !showAdd ? (
        <div className="py-12 flex flex-col items-center justify-center gap-3">
          ...
          <p className="text-[14px] font-medium text-hz-text-secondary">
            No {mode === 'block' ? 'block hours' : 'fuel data'} configured
          </p>
          <p className="text-[13px] text-hz-text-tertiary max-w-sm text-center">
            Add entries for specific aircraft types and seasons to track{' '}
            {mode === 'block' ? 'block and flight times' : 'fuel consumption'} per direction.
          </p>
          <button
            onClick={() => {
              setShowAdd(true)
              ...
              <table className="w-full text-[13px]">
                <thead>
                  <tr className="bg-hz-bg">
                    <th className="px-4 py-2.5 text-left text-[12px] font-medium uppercase tracking-wider text-hz-text-tertiary">
                      Type
                    </th>
                    <th className="px-4 py-2.5 text-left text-[12px] font-medium uppercase tracking-wider text-hz-text-tertiary">
                      Season
                ...
                <tbody className="divide-y divide-hz-border/50">
                  {blockHours.map((bh, i) => (
                    <tr
                      key={bh._id}
                      className={`group transition-colors hover:bg-hz-border/20 ${editId === bh._id ? 'bg-module-accent/[0.04]' : i % 2 === 1 ? 'bg-hz-border/[0.04]' : ''}`}
                    >
                      ...
                      {mode === 'block' ? (
                        <>
                          <td className="px-4 py-2.5 font-medium font-mono tabular-nums">
                            {fmtBlock(bh.dir1BlockMinutes)}
                          </td>
                          <td className="px-4 py-2.5 font-medium font-mono tabular-nums">
                            {fmtBlock(bh.dir2BlockMinutes)}
                          </td>
                        </>
                      ) : (
                        <>
                          <td className="px-4 py-2.5 font-mono tabular-nums text-hz-text-secondary">
                            {bh.dir1FuelKg != null ? `${bh.dir1FuelKg.toLocaleString()} kg` : '—'}
                          </td>
                          <td className="px-4 py-2.5 font-mono tabular-nums text-hz-text-secondary">
                            {bh.dir2FuelKg != null ? `${bh.dir2FuelKg.toLocaleString()} kg` : '—'}
                          </td>
                        </>
                      )}
                          ...
                          {confirmDeleteId === bh._id ? (
                            <div className="flex items-center gap-0.5">
                              ...
                              <button
                                ...
                                className="px-2 py-1 rounded-lg text-[11px] text-hz-text-secondary hover:bg-hz-border/30"
                              >
                                No
                              </button>
                            </div>
                            ...
                            <button
                              onClick={() => setConfirmDeleteId(bh._id)}
                              className="p-1.5 rounded-lg text-hz-text-secondary/40 hover:text-red-500 transition-colors"
                            >
...
function BlockHourFormFields({
  ...
  const inputClass =
    'w-full mt-1 px-3 py-2 rounded-lg text-[13px] border border-hz-border bg-hz-bg outline-none focus:ring-2 focus:ring-module-accent/30 focus:border-module-accent text-hz-text transition-colors'
  return (
    <>
      <div className="flex gap-3">
        <div className="flex-1">
          ...
          <input
            ...
            placeholder="e.g. A320 (or blank for all)"
        ...
        <div className="w-32">
          ...
          <select
            ...
            onChange={(e) => setForm((p: any) => ({ ...p, seasonType: e.target.value }))}
            className={inputClass}
          >
      ...
      {mode === 'block' ? (
        <div className="flex gap-3">
          <div className="flex-1">
            <label className="text-[12px] text-hz-text-secondary uppercase tracking-wider font-medium">
              Block {label1}→{label2} (HH:MM) *
            </label>
            <input
              type="text"
              value={form.dir1Block}
          ...
          <div className="flex-1">
            <label className="text-[12px] text-hz-text-secondary uppercase tracking-wider font-medium">
              Block {label2}→{label1} (HH:MM) *
            </label>
            <input
              type="text"
              value={form.dir2Block}
              placeholder="e.g. 02:10"
