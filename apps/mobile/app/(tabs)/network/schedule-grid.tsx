...
export default function ScheduleGridScreen() {
  ...
  const activeFilterCount = [dateFrom, dateTo, depStations !== null ? 'x' : '', arrStations !== null ? 'x' : '', acTypeFilter, statusFilter]
    .filter(Boolean).length

  // Reference data for dropdowns
  const airportItems = useMemo(() => airports.map(a => ({ key: a.icaoCode, label: `${a.icaoCode} ${a.name}` })), [airports])
  const acTypeOptions = useMemo(() => [
    { value: '', label: 'All Types' },
    ...acTypes.map(t => ({ value: t, label: t })),
  ], [acTypes])
  ...
  const handleAddFlight = useCallback(() => {
    ...
    const newFlight: ScheduledFlightRef = {
      ...
      effectiveFrom: dateFrom, effectiveUntil: dateTo, cockpitCrewRequired: null, cabinCrewRequired: null,
    ...
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={[]}>
      ...
      {isTablet && (
        <View className="flex-1 flex-row" style={{ gap: 0, paddingBottom: Math.max(40, insets.bottom + 20) }}>
          ...
          <FilterPanel onApply={fetchData} applyDisabled={!dateFrom || !dateTo}
            loading={loading} activeCount={activeFilterCount}
            bottomInset={insets.bottom}
            accent={accent} palette={palette} isDark={isDark}>
            ...
            <FilterSection label="Departure" palette={palette}>
              <MultiSelect items={airportItems} value={depStations} onChange={setDepStations}
                allLabel="All Departures" accent={accent} palette={palette} isDark={isDark} />
            </FilterSection>
            <FilterSection label="Arrival" palette={palette}>
              <MultiSelect items={airportItems} value={arrStations} onChange={setArrStations}
                allLabel="All Arrivals" accent={accent} palette={palette} isDark={isDark} />
            </FilterSection>
            <FilterSection label="Aircraft Type" palette={palette}>
              <DropdownSelect options={acTypeOptions} value={acTypeFilter || null}
                onChange={setAcTypeFilter} placeholder="All Types"
                accent={accent} palette={palette} isDark={isDark} />
            </FilterSection>
            <FilterSection label="Status" palette={palette}>
              <DropdownSelect options={statusOptions} value={statusFilter || null}
      ...
      {!isTablet && showFilter && (
        <PhoneFilterModal
          ...
          dateFrom={dateFrom} dateTo={dateTo} onChangeFrom={setDateFrom} onChangeTo={setDateTo}
          acTypeFilter={acTypeFilter} onChangeAcType={setAcTypeFilter}
          statusFilter={statusFilter} onChangeStatus={setStatusFilter}
          acTypeOptions={acTypeOptions} statusOptions={statusOptions}
          onApply={() => { fetchData(); setShowFilter(false) }}
...
function PhoneFilterModal({
  visible, onClose, dateFrom, dateTo, onChangeFrom, onChangeTo,
  acTypeFilter, onChangeAcType, statusFilter, onChangeStatus,
  acTypeOptions, statusOptions, onApply, applyDisabled,
  accent, palette, isDark,
}: {
  visible: boolean; onClose: () => void
  dateFrom: string; dateTo: string; onChangeFrom: (v: string) => void; onChangeTo: (v: string) => void
  acTypeFilter: string; onChangeAcType: (v: string) => void
  statusFilter: string; onChangeStatus: (v: string) => void
  acTypeOptions: { value: string; label: string }[]
  statusOptions: { value: string; label: string; color?: string }[]
  onApply: () => void; applyDisabled: boolean
  accent: string; palette: any; isDark: boolean
}) {
  return (
    <Modal visible={visible} animationType="slide" transparent>
      <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.4)' }}>
        <View style={{ flex: 1, marginTop: 80, borderTopLeftRadius: 20, borderTopRightRadius: 20, backgroundColor: palette.background }}>
          {/* Header */}
          ...
          <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, gap: 16 }}
            ...
            <FilterSection label="Aircraft Type" palette={palette}>
              <DropdownSelect options={acTypeOptions} value={acTypeFilter || null}
                onChange={onChangeAcType} placeholder="All Types"
                accent={accent} palette={palette} isDark={isDark} />
            </FilterSection>
            <FilterSection label="Status" palette={palette}>
              <DropdownSelect options={statusOptions} value={statusFilter || null}
                onChange={onChangeStatus} placeholder="All Statuses"
                accent={accent} palette={palette} isDark={isDark} />
            </FilterSection>
          </ScrollView>
          ...
          <View style={{ padding: 16, paddingBottom: 32, borderTopWidth: 1, borderTopColor: palette.border }}>
            ...
            <Pressable onPress={onApply} disabled={applyDisabled}
