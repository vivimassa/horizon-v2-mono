    ...
    return (
      <div className="flex flex-col h-full overflow-hidden">
        <div className="px-6 py-4 border-b border-hz-border shrink-0">
          <h1 className="text-[18px] font-semibold">Add New Cabin Class</h1>
        </div>
        <div className="px-6 py-4 space-y-3 overflow-y-auto">
          {/* Preset selector */}
          <div>
            <label className="text-[12px] text-hz-text-secondary uppercase tracking-wider font-semibold">Start from Preset</label>
            <div className="flex flex-wrap gap-2 mt-1.5">
              {SEAT_PRESETS.map((preset) => (
                <button key={preset.label}
                  ...
                  className="px-3 py-1.5 rounded-lg text-[12px] font-medium border border-hz-border hover:border-[#1e40af]/40 hover:bg-[#1e40af]/5 transition-colors"
                  style={createForm.seatLayout === preset.seatLayout && createForm.seatType === preset.seatType
                    ? { borderColor: "#1e40af", backgroundColor: "rgba(30,64,175,0.08)", color: "#1e40af", fontWeight: 600 }
                    : undefined}
                >
                  {preset.label}
                </button>
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <input type="text" value={createForm.color}
                  onChange={(e) => setCreateForm(p => ({ ...p, color: e.target.value }))}
                  className="flex-1 px-3 py-2.5 rounded-lg text-[13px] font-mono border border-hz-border bg-hz-bg outline-none focus:ring-2 focus:ring-module-accent/30 text-hz-text"
                  maxLength={7} />
              </div>
            </div>
          ...
          <div className="flex gap-3">
            <div className="flex-1">
              <label className="text-[12px] text-hz-text-secondary uppercase tracking-wider font-semibold">Seat Type</label>
      ...
      <div className="px-6 py-4 border-b border-hz-border shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-[18px] font-semibold">{cabinClass.name}</h1>
            {cabinClass.isActive ? (
              <span className="text-[13px] font-semibold px-3 py-0.5 rounded-full bg-green-50 text-green-700 dark:bg-green-500/15 dark:text-green-400">Active</span>
            ) : (
              <span className="text-[13px] font-semibold px-3 py-0.5 rounded-full bg-red-50 text-red-600">Inactive</span>
            )}
          </div>
              ...
              <>
                {onDelete && (
                  confirmDelete ? (
                    ...
                    <button onClick={() => setConfirmDelete(true)} className="flex items-center gap-1 p-1.5 rounded-lg text-hz-text-secondary/50 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors" title="Delete cabin class">
            ...
            <div className="flex flex-wrap gap-1.5">
              {SEAT_PRESETS.map((preset) => (
                <button key={preset.label}
                  ...
                  className="px-2.5 py-1 rounded-lg text-[11px] font-medium border border-hz-border hover:border-[#1e40af]/40 transition-colors"
                  style={createForm.seatLayout === preset.seatLayout && createForm.seatType === preset.seatType
                    ? { borderColor: "#1e40af", backgroundColor: "rgba(30,64,175,0.08)", color: "#1e40af", fontWeight: 600 }
                    : undefined}
                >
        ...
        {activeTab === "specs" && (
          <div className="px-6 pt-3 pb-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8">
              <FieldRow label="Code" value={<span className="font-bold font-mono">{cabinClass.code}</span>}
              ...
              <FieldRow label="Seat Layout" value={cabinClass.seatLayout ? <span className="font-mono font-bold">{cabinClass.seatLayout}</span> : null}
              ...
              <div className="py-2.5 border-b border-hz-border/50">
                <div className="text-[12px] text-hz-text-secondary uppercase tracking-wider font-semibold mb-1">Seat Type</div>
                {editing ? (
                  <select
              ...
              <FieldRow label="IFE Screen"
                value={cabinClass.hasIfe ? <span className="text-green-600 font-semibold">Yes</span> : <span className="text-hz-text-secondary">No</span>}
              ...
              <FieldRow label="Power Outlet"
                value={cabinClass.hasPower ? <span className="text-green-600 font-semibold">Yes</span> : <span className="text-hz-text-secondary">No</span>}
                editing={editing} fieldKey="hasPower" editValue={getVal("hasPower")} onChange={handleFieldChange} inputType="toggle" />
              <FieldRow label="Active"
                value={cabinClass.isActive ? <span className="text-green-600 font-semibold">Active</span> : <span className="text-red-600 font-semibold">Inactive</span>}
            ...
            {usage.length === 0 ? (
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between pt-2 mt-2 border-t border-hz-border/50">
                  <span className="text-[12px] text-hz-text-secondary font-medium">
...
function MetricCard({ icon: Icon, label, value, color }: {
  ...
  return (
    <div className="flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl border border-hz-border/50 shrink-0">
      ...
      <div>
        <div className="text-[11px] text-hz-text-tertiary uppercase tracking-wider font-semibold leading-none">{label}</div>
