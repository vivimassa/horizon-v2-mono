...
export default function CrewPositionDetailScreen() {
  ...
  const handleSave = useCallback(async () => {
    if (!position || Object.keys(draft).length === 0) { setEditing(false); return }
    setSaving(true)
    try {
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      if (total > 0) {
        ...
        if (refs.crewComplements > 0) parts.push(`${refs.crewComplements} crew complement${refs.crewComplements !== 1 ? 's' : ''}`)
        ...
        Alert.alert(
          'Delete Position',
          `Delete ${position.code} — ${position.name}?`,
          [
            { text: 'Cancel', style: 'cancel' },
            {
              ...
              onPress: async () => {
                try {
                  ...
                  try { const m = msg.match(/API (\d+): (.+)/); if (m) msg = JSON.parse(m[2]).error || msg } catch {}
    ...
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={['top']}>
      ...
      <View className="px-4 pt-4 pb-4" style={{ borderBottomWidth: 1, borderBottomColor: palette.border }}>
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center flex-1 mr-2">
            ...
            <Text style={{ fontSize: 24, fontWeight: '700', fontFamily: 'monospace', color: palette.text, marginRight: 8 }}>
              {position.code}
            </Text>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 100 }} keyboardShouldPersistTaps="handled">
        ...
        <View className={isTablet ? 'flex-row flex-wrap' : ''}>
          <Field label="Code" value={position.code} editing={editing} fieldKey="code" editValue={get('code')}
            onChange={handleFieldChange} palette={palette} mono maxLength={3} half={isTablet} />
          <Field label="Rank Order" value={position.rankOrder} editing={editing} fieldKey="rankOrder" editValue={get('rankOrder')}
            onChange={handleFieldChange} palette={palette} numeric half={isTablet} />
          <Field label="Name" value={position.name} editing={editing} fieldKey="name" editValue={get('name')}
            onChange={handleFieldChange} palette={palette} half={isTablet} />
          <PickerField label="Category" value={catLabel} options={CATEGORIES}
            editing={editing} fieldKey="category" editValue={get('category')}
            onChange={handleFieldChange} palette={palette} isDark={isDark} accent={accent} half={isTablet} />
        </View>
        ...
        <View className={isTablet ? 'flex-row flex-wrap' : ''}>
          {(get('category') === 'cockpit') && (
            <ToggleField label="Pilot in Command (PIC)" value={position.isPic} editing={editing}
              fieldKey="isPic" editValue={get('isPic')} onChange={handleFieldChange} palette={palette} isDark={isDark} half={isTablet} />
          )}
          <ToggleField label="Can Downrank" value={position.canDownrank} editing={editing}
            fieldKey="canDownrank" editValue={get('canDownrank')} onChange={handleFieldChange} palette={palette} isDark={isDark} half={isTablet} />
          <ToggleField label="Active" value={position.isActive} editing={editing}
            fieldKey="isActive" editValue={get('isActive')} onChange={handleFieldChange} palette={palette} isDark={isDark} half={isTablet} />
        </View>
        ...
        <View style={{ paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: palette.border }}>
          ...
          <ColorSwatchPicker value={get('color') || accent} onChange={(v) => handleFieldChange('color', v)}
            palette={palette} isDark={isDark} editing={editing} />
        </View>
        <Field label="Description" value={position.description} editing={editing} fieldKey="description"
      ...
      <View style={{ paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: palette.border, ...halfStyle }}>
        ...
        <TextInput
          ...
          style={{
            ...
            borderBottomWidth: 1, borderBottomColor: accentTint(palette.text, 0.15),
            paddingVertical: 4,
...
function PickerField({ label, value, options, editing, fieldKey, editValue, onChange, palette, isDark, accent, half }: {
  label: string; value: string; options: Array<{ key: string; label: string }>; editing: boolean; fieldKey: string; editValue: any;
      ...
      <View style={{ paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: palette.border, ...halfStyle }}>
        ...
        <View className="flex-row" style={{ gap: 8 }}>
          {options.map(opt => {
            ...
            return (
              <Pressable key={opt.key} onPress={() => onChange(fieldKey, opt.key)}
                ...
                <Text style={{ fontSize: 13, fontWeight: active ? '600' : '400', color: active ? accent : palette.text }}>{opt.label}</Text>
              </Pressable>
            )
          })}
        </View>
