...
interface AircraftDetail {
  ...
  lopa: { configName: string; totalSeats: number; cabins: { classCode: string; seats: number; color: string | null; sortOrder: number }[] } | null
}

// Fallback colors if DB color is null
const FALLBACK_COLORS: Record<string, string> = {
  F: '#8B5CF6', J: '#3B82F6', C: '#3B82F6', W: '#14B8A6', Y: '#06C270',
}
...
export function AircraftPopover() {
  ...
  useEffect(() => {
    if (!pop) { setDetail(null); return }
    const operatorId = useOperatorStore.getState().operator?._id ?? ''
    fetch(`${getApiBaseUrl()}/gantt/aircraft-detail?operatorId=${operatorId}&registration=${encodeURIComponent(pop.registration)}`)
  ...
  const stats = useMemo(() => {
    if (!pop) return null
    const acFlights = flights.filter(f =>
      f.aircraftReg === pop.registration ||
      (!f.aircraftReg && f.aircraftTypeIcao === pop.aircraftTypeIcao)
  ...
  const content = (
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 pt-3 pb-2">
        <div>
          ...
          <span className="text-[13px] ml-2" style={{ color: textSec }}>
            {detail?.aircraftTypeName ?? icao}{lopa ? ` (${lopa.totalSeats} seats)` : ''}
          </span>
        </div>
      ...
      {fuselageSrc && (
        <div className="relative mx-4 mb-1 overflow-hidden rounded-lg flex items-center justify-center" style={{ height: 80 }}>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={fuselageSrc} alt="" className="select-none"
            style={{
              height: '160%',
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      background: color,
                      opacity: 0.6,
                      borderRight: i < lopa.cabins.length - 1 ? `1px solid ${isDark ? 'rgba(0,0,0,0.3)' : 'rgba(255,255,255,0.4)'}` : 'none',
                    }}
                  >
                    <span className="text-[11px] font-mono font-bold text-white" style={{ opacity: 1, textShadow: '0 1px 3px rgba(0,0,0,0.7)' }}>
                      {c.classCode}
                    </span>
                  </div>
                )
      ...
      <div className="mx-4 mb-3 rounded-lg p-3" style={{ background: cardBg, border: `1px solid ${cardBorder}` }}>
        <div className="text-[13px] font-bold uppercase tracking-wider mb-2" style={{ color: textMuted }}>Period Summary</div>
        <div className="grid grid-cols-2 gap-y-1.5 gap-x-4">
          <StatRow label="Flights" value={String(stats?.totalFlights ?? 0)} text={text} muted={textSec} />
          <StatRow label="Block Hours" value={`${(stats?.totalBlockHrs ?? 0).toFixed(1)}h`} text={text} muted={textSec} />
