...
export default function MppLeadTimeAddScreen() {
  ...
  const [form, setForm] = useState({
    code: '', label: '', description: '', color: '#7c3aed', crewType: 'cockpit',
  })
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      ...
      try { const m = msg.match(/API (\d+): (.+)/); if (m) { const p = JSON.parse(m[2]); msg = Number(m[1]) === 409 ? 'This group code already exists.' : p.error || msg } } catch {}
      setError(msg)
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 100 }} keyboardShouldPersistTaps="handled">
        <View className={isTablet ? 'flex-row' : ''} style={{ gap: 12, marginBottom: 12 }}>
          <FormField label="Code *" value={form.code} flex={isTablet ? 0.4 : 1}
            ...
            palette={palette} mono maxLength={6} placeholder="e.g. PER" />
          <FormField label="Label *" value={form.label} flex={1}
            onChangeText={(v) => setForm(p => ({ ...p, label: v }))}
            palette={palette} placeholder="e.g. External Recruitment" />
        </View>
        ...
        <View className="flex-row" style={{ gap: 8, marginBottom: 12 }}>
          {CREW_TYPES.map(ct => {
            ...
            return (
              <Pressable key={ct.key} onPress={() => setForm(p => ({ ...p, crewType: ct.key }))}
                ...
                <Text style={{ fontSize: 14, fontWeight: active ? '600' : '400', color: active ? accent : palette.text }}>{ct.label}</Text>
              </Pressable>
            )
          })}
        </View>
        ...
        <View className="flex-row flex-wrap" style={{ gap: 8, marginBottom: 12 }}>
          {['#7c3aed', '#0891b2', '#be185d', '#dc2626', '#ea580c', '#16a34a', '#1e40af', '#6b7280'].map(c => (
            <Pressable key={c} onPress={() => setForm(p => ({ ...p, color: c }))}
              style={{ width: 36, height: 36, borderRadius: 8, backgroundColor: c, borderWidth: form.color === c ? 3 : 0, borderColor: '#fff',
                ...(form.color === c ? { shadowColor: c, shadowOpacity: 0.4, shadowRadius: 6, shadowOffset: { width: 0, height: 2 } } : {}) }} />
          ))}
        </View>
        ...
        <Pressable onPress={handleCreate} disabled={creating}
          ...
          <Text style={{ fontSize: 15, fontWeight: '600', color: '#fff' }}>{creating ? 'Creating...' : 'Add Group'}</Text>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
  )
