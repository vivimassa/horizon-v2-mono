...
import {
  Building2, Globe, Clock, Palette, Shield, Plane, Radio,
  Save, Check, X, ChevronRight, Upload, Trash2,
  Banknote, MapPin, Scale, Tag, Layers,
} from "lucide-react";
...
export default function OperatorConfigPage() {
  ...
  if (loading) {
    return (
      <div className="flex-1 flex items-center justify-center">
        <span className="text-[14px]" style={{ color: palette.textTertiary }}>Loading operator…</span>
      </div>
    );
  }
  ...
  if (!operator) {
    return (
      <div className="flex-1 flex items-center justify-center">
        <span className="text-[14px]" style={{ color: palette.textTertiary }}>No operator configured</span>
      </div>
    );
  }
...
function OperatorSidebar({
  operator, sections, activeSection, onSelect, palette, isDark, accent,
}: {
  operator: OperatorRef; sections: SectionDef[]; activeSection: string;
  onSelect: (key: string) => void; palette: PaletteType; isDark: boolean; accent: string;
}) {
  return (
            ...
            return (
              ...
              >
                <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0"
                  style={{ background: active ? accentTint(accent, isDark ? 0.18 : 0.1) : isDark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.04)" }}>
...
function OperatorCenter({
  operator, activeSection, draft, getVal, onChange, onSave, onRefresh,
  saving, saved, hasDraft, error, setError, palette, isDark, accent,
}: {
  operator: OperatorRef; activeSection: string; draft: Partial<OperatorRef>;
  getVal: (key: keyof OperatorRef) => any; onChange: (key: string, value: any) => void;
  onSave: () => void; onRefresh: () => void; saving: boolean; saved: boolean; hasDraft: boolean;
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {/* Error */}
      {error && (
        <div className="mx-6 mt-3 px-4 py-2.5 rounded-xl border flex items-center justify-between"
          ...
          <button onClick={() => setError("")}><X size={14} color={isDark ? "#f87171" : "#dc2626"} /></button>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5">
        {activeSection === "company" && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-5 max-w-3xl">
            <FormField label="Company Name" required value={getVal("name")} fieldKey="name" onChange={onChange} palette={palette} isDark={isDark} />
            <FormField label="Country" required value={getVal("country")} fieldKey="country" onChange={onChange} palette={palette} isDark={isDark} />
            <FormField label="ICAO Code" value={getVal("icaoCode") || getVal("code")} fieldKey="icaoCode" onChange={onChange} palette={palette} isDark={isDark}
              hint="3 uppercase letters" maxLength={3} uppercase />
            <FormField label="IATA Code" value={getVal("iataCode")} fieldKey="iataCode" onChange={onChange} palette={palette} isDark={isDark}
              hint="2 uppercase alphanumeric" maxLength={2} uppercase />
            <FormField label="Callsign" value={getVal("callsign")} fieldKey="callsign" onChange={onChange} palette={palette} isDark={isDark}
              hint="Radio telephony callsign" />
            <FormField label="Regulatory Authority" value={getVal("regulatoryAuthority")} fieldKey="regulatoryAuthority" onChange={onChange} palette={palette} isDark={isDark}
              hint="e.g. CAAV, FAA, EASA, CAA" />
          </div>
        )}
        {activeSection === "operations" && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-5 max-w-3xl">
            <FormField label="Timezone (IANA)" required value={getVal("timezone")} fieldKey="timezone" onChange={onChange} palette={palette} isDark={isDark}
              hint="e.g. Asia/Ho_Chi_Minh, Europe/London" />
            <AirportSelectField
              label="Main Base" required
            ...
            />
            <FormField label="FDTL Ruleset" value={getVal("fdtlRuleset")} fieldKey="fdtlRuleset" onChange={onChange} palette={palette} isDark={isDark}
        ...
        {activeSection === "branding" && (
          <LogoUploadSection operator={operator} onRefresh={onRefresh} palette={palette} isDark={isDark} accent={accent} />
        )}
            ...
            {ALL_MODULES.map(mod => {
              ...
              return (
                <div key={mod}
                  ...
                  <div>
                    <span className="text-[14px] font-medium capitalize" style={{ color: enabled ? accent : palette.text }}>
                      {mod.replace("-", " ")}
                    </span>
                  </div>
                  <button
                    ...
                    style={{
                      backgroundColor: enabled ? (isDark ? "rgba(22,163,74,0.15)" : "#dcfce7") : (isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.05)"),
...
/* ── Form Field ── */
function LogoUploadSection({ operator, onRefresh, palette, isDark, accent }: {
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    if (!/\.(jpe?g|png|svg|webp)$/i.test(file.name)) { setError("Only JPG, PNG, SVG, or WebP"); return; }
  ...
  return (
    <div className="max-w-xl">
      <label className="text-[12px] uppercase tracking-wider font-semibold mb-3 block" style={{ color: palette.textSecondary }}>
        Airline Logo
      </label>

      <div className="flex items-stretch gap-5">
        {/* Logo preview */}
        <div className="w-32 rounded-xl flex items-center justify-center overflow-hidden shrink-0"
          style={{
      ...
      <input ref={fileRef} type="file" accept=".jpg,.jpeg,.png,.svg,.webp" className="hidden"
        onChange={(e) => { const f = e.target.files?.[0]; if (f) handleFile(f); e.target.value = ""; }} />
    </div>
  );
}

function AirportSelectField({ label, value, onChange, palette, isDark, required, hint }: {
  label: string; value: string | null; onChange: (v: string | null) => void;
  palette: PaletteType; isDark: boolean; required?: boolean; hint?: string;
}) {
  const [airports, setAirports] = useState<{ iataCode: string | null; icaoCode: string; name: string; city: string | null }[]>([]);
  ...
  return (
    <div ref={containerRef} className="relative">
      <label className="text-[12px] uppercase tracking-wider font-semibold mb-1.5 block" style={{ color: palette.textSecondary }}>
      ...
      <button type="button" onClick={() => setOpen(!open)}
        ...
        <ChevronRight size={14} color={palette.textTertiary} style={{ transform: open ? "rotate(90deg)" : "rotate(0deg)", transition: "transform 150ms" }} />
      </button>
      ...
      {open && (
        <div className="absolute z-30 left-0 right-0 mt-1.5 rounded-xl overflow-hidden"
          ...
          <input type="text" value={search} placeholder="Search IATA, ICAO, name, city…"
            onChange={(e) => setSearch(e.target.value)} autoFocus
            className="w-full px-3 py-2.5 text-[13px] outline-none"
            style={{ borderBottom: `1px solid ${isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.06)"}`, background: "transparent", color: palette.text }}
          />
          <div className="max-h-[240px] overflow-y-auto">
            {filtered.length === 0 ? (
              <div className="px-3 py-3 text-[13px]" style={{ color: palette.textTertiary }}>No airports found</div>
            ) : (
              filtered.map(a => {
                const isCurrent = a.icaoCode === value;
                return (
                  <button key={a.icaoCode} type="button"
                    ...
                    onClick={() => { onChange(a.icaoCode); setOpen(false); setSearch(""); }}
                  ...
                  >
                    ...
                    <span className="text-[11px] shrink-0" style={{ color: palette.textTertiary }}>{a.city}</span>
                  </button>
                );
              })
            )}
      ...
      {hint && <p className="text-[11px] mt-1" style={{ color: palette.textTertiary }}>{hint}</p>}
    </div>
  );
}

function FormField({ label, value, fieldKey, onChange, palette, isDark, required, hint, maxLength, uppercase }: {
  label: string; value: any; fieldKey: string;
  onChange: (key: string, value: string | null) => void;
  palette: PaletteType; isDark: boolean;
  required?: boolean; hint?: string; maxLength?: number; uppercase?: boolean;
}) {
  return (
    <div>
      <label className="text-[12px] uppercase tracking-wider font-semibold mb-1.5 block" style={{ color: palette.textSecondary }}>
