...
export function AircraftTypesShell() {
  ...
  const handleSave = useCallback(
    async (id: string, data: Partial<AircraftTypeRef>) => {
  ...
  const handleCreate = useCallback(
    async (data: Partial<AircraftTypeRef>) => {
  ...
  const { filtered, groups } = useMemo(() => {
    ...
    const groups = Array.from(map.entries()).sort(([a], [b]) => (CATEGORY_ORDER[a] ?? 99) - (CATEGORY_ORDER[b] ?? 99))
