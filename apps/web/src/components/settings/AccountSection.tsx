...
export function AccountSection({ palette, isDark, accent, profileComplete, twoFactorEnabled }: AccountSectionProps) {
  ...
  const items: AccountItem[] = [
    ...
    },
    {
      ...
      iconColor: '#b45309',
      title: 'Notifications',
      subtitle: 'Push notifications, email alerts, in-app',
    },
    {
...
function AccountListItem({
  ...
  const content = (
    ...
    >
      ...
      >
        <Icon
          style={{ width: 'clamp(18px, 1.5vw, 24px)', height: 'clamp(18px, 1.5vw, 24px)', color: item.iconColor }}
          strokeWidth={1.6}
        />
      </div>
      <div className="flex-1 min-w-0" style={{ marginRight: 12 }}>
        ...
        <div style={{ fontSize: 'clamp(12px, 0.85vw, 14px)', color: palette.textSecondary, marginTop: 2 }}>
          {item.subtitle}
        </div>
      </div>
      <div className="flex items-center shrink-0" style={{ gap: 8 }}>
        {item.badge && (
          <div className="flex items-center" style={{ gap: 4 }}>
            {item.badge.label === 'Complete' && (
              <Check size={14} style={{ color: item.badge.color }} strokeWidth={2.5} />
            )}
            <span style={{ fontSize: F.min, fontWeight: 600, color: item.badge.color }}>{item.badge.label}</span>
          </div>
        )}
  ...
  return (
    <>
      ...
      {!isLast && <div style={{ height: 1, backgroundColor: palette.border, marginLeft: 84, marginRight: 20 }} />}
    </>
  )
}
