...
export function ExpiryCodesShell() {
  ...
  const fetchData = useCallback(() => {
    ...
    Promise.all([
      ...
      .then(([codesData, catsData]) => {
        ...
        setSelected((prev) => {
          if (prev) { const f = codesData.find((c) => c._id === prev._id); if (f) return f; }
  ...
  const handleSave = useCallback(async (id: string, data: Partial<ExpiryCodeRef>) => {
  ...
  const { groups, totalCount } = useMemo(() => {
    ...
    const groups = Array.from(map.entries()).sort(
      ([a], [b]) => (catOrder.get(a) ?? 99) - (catOrder.get(b) ?? 99)
    );
      ...
      center={
        showCreate ? (
          <ExpiryCodeDetail
            ...
            onCancel={() => { setShowCreate(false); if (codes.length > 0) setSelected(codes[0]); }}
          />
        ) : selected ? (
          <ExpiryCodeDetail
            code={selected}
    ...
    <div className="flex flex-col items-center justify-center h-full gap-4 px-6">
      ...
      <div className="text-center">
        ...
        <p className="text-[13px] text-hz-text-secondary max-w-sm">
          Expiry codes define qualification tracking rules, validity formulas, and enforcement levels.
          Seed defaults for a quick start, or add codes manually.
        </p>
      </div>
