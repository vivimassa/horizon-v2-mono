...
export default function ActivityCodeDetailScreen() {
  ...
  useEffect(() => {
    ...
    Promise.all([api.getActivityCodes(operatorId), api.getActivityCodeGroups(operatorId)])
      .then(([codes, g]) => { setCode(codes.find(c => c._id === id) ?? null); setGroups(g) })
  ...
  const handleDelete = useCallback(() => {
    if (!code) return
    if (code.isSystem) { Alert.alert('Cannot Delete', 'System codes cannot be deleted.'); return }
    Alert.alert('Delete Activity Code', `Delete ${code.code} — ${code.name}?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: async () => {
        try { await api.deleteActivityCode(code._id); router.back() }
      ...
      <View className="px-4 pt-4 pb-4" style={{ borderBottomWidth: 1, borderBottomColor: palette.border }}>
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center flex-1 mr-2">
            ...
            <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: code.color ?? groupColor, marginRight: 8 }} />
        ...
        <View className="flex-row items-center mt-2 flex-wrap" style={{ gap: 6, marginLeft: 36 }}>
          ...
          {isSystem && (
            <View className="flex-row items-center px-2 py-0.5 rounded-full" style={{ backgroundColor: isDark ? 'rgba(255,255,255,0.06)' : '#f3f4f6', gap: 3 }}>
          ...
          {code.isArchived && (
            <View className="px-2 py-0.5 rounded-full" style={{ backgroundColor: isDark ? 'rgba(217,119,6,0.15)' : '#fef3c7' }}>
        ...
        {(['general', 'credits'] as const).map(t => {
          ...
          return (
            <Pressable key={t} onPress={() => setTab(t)}
              ...
              <Text style={{ fontSize: 15, fontWeight: active ? '600' : '500', color: active ? accent : palette.textSecondary }}>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 100 }} keyboardShouldPersistTaps="handled">
        {tab === 'general' && (
          <>
            ...
            <View className={isTablet ? 'flex-row flex-wrap' : ''}>
              <Field label="Code" value={code.code} editing={editing && !isSystem} fieldKey="code" editValue={get('code')} onChange={handleFieldChange} palette={palette} mono maxLength={8} half={isTablet} />
              <Field label="Name" value={code.name} editing={editing && !isSystem} fieldKey="name" editValue={get('name')} onChange={handleFieldChange} palette={palette} half={isTablet} />
              <Field label="Short Label" value={code.shortLabel} editing={editing && !isSystem} fieldKey="shortLabel" editValue={get('shortLabel')} onChange={handleFieldChange} palette={palette} half={isTablet} />
              {!editing && <Field label="Group" value={group?.name} editing={false} fieldKey="" editValue="" onChange={() => {}} palette={palette} half={isTablet} />}
            </View>
            {editing && !isSystem && (
              <PickerField label="Group" options={groups.map(g => ({ key: g._id, label: g.name, color: g.color }))}
                editValue={get('groupId')} fieldKey="groupId" onChange={handleFieldChange} palette={palette} isDark={isDark} accent={accent} />
            )}
            ...
            <View style={{ paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: palette.border }}>
              ...
              <ColorSwatchPicker value={get('color') || groupColor} onChange={(v) => handleFieldChange('color', v)}
                palette={palette} isDark={isDark} editing={editing} />
            </View>

            <Field label="Description" value={code.description} editing={editing && !isSystem} fieldKey="description" editValue={get('description')} onChange={handleFieldChange} palette={palette} multiline />

            {editing && !isSystem && (
              <View className={isTablet ? 'flex-row flex-wrap' : ''}>
                <ToggleField label="Active" value={code.isActive} editing fieldKey="isActive" editValue={get('isActive')} onChange={handleFieldChange} palette={palette} isDark={isDark} half={isTablet} />
                <ToggleField label="Archived" value={code.isArchived} editing fieldKey="isArchived" editValue={get('isArchived')} onChange={handleFieldChange} palette={palette} isDark={isDark} half={isTablet} />
              </View>
            )}
            ...
            <View className={isTablet ? 'flex-row flex-wrap' : ''}>
              <Field label="Default Duration" value={minutesToHHMM(code.defaultDurationMin)} editing={editing && !isSystem} fieldKey="defaultDurationMin"
                editValue={get('defaultDurationMin') != null ? minutesToHHMM(get('defaultDurationMin')) : ''}
                onChange={(k, v) => handleFieldChange(k, hhmmToMinutes(v))} palette={palette} half={isTablet} placeholder="HH:MM" />
              <ToggleField label="Requires Time" value={code.requiresTime} editing={editing && !isSystem} fieldKey="requiresTime" editValue={get('requiresTime')} onChange={handleFieldChange} palette={palette} isDark={isDark} half={isTablet} />
            </View>
            {(get('requiresTime') ?? code.requiresTime) && (
              <View className={isTablet ? 'flex-row flex-wrap' : ''}>
                <Field label="Default Start" value={code.defaultStartTime} editing={editing && !isSystem} fieldKey="defaultStartTime" editValue={get('defaultStartTime')} onChange={handleFieldChange} palette={palette} half={isTablet} placeholder="HH:MM" />
                <Field label="Default End" value={code.defaultEndTime} editing={editing && !isSystem} fieldKey="defaultEndTime" editValue={get('defaultEndTime')} onChange={handleFieldChange} palette={palette} half={isTablet} placeholder="HH:MM" />
              </View>
            )}
            ...
            {FLAG_CATEGORIES.map(cat => {
              ...
              return (
                <View key={cat.label} className="mb-5" style={isTablet ? { width: '50%', paddingRight: 12 } : undefined}>
                  <Text style={{ fontSize: 13, fontWeight: '600', color: palette.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>{cat.label}</Text>
                  <View className="flex-row flex-wrap" style={{ gap: 8 }}>
                    {cat.flags.map(flag => {
                      const active = flags.includes(flag)
                      return (
                        <Pressable key={flag} disabled={!editing || isSystem}
                          onPress={() => {
                            const cur = [...flags]
                            const i = cur.indexOf(flag)
                            if (i >= 0) cur.splice(i, 1); else cur.push(flag)
                          ...
                          <Text style={{ fontSize: 14, fontWeight: active ? '600' : '400', color: active ? accent : palette.text }}>
        ...
        {tab === 'credits' && (
          <>
            ...
            <View className={isTablet ? 'flex-row flex-wrap' : ''}>
              <Field label="Credit Ratio" value={code.creditRatio != null ? `${code.creditRatio}x` : null} editing={editing && !isSystem} fieldKey="creditRatio"
                editValue={get('creditRatio')} onChange={handleFieldChange} palette={palette} numeric half={isTablet} placeholder="e.g. 1.0" />
              <Field label="Credit Override" value={minutesToHHMM(code.creditFixedMin)} editing={editing && !isSystem} fieldKey="creditFixedMin"
                editValue={get('creditFixedMin') != null ? minutesToHHMM(get('creditFixedMin')) : ''}
                onChange={(k, v) => handleFieldChange(k, hhmmToMinutes(v))} palette={palette} half={isTablet} placeholder="HH:MM" />
              <Field label="Pay Ratio" value={code.payRatio != null ? `${code.payRatio}x` : null} editing={editing && !isSystem} fieldKey="payRatio"
                editValue={get('payRatio')} onChange={handleFieldChange} palette={palette} numeric half={isTablet} placeholder="e.g. 1.0" />
            </View>

            <SectionBar label="Rest Periods" color={accent} />
            <View className={isTablet ? 'flex-row flex-wrap' : ''}>
              <Field label="Min Rest Before" value={minutesToHHMM(code.minRestBeforeMin)} editing={editing && !isSystem} fieldKey="minRestBeforeMin"
                editValue={get('minRestBeforeMin') != null ? minutesToHHMM(get('minRestBeforeMin')) : ''}
                onChange={(k, v) => handleFieldChange(k, hhmmToMinutes(v))} palette={palette} half={isTablet} placeholder="HH:MM" />
              <Field label="Min Rest After" value={minutesToHHMM(code.minRestAfterMin)} editing={editing && !isSystem} fieldKey="minRestAfterMin"
            ...
            {(get('flags') ?? code.flags ?? []).includes('is_simulator') && (
              <>
                ...
                <View className={isTablet ? 'flex-row flex-wrap' : ''}>
                  <Field label="Platform" value={code.simPlatform} editing={editing && !isSystem} fieldKey="simPlatform"
                    editValue={get('simPlatform')} onChange={handleFieldChange} palette={palette} half={isTablet} />
                  <Field label="Sim Duration" value={minutesToHHMM(code.simDurationMin)} editing={editing && !isSystem} fieldKey="simDurationMin"
            ...
            <View className="mt-4 p-3 rounded-lg" style={{ backgroundColor: accentTint(accent, isDark ? 0.06 : 0.03), borderWidth: 1, borderColor: accentTint(accent, 0.1) }}>
              <Text style={{ fontSize: 12, color: palette.textSecondary, lineHeight: 18 }}>
                Credit ratio multiplies block hours to derive credit hours. Override sets fixed credit time regardless of block. Rest periods enforced by FDTL rule engine.
              </Text>
            </View>
          </>
        )}
...
function Field({ label, value, editing, fieldKey, editValue, onChange, palette, mono, maxLength, multiline, numeric, half, placeholder }: {
  label: string; value: any; editing: boolean; fieldKey: string; editValue: any;
  onChange: (k: string, v: any) => void; palette: Palette; mono?: boolean; maxLength?: number;
  multiline?: boolean; numeric?: boolean; half?: boolean; placeholder?: string
}) {
    ...
    return (
      <View style={{ paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: palette.border, ...halfStyle }}>
        ...
        <TextInput value={editValue != null ? String(editValue) : ''}
          ...
          maxLength={maxLength} multiline={multiline} placeholder={placeholder} placeholderTextColor={palette.textTertiary}
...
function PickerField({ label, options, editValue, fieldKey, onChange, palette, isDark, accent }: {
