      ...
      <div className="bg-hz-card border border-hz-border rounded-2xl p-6 max-w-2xl w-full mx-4 shadow-xl space-y-4 max-h-[80vh] flex flex-col">
        ...
        {!generated ? (
          <div className="py-8 text-center space-y-4">
            <p className="text-[14px] text-hz-text-secondary">Generate ASM messages by comparing production schedule with the current view.</p>
          ...
          <>
            <div className="flex items-center justify-between shrink-0">
              <span className="text-[13px] text-hz-text-secondary">{messages.length} message{messages.length !== 1 ? "s" : ""}</span>
              <button onClick={handleCopy} className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-[12px] font-medium text-hz-text-secondary hover:bg-hz-border/30 transition-colors">
            ...
            <div className="flex-1 overflow-y-auto space-y-1">
              {messages.length === 0 ? (
                <p className="text-[14px] text-hz-text-tertiary py-8 text-center">No differences found — schedules are identical.</p>
              ) : messages.map((m, i) => {
                const color = ACTION_COLORS[m.actionCode] ?? ACTION_COLORS.TIM;
                return (
                  <div key={i} className="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-hz-border/50 hover:bg-hz-border/10 transition-colors">
                    <span className="text-[11px] font-bold px-2 py-0.5 rounded shrink-0" style={{ backgroundColor: color.bg, color: color.text }}>{m.actionCode}</span>
                    <span className="text-[13px] font-medium shrink-0">{m.flightNumber}</span>
                    <span className="text-[13px] text-hz-text-secondary shrink-0">{m.depStation}→{m.arrStation}</span>
