...
export function FdtMatrix({ table, isDark, onCellChange, onResetTable }: Props) {
  ...
  const commitAndMove = useCallback(
    async (direction: 'next' | 'prev' | 'stay') => {
  ...
  return (
    <div className="mb-6">
      ...
      {table.legalReference && <p className="text-[13px] text-hz-text-tertiary mb-2">{table.legalReference}</p>}
      ...
      <div className="overflow-x-auto rounded-xl border border-hz-border shadow-sm">
        ...
        >
          <thead>
            <tr>
              <th
                className="sticky left-0 z-10 px-4 py-2.5 text-left font-medium uppercase tracking-wider text-hz-text-tertiary bg-hz-bg border-b border-r border-hz-border"
                style={{ minWidth: 220 }}
              >
              ...
              {table.colLabels.map((cl, ci) => (
                <th
                  key={ci}
                  className="px-3 py-2.5 text-center font-medium uppercase tracking-wider text-hz-text-tertiary bg-hz-bg border-b border-hz-border whitespace-nowrap"
                  style={{ minWidth: 72 }}
                >
                  {cl}
                </th>
          ...
          <tbody>
            {table.rowKeys.map((rk, ri) => (
              <tr key={rk} className={ri % 2 === 0 ? '' : 'bg-hz-border/[0.04]'}>
                <td
                  className="sticky left-0 z-10 px-4 py-2.5 font-medium font-mono tabular-nums text-hz-text border-r border-hz-border bg-hz-bg whitespace-nowrap"
                  style={{ minWidth: 220 }}
                >
