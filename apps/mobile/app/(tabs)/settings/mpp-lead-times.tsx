...
import { Search, ChevronLeft, ChevronDown, Timer, Plus, RefreshCw, Sparkles, Pencil, Trash2 } from 'lucide-react-native'
          ...
          <View className="flex-row items-center mb-3">
            ...
            <View className="flex-1">
              ...
              <Text style={{ fontSize: 15, color: palette.textSecondary }}>
                {groups.length} group{groups.length !== 1 ? 's' : ''} · {items.length} item
                {items.length !== 1 ? 's' : ''}
              </Text>
            </View>
            <Pressable
              onPress={() => router.push('/(tabs)/settings/mpp-lead-time-add' as any)}
        ...
        {loading ? (
          ...
          <SectionList
            ...
            renderItem={({ item: group }) => (
              <GroupAccordion
                group={group}
                items={itemsByGroup.get(group._id) ?? []}
                palette={palette}
                accent={accent}
                isDark={isDark}
                ...
                onToggle={() =>
                  setExpanded((prev) => {
                    ...
                    n.has(group._id) ? n.delete(group._id) : n.add(group._id)
                    return n
                  })
                }
                onEdit={() =>
                  router.push({ pathname: '/(tabs)/settings/mpp-lead-time-detail' as any, params: { id: group._id } })
                }
    ...
    <View style={{ borderBottomWidth: 1, borderBottomColor: palette.border }}>
      ...
      >
        ...
        <View className="px-2 py-0.5 rounded mr-3" style={{ backgroundColor: `${group.color}20` }}>
          <Text style={{ fontSize: 13, fontWeight: '700', fontFamily: 'monospace', color: group.color }}>
            {group.code}
          </Text>
        </View>
        <View className="flex-1">
      ...
      {isExpanded && (
        <View style={{ paddingHorizontal: 16, paddingBottom: 12 }}>
          ...
          {items.length > 0 ? (
              ...
              {items.map((item, i) => (
                ...
                >
                  ...
                  <View
                    ...
                    style={{ backgroundColor: accentTint(group.color, isDark ? 0.15 : 0.08) }}
                  >
            ...
            <Text style={{ fontSize: 13, color: palette.textTertiary, fontStyle: 'italic' }}>
