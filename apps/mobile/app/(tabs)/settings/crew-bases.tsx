...
import {
  Search, ChevronLeft, ChevronRight, MapPin, Plus, Building2, RefreshCw,
...
export default function CrewBasesList() {
  ...
  useFocusEffect(useCallback(() => { fetchBases() }, [fetchBases]))
    ...
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={[]}>
      ...
      {loading ? (
        ...
        <View className="flex-1 justify-center items-center px-8">
          ...
          <Pressable onPress={fetchBases} className="flex-row items-center px-4 py-2 rounded-lg active:opacity-70"
        ...
        <SectionList
          ...
          renderItem={({ item }) => (
            <CrewBaseRow base={item} palette={palette} accent={accent} isDark={isDark}
              onPress={() => router.push({ pathname: '/(tabs)/settings/crew-base-detail' as any, params: { id: item._id } })} />
          )}
        />
      )}
    </SafeAreaView>
...
const CrewBaseRow = memo(function CrewBaseRow({
  base, palette, accent, isDark, onPress,
}: {
  base: AirportRef; palette: Palette; accent: string; isDark: boolean; onPress: () => void
}) {
  return (
    <Pressable
      onPress={onPress}
    ...
    >
      ...
      <View className="flex-row items-center" style={{ gap: 6 }}>
        {base.hasCrewFacilities && (
          <Building2 size={14} color={isDark ? '#4ade80' : '#16a34a'} strokeWidth={1.8} />
        )}
