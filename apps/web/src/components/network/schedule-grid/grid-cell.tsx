...
export const GridCell = React.memo(function GridCell({
  ...
  const isOutOfPeriod = isDateCol && normValue && normFrom && normTo && (normValue < normFrom || normValue > normTo)
  ...
  const periodWarningStyle: React.CSSProperties =
    isOutOfPeriod && !clampedDateDisplay ? { outline: '2px solid #E63535', outlineOffset: -2 } : {}
    ...
    >
      <span
        ...
        style={{
          color:
            forcedTextColor ??
            (isOutOfPeriod && !clampedDateDisplay
              ? '#E63535'
              : isSuggested
                ? '#8F90A6'
                : (fmt?.textColor ?? condFmt?.textColor ?? statusColor ?? undefined)),
