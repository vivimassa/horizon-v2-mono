...
import { FieldRow, Text } from '@/components/ui'
import { Plus, Pencil, Trash2, X, Save, Check, Lightbulb } from 'lucide-react'
...
const SURFACE_OPTIONS = ['ASPHALT', 'CONCRETE', 'ASPHALT/CONCRETE', 'GRAVEL', 'GRASS', 'DIRT', 'WATER', 'OTHER']
...
const ILS_OPTIONS = ['', 'CAT I', 'CAT II', 'CAT IIIA', 'CAT IIIB', 'CAT IIIC']
...
export function AirportRunwayTab({ airport, editing, draft = {}, onChange, onRefresh }: Props) {
  ...
  const handleAdd = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to add runway')
    } finally {
      setSaving(false)
    }
  ...
  const handleEditSave = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to update runway')
    } finally {
      setSaving(false)
    }
  ...
  const handleDelete = useCallback(
    async (rwId: string) => {
      ...
      try {
        ...
        setError(err.message || 'Failed to delete runway')
      } finally {
        setSaving(false)
      }
    },
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <Text variant="panelHeader" as="h3" className="!font-bold">
              Runways
            </Text>
        ...
        {error && (
          <div className="mb-3 px-3 py-2 rounded-lg border border-red-200 bg-red-50 dark:border-red-500/20 dark:bg-red-500/10 flex items-center justify-between">
            <Text variant="caption" as="span" className="!text-red-600 dark:!text-red-400">
              {error}
            </Text>
        ...
        {showAdd && (
          <div className="mb-4 p-4 rounded-xl border border-hz-border bg-hz-card space-y-3">
            <Text variant="cardTitle" as="div" className="!font-semibold">
              New Runway
            </Text>
            ...
            <div className="flex items-center gap-2 pt-1">
              <button
                onClick={handleAdd}
                disabled={saving}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[13px] font-semibold text-white transition-colors disabled:opacity-50 bg-module-accent"
              >
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-lg text-[13px] font-medium text-hz-text-secondary hover:bg-hz-border/30 transition-colors"
              >
                Cancel
              </button>
            </div>
            ...
            {runways.map((rw) => {
              ...
              if (isEditing) {
                return (
                  <div
                    key={rw._id}
                    className="p-4 rounded-xl border-2 border-module-accent/30 bg-module-accent/[0.03] space-y-3"
                  >
              ...
              return (
                ...
                >
                  ...
                  <div className="w-24 shrink-0">
                    <Text variant="body" as="div" className="!font-bold">
                      {rw.identifier}
                    </Text>
                    <Text variant="caption" as="div" className="mt-0.5">
                      {rw.status === 'active' ? (
                  ...
                  <div className="flex-1 grid grid-cols-2 lg:grid-cols-4 gap-x-4 gap-y-1">
                    <Field
                      label="Length"
                      value={rw.lengthFt ? `${rw.lengthFt.toLocaleString()} ft` : '—'}
                      sub={rw.lengthM ? `${rw.lengthM.toLocaleString()} m` : undefined}
                    />
                    <Field
                      label="Width"
                      value={rw.widthFt ? `${rw.widthFt.toLocaleString()} ft` : '—'}
                      sub={rw.widthM ? `${rw.widthM.toLocaleString()} m` : undefined}
                    />
                  ...
                  <div className="shrink-0 flex items-center gap-1">
                    ...
                    {confirmDeleteId === rw._id ? (
                      <div className="flex items-center gap-1">
                        <button
                          onClick={() => handleDelete(rw._id)}
                          disabled={saving}
                          className="px-2 py-1 rounded-lg text-[13px] font-semibold text-white bg-red-500 hover:bg-red-600 transition-colors"
                        >
                          Yes
                        </button>
                        <button
                          onClick={() => setConfirmDeleteId(null)}
                          className="px-2 py-1 rounded-lg text-[13px] font-medium text-hz-text-secondary hover:bg-hz-border/30 transition-colors"
                        >
                          No
                        </button>
                      </div>
                      ...
                      <button
                        onClick={() => setConfirmDeleteId(rw._id)}
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
          <FieldRow
            ...
            onChangeValue={change('numberOfGates')}
            type="number"
          />
          <FieldRow
            label="Fire Category"
            ...
            onChangeValue={change('fireCategory')}
            type="number"
          />
          <FieldRow
            ...
            onChangeValue={change('hasFuelAvailable')}
            type="toggle"
          />
          <FieldRow
            ...
            onChangeValue={change('hasCrewFacilities')}
            type="toggle"
          />
        </div>
      </div>
...
function RunwayFormFields({
  form,
  setForm,
}: {
  form: RunwayForm
  setForm: (fn: (prev: RunwayForm) => RunwayForm) => void
}) {
  return (
    <>
      <div className="flex gap-3">
        ...
        <div className="flex-1">
          <label className="text-[13px] text-hz-text-secondary uppercase tracking-wider font-medium">Surface</label>
          <select
            value={form.surface}
        ...
        <div className="flex-1">
          <label className="text-[13px] text-hz-text-secondary uppercase tracking-wider font-medium">Status</label>
          <select
            value={form.status}
      ...
      <div className="flex gap-3">
        <div className="flex-1">
          <label className="text-[13px] text-hz-text-secondary uppercase tracking-wider font-medium">Length (ft)</label>
          <input
            type="number"
            value={form.lengthFt}
        ...
        <div className="flex-1">
          <label className="text-[13px] text-hz-text-secondary uppercase tracking-wider font-medium">Width (ft)</label>
          <input
            type="number"
            value={form.widthFt}
...
function Field({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div>
      ...
      {sub && <div className="text-[13px] text-hz-text-secondary">{sub}</div>}
    </div>
  )
}
