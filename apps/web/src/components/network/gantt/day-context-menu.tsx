...
export function DayContextMenu() {
  const ctx = useGanttStore(s => s.dayContextMenu)
  const close = useGanttStore(s => s.closeDayContextMenu)
  const openSummary = useGanttStore(s => s.openDailySummary)
  ...
  return createPortal(
    ...
    >
      <button
        onClick={() => { openSummary(ctx.x, ctx.y, ctx.date); close() }}
      ...
      >
        ...
        <span className="text-[11px] font-mono" style={{ color: textMuted }}>F3</span>
      </button>
    </div>,
    document.body
  )
