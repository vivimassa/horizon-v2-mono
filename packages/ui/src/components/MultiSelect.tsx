...
export const MultiSelect = memo(function MultiSelect({
  items, value, onChange, allLabel, placeholder, accent, palette, isDark,
}: MultiSelectProps) {
  ...
  const isChecked = useCallback((key: string): boolean => {
    ...
    <>
      ...
      <Pressable onPress={() => setOpen(true)}
        ...
        style={{
          ...
          borderWidth: 1, borderColor: !isAll ? accentTint(accent, 0.3) : palette.cardBorder,
        }}>
        <Text style={{ flex: 1, fontSize: 14, fontWeight: !isAll ? '600' : '400',
          color: !isAll ? palette.text : palette.textTertiary }} numberOfLines={1}>
          {displayLabel}
        </Text>
      ...
      <Modal visible={open} transparent animationType="fade">
        <Pressable className="flex-1" style={{ backgroundColor: 'rgba(0,0,0,0.3)' }}
          ...
          <View className="flex-1 justify-center items-center">
            <Pressable onPress={() => {}} className="rounded-2xl overflow-hidden"
              ...
              {/* Search */}
              <View className="flex-row items-center px-3 py-2" style={{ borderBottomWidth: 1, borderBottomColor: palette.border }}>
                ...
                <TextInput value={search} onChangeText={setSearch}
                  ...
                  className="flex-1 ml-2" style={{ fontSize: 14, color: palette.text, height: 36 }} />
                {search.length > 0 && (
              ...
              {/* List */}
              <FlatList data={filtered} keyExtractor={i => i.key}
                style={{ maxHeight: 340 }}
                ListEmptyComponent={
                  ...
                  return (
                    <Pressable onPress={() => toggle(item.key)}
                      ...
                      <View style={{
                        ...
                        borderWidth: 2, borderColor: checked ? accent : palette.textTertiary,
                        backgroundColor: checked ? accent : 'transparent',
                        alignItems: 'center', justifyContent: 'center',
                      }}>
                        {checked && <Text style={{ color: '#fff', fontSize: 11, fontWeight: '800' }}>{'\u2713'}</Text>}
                      </View>
                      <Text style={{ fontSize: 14, fontWeight: '500', color: palette.text, flex: 1 }}>{item.label}</Text>
                      <Text style={{ fontSize: 13, fontFamily: 'monospace', color: palette.textTertiary }}>{item.key}</Text>
                    </Pressable>
                  )
                }} />
