...
import {
  ChevronLeft, Building2, Clock, Palette, Layers,
  Save, Check, X, ChevronDown, Search,
...
export default function OperatorConfigScreen() {
    ...
    return (
      <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={['top']}>
        <View className="flex-1 justify-center items-center">
          <Text style={{ fontSize: 14, color: palette.textTertiary }}>{loading ? 'Loading…' : 'No operator configured'}</Text>
        </View>
      </SafeAreaView>
    )
  }
  ...
  return (
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={['top']}>
      {/* Header */}
      <View className="px-4 pt-2 pb-3 flex-row items-center justify-between" style={{ borderBottomWidth: 1, borderBottomColor: palette.border }}>
        ...
        <Pressable onPress={handleSave} disabled={saving || !hasDraft}
          ...
          {saved ? <Check size={14} color="#fff" strokeWidth={2.5} /> : <Save size={14} color="#fff" strokeWidth={1.8} />}
          <Text style={{ fontSize: 13, fontWeight: '600', color: '#fff' }}>{saving ? 'Saving…' : saved ? 'Saved' : 'Save'}</Text>
        </Pressable>
      </View>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}
        ...
        {SECTIONS.map(section => {
          ...
          return (
            <Pressable key={section.key} onPress={() => setActiveSection(section.key)}
              ...
              <Text style={{ fontSize: 13, fontWeight: active ? '600' : '500', color: active ? accent : palette.textSecondary }}>{section.label}</Text>
            </Pressable>
          )
        })}
      </ScrollView>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 100 }}>
        {activeSection === 'company' && (
          <View style={{ gap: 0 }}>
            <Field label="Company Name" required value={getVal('name')} fieldKey="name" onChange={handleChange} palette={palette} />
            <Field label="Country" required value={getVal('country')} fieldKey="country" onChange={handleChange} palette={palette} />
            <View className="flex-row" style={{ gap: 16 }}>
              <View style={{ flex: 1 }}>
                <Field label="ICAO Code" value={getVal('icaoCode') || getVal('code')} fieldKey="icaoCode" onChange={handleChange} palette={palette}
                  hint="3 uppercase letters" maxLength={3} uppercase />
              </View>
              <View style={{ flex: 1 }}>
                <Field label="IATA Code" value={getVal('iataCode')} fieldKey="iataCode" onChange={handleChange} palette={palette}
                  hint="2 uppercase alphanumeric" maxLength={2} uppercase />
              </View>
            </View>
            <Field label="Callsign" value={getVal('callsign')} fieldKey="callsign" onChange={handleChange} palette={palette}
              hint="Radio telephony callsign" />
            <Field label="Regulatory Authority" value={getVal('regulatoryAuthority')} fieldKey="regulatoryAuthority" onChange={handleChange} palette={palette}
        ...
        {activeSection === 'operations' && (
          <View style={{ gap: 0 }}>
            <Field label="Timezone (IANA)" required value={getVal('timezone')} fieldKey="timezone" onChange={handleChange} palette={palette}
              hint="e.g. Asia/Ho_Chi_Minh" />
            <AirportPicker label="Main Base" required value={getVal('mainBaseIcao')} onChange={(v) => handleChange('mainBaseIcao', v)}
              palette={palette} isDark={isDark} accent={accent} hint="Primary hub — basis for FDTL and crew rules" />
            <Field label="FDTL Ruleset" value={getVal('fdtlRuleset')} fieldKey="fdtlRuleset" onChange={handleChange} palette={palette}
              hint="Fatigue & duty time regulation set" />
            <Field label="Currency Code" value={getVal('currencyCode')} fieldKey="currencyCode" onChange={handleChange} palette={palette}
              hint="e.g. USD, EUR, VND" maxLength={3} uppercase />
          </View>
        )}

        {activeSection === 'branding' && operator && (
          <LogoUploadMobile operator={operator} onRefresh={fetchOperator} palette={palette} isDark={isDark} accent={accent} />
        )}
              ...
              return (
                <Pressable key={mod}
                  ...
                  style={{
                    ...
                    borderWidth: 1,
                    borderColor: enabled ? accentTint(accent, isDark ? 0.2 : 0.12) : isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)',
                  }}
                  onPress={() => {
                    const next = enabled ? modules.filter(m => m !== mod) : [...modules, mod]
                    handleChange('enabledModules', next)
                  }}>
                  <Text style={{ fontSize: 15, fontWeight: '500', color: enabled ? accent : palette.text, textTransform: 'capitalize' }}>
                    {mod.replace('-', ' ')}
                  </Text>
                  <View className="px-3 py-1 rounded-lg"
                    style={{ backgroundColor: enabled ? (isDark ? 'rgba(22,163,74,0.15)' : '#dcfce7') : (isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.05)') }}>
                    <Text style={{ fontSize: 13, fontWeight: '600', color: enabled ? (isDark ? '#4ade80' : '#16a34a') : palette.textSecondary }}>
...
function LogoUploadMobile({ operator, onRefresh, palette, isDark, accent }: {
  operator: OperatorRef; onRefresh: () => void; palette: PaletteType; isDark: boolean; accent: string;
}) {
  ...
  const logoSrc = operator.logoUrl
    ? operator.logoUrl.startsWith('/uploads/') ? `${API_BASE}${operator.logoUrl}` : operator.logoUrl
    : null
  ...
  return (
    <View>
      <Text style={{ fontSize: 13, color: palette.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5, fontWeight: '600', marginBottom: 10 }}>
        Airline Logo
      </Text>

      {/* Logo preview */}
      <View className="items-center justify-center rounded-xl mb-4"
        style={{
          height: 100,
      ...
      <View className="flex-row" style={{ gap: 10 }}>
        ...
        {logoSrc && (
          <Pressable onPress={handleRemove} disabled={uploading}
...
function Field({ label, value, fieldKey, onChange, palette, required, hint, maxLength, uppercase }: {
  label: string; value: any; fieldKey: string;
  onChange: (key: string, value: any) => void;
  palette: PaletteType; required?: boolean; hint?: string; maxLength?: number; uppercase?: boolean;
}) {
  return (
    <View style={{ paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: palette.border }}>
      <Text style={{ fontSize: 13, color: palette.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5, fontWeight: '600', marginBottom: 4 }}>
...
function AirportPicker({ label, value, onChange, palette, isDark, accent, required, hint }: {
  label: string; value: string | null; onChange: (v: string | null) => void;
  palette: PaletteType; isDark: boolean; accent: string; required?: boolean; hint?: string;
}) {
  ...
  return (
    <View style={{ paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: palette.border }}>
      ...
      <Pressable onPress={() => setModalOpen(true)}
        ...
        <Text style={{ fontSize: 15, fontWeight: '500', color: value ? palette.text : palette.textTertiary }}>{displayLabel}</Text>
      ...
      <Modal visible={modalOpen} animationType="slide" presentationStyle="pageSheet">
        <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }}>
          <View className="px-4 pt-3 pb-3 flex-row items-center justify-between" style={{ borderBottomWidth: 1, borderBottomColor: palette.border }}>
            <Text style={{ fontSize: 18, fontWeight: '700', color: palette.text }}>Select Airport</Text>
            <Pressable onPress={() => { setModalOpen(false); setSearch('') }} className="active:opacity-60">
          ...
          <View className="px-4 py-2" style={{ borderBottomWidth: 1, borderBottomColor: palette.border }}>
            <View className="flex-row items-center rounded-xl" style={{ backgroundColor: palette.card, borderWidth: 1, borderColor: palette.cardBorder, paddingHorizontal: 13 }}>
              <Search size={16} color={palette.textTertiary} strokeWidth={1.8} />
              <TextInput className="flex-1 py-2.5 ml-2"
            ...
            renderItem={({ item: a }) => {
              ...
              return (
                <Pressable
                  ...
                  onPress={() => { onChange(a.icaoCode); setModalOpen(false); setSearch('') }}>
                  <Text style={{ fontSize: 15, fontWeight: '700', color: isCurrent ? accent : palette.text, width: 44 }}>{a.iataCode ?? '—'}</Text>
                  <View className="flex-1 ml-2">
                    <Text style={{ fontSize: 15, fontWeight: '500', color: palette.text }} numberOfLines={1}>{a.name}</Text>
                    <Text style={{ fontSize: 13, color: palette.textSecondary, marginTop: 1 }}>{a.city} · {a.icaoCode}</Text>
                  </View>
