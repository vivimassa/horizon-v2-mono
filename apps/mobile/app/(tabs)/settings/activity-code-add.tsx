    ...
    try {
      ...
      try {
        ...
        if (m) {
          ...
          msg = Number(m[1]) === 409 ? 'This activity code already exists.' : p.error || msg
        }
      } catch {}
      setError(msg)
    } finally {
      ...
      >
        ...
        <View className="flex-row flex-wrap" style={{ gap: 6, marginBottom: 12 }}>
          {groups.map((g) => {
            ...
            return (
              <Pressable
                key={g._id}
                onPress={() => setForm((p) => ({ ...p, groupId: g._id }))}
                className="flex-row items-center px-2.5 py-1.5 rounded-lg"
                style={{
                  backgroundColor: active ? accentTint(g.color, isDark ? 0.15 : 0.08) : 'transparent',
                  borderWidth: 1,
                  borderColor: active ? g.color : palette.cardBorder,
                  gap: 4,
                }}
              >
                <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: g.color }} />
                <Text
                  style={{ fontSize: 12, fontWeight: active ? '600' : '400', color: active ? g.color : palette.text }}
                >
                  {g.name}
                </Text>
              </Pressable>
        ...
        >
          <Text style={{ fontSize: 15, fontWeight: '600', color: '#fff' }}>
            {creating ? 'Creating...' : 'Add Activity Code'}
          </Text>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
