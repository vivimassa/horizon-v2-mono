...
export function DutyPatternDetail({ pattern, onSave, onDelete, onCreate }: DutyPatternDetailProps) {
  ...
  const handleCreate = useCallback(async () => {
    ...
    if (createForm.sequence.length < 2 || createForm.sequence.length % 2 !== 0) {
      setCreateError('Sequence must have at least one ON/OFF pair')
      return
    }
    if (createForm.sequence.some((v) => v < 1)) {
      setCreateError('Each segment must be at least 1 day')
      return
    }
    setCreating(true)
    setCreateError('')
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaving(true)
    try {
      await onSave(pattern._id, draft)
      setEditing(false)
      setDraft({})
    } catch {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-hz-border shrink-0">
        ...
        {pattern.description && <p className="text-[13px] text-hz-text-secondary mt-1">{pattern.description}</p>}
        {errorMsg && (
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
        ...
        {/* Sequence Section */}
        <div>
          <label className="text-[12px] text-hz-text-secondary uppercase tracking-wider font-medium mb-2 block">
            Sequence
          </label>
          {editing ? (
            <SequenceEditor sequence={currentSeq} onChange={(seq) => handleFieldChange('sequence', seq)} />
          ) : (
            <div className="flex items-center gap-2 flex-wrap">
              {currentSeq.map((n, i) => (
                <span
                  key={i}
                  className="text-[13px] font-bold font-mono px-2.5 py-1 rounded-lg"
                  style={{
          ...
          <p className="text-[13px] text-hz-text-tertiary mt-2">
            {seqLabel(currentSeq)} (repeats every {stats.cycleDays} days)
          </p>
        </div>
        ...
        {editing && (
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <button
                onClick={() => handleFieldChange('isActive', !getVal('isActive'))}
          ...
          <div className="grid grid-cols-15 gap-1" style={{ gridTemplateColumns: 'repeat(15, 1fr)' }}>
            {Array.from({ length: 30 }, (_, d) => {
              ...
              return (
                <div
                  key={d}
                  className="flex items-center justify-center h-8 rounded text-[11px] font-semibold"
                  style={{
...
function StatBox({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div className="text-center">
      ...
      <div className="text-[20px] font-bold font-mono tabular-nums" style={color ? { color } : undefined}>
        {value}
      </div>
    </div>
  )
...
function SequenceEditor({ sequence, onChange }: { sequence: number[]; onChange: (seq: number[]) => void }) {
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap mt-1">
      ...
      <button
        onClick={addPair}
        className="h-8 px-2 rounded-lg text-[12px] font-medium text-hz-text-secondary hover:bg-hz-border/30 border border-hz-border transition-colors"
