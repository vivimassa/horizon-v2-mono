...
import { ChevronLeft, Plane, ChevronDown, Search, Check } from 'lucide-react-native'
...
export default function LopaConfigAddScreen() {
  ...
  const handleCreate = useCallback(async () => {
    if (!form.aircraftType || !form.configName || form.cabins.length === 0) {
      setError('Aircraft type, config name, and at least one cabin are required')
      return
    }
    setCreating(true)
    setError('')
            ...
            <ScrollView style={{ maxHeight: 160 }} nestedScrollEnabled>
              {filteredTypes.map((t) => (
                <Pressable
                  key={t._id}
                  onPress={() => {
                    setForm((p) => ({ ...p, aircraftType: t.icaoType }))
        ...
        <View className="flex-row flex-wrap" style={{ gap: 6, marginBottom: 12 }}>
          {classOptions.map((cc) => {
            ...
            return (
              <Pressable
                key={cc.code}
                onPress={() => toggleCabinClass(cc.code)}
