...
import {
  Search, ChevronLeft, ChevronRight, Tag, Plus, RefreshCw, Sparkles, Lock,
...
export default function ActivityCodesList() {
  ...
  const fetchData = useCallback(() => {
    ...
    Promise.all([api.getActivityCodes(operatorId), api.getActivityCodeGroups(operatorId)])
      .then(([c, g]) => { setCodes(c); setGroups(g) })
  ...
  const { sections, filteredCount } = useMemo(() => {
    ...
    const filtered = q
      ? codes.filter(c => c.code.toLowerCase().includes(q) || c.name.toLowerCase().includes(q) || (c.description?.toLowerCase().includes(q)))
      : codes
    ...
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={[]}>
      <View className="px-4 pt-2 pb-3" style={{ borderBottomWidth: 1, borderBottomColor: palette.border }}>
        <View className="flex-row items-center mb-3">
          ...
          <Pressable onPress={() => router.push('/(tabs)/settings/activity-code-add' as any)}
        ...
        <View className="flex-row items-center rounded-xl" style={{
          ...
          <TextInput className="flex-1 py-2.5 ml-2" style={{ fontSize: 15, color: palette.text }}
            placeholder="Search code, name..." placeholderTextColor={palette.textTertiary}
      ...
      {loading ? (
        ...
        <SectionList
          ...
          renderItem={({ item }) => (
            <ActivityCodeRow code={item} palette={palette} accent={accent} isDark={isDark}
              groupColor={groupMap.get(item.groupId)?.color ?? accent}
              onPress={() => router.push({ pathname: '/(tabs)/settings/activity-code-detail' as any, params: { id: item._id } })} />
          )}
        />
      )}
    </SafeAreaView>
...
const ActivityCodeRow = memo(function ActivityCodeRow({
  code, palette, accent, isDark, groupColor, onPress,
}: {
  code: ActivityCodeRef; palette: Palette; accent: string; isDark: boolean; groupColor: string; onPress: () => void
}) {
  return (
    <Pressable onPress={onPress} className="flex-row items-center active:opacity-70" style={{
      ...
      <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: code.color ?? groupColor, marginRight: 10 }} />
      ...
      <View className="flex-1 ml-2">
        ...
        {code.description && (
          <Text style={{ fontSize: 12, color: palette.textTertiary, marginTop: 1 }} numberOfLines={1}>{code.description}</Text>
        )}
      </View>
