...
import { ChevronLeft, Pencil, Save, X, Trash2, Info, Landmark } from 'lucide-react-native'
...
export default function CountryDetailScreen() {
  ...
  const handleSave = useCallback(async () => {
    if (!country || Object.keys(draft).length === 0) {
      setEditing(false)
      return
    }
    setSaving(true)
  ...
  const handleDelete = useCallback(() => {
    if (!country) return
    Alert.alert('Delete Country', `Are you sure you want to delete ${country.name}?`, [
        ...
        <View className="flex-row items-center mt-2" style={{ gap: 8, marginLeft: 36 }}>
          ...
          >
            <Text style={{ fontSize: 13, color: palette.textSecondary }}>
              ISO:{' '}
              <Text style={{ fontWeight: '700', fontFamily: 'monospace', color: palette.text }}>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 100 }}>
        {activeTab === 'basic' && (
          <View className={isTablet ? 'flex-row flex-wrap' : ''}>
            ...
            <Field
              label="ICAO Prefix"
              value={country.icaoPrefix}
              editing={editing}
              fieldKey="icaoPrefix"
              editValue={get('icaoPrefix')}
              onChange={handleFieldChange}
              palette={palette}
              half={isTablet}
            />
