...
import {
  Search, ChevronLeft, ChevronRight, Timer, Plus,
...
const CATEGORY_ORDER: Record<string, number> = {
  'Airline Internal': 0, 'Passenger & Baggage': 1, 'Cargo & Mail': 2, 'Aircraft Handling': 3,
  'Technical': 4, 'Damage & EDP': 5, 'Operations & Crew': 6, 'Weather': 7,
  'ATC & Airport': 8, 'Reactionary & Misc': 9,
}

type ViewMode = 'legacy' | 'ahm732'
interface CodeSection { title: string; color: string; data: DelayCodeRef[] }
...
export default function DelayCodesList() {
  ...
  const fetchCodes = useCallback(() => {
    setLoading(true)
    api.getDelayCodes().then(setCodes).catch(console.error).finally(() => setLoading(false))
  }, [])

  useFocusEffect(useCallback(() => { fetchCodes() }, [fetchCodes]))

  const toggleGroup = useCallback((title: string) => {
    setCollapsed(prev => { const next = new Set(prev); if (next.has(title)) next.delete(title); else next.add(title); return next })
  }, [])

  const { sections, filteredCount } = useMemo(() => {
    const q = search.toLowerCase().trim()
    const filtered = q
      ? codes.filter(c => c.code.toLowerCase().includes(q) || (c.alphaCode?.toLowerCase().includes(q)) || c.name.toLowerCase().includes(q) || c.category.toLowerCase().includes(q))
      : codes
    ...
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={[]}>
      <View className="px-4 pt-2 pb-3" style={{ borderBottomWidth: 1, borderBottomColor: palette.border }}>
        <View className="flex-row items-center mb-3">
          ...
          <Pressable onPress={() => router.push('/(tabs)/settings/delay-code-add' as any)}
        ...
        {/* AHM mode toggle */}
        <View className="flex-row rounded-lg overflow-hidden mb-3" style={{ borderWidth: 1, borderColor: palette.cardBorder }}>
          <Pressable onPress={() => setViewMode('legacy')} className="flex-1 items-center py-4"
            style={viewMode === 'legacy' ? { backgroundColor: accent } : undefined}>
            <Text style={{ fontSize: 12, fontWeight: '600', color: viewMode === 'legacy' ? '#fff' : palette.textSecondary }}>AHM 730/731</Text>
          </Pressable>
          <Pressable onPress={() => setViewMode('ahm732')} className="flex-1 items-center py-4"
            style={viewMode === 'ahm732' ? { backgroundColor: accent } : undefined}>
            <Text style={{ fontSize: 12, fontWeight: '600', color: viewMode === 'ahm732' ? '#fff' : palette.textSecondary }}>AHM 732</Text>
          </Pressable>
        </View>
        ...
        <View className="flex-row items-center rounded-xl" style={{ backgroundColor: palette.card, borderWidth: 1, borderColor: palette.cardBorder, paddingHorizontal: 12 }}>
          ...
          <TextInput className="flex-1 py-2.5 ml-2" style={{ fontSize: 15, color: palette.text }}
            placeholder="Search code, name, category..." placeholderTextColor={palette.textTertiary}
      ...
      {loading ? (
        ...
        <SectionList
          ...
          renderItem={({ item }) => (
            <CodeRow code={item} viewMode={viewMode} palette={palette} accent={accent} isDark={isDark}
              onPress={() => router.push({ pathname: '/(tabs)/settings/delay-code-detail' as any, params: { id: item._id } })} />
          )}
        />
      )}
    </SafeAreaView>
...
const CodeRow = memo(function CodeRow({ code, viewMode, palette, accent, isDark, onPress }: {
  code: DelayCodeRef; viewMode: ViewMode; palette: Palette; accent: string; isDark: boolean; onPress: () => void
}) {
  const catColor = CATEGORY_COLORS[code.category] || '#6b7280'
  const ahm732 = code.ahm732Process || code.ahm732Reason || code.ahm732Stakeholder
  ...
  return (
    <Pressable onPress={onPress} className="flex-row items-center active:opacity-70"
      ...
      {viewMode === 'legacy' ? (
        <>
          <Text style={{ fontSize: 15, fontWeight: '700', fontFamily: 'monospace', color: accent, width: 36 }}>{code.code}</Text>
          {code.alphaCode && (
            <View className="px-1.5 py-0.5 rounded mr-2" style={{ backgroundColor: `${catColor}20` }}>
              <Text style={{ fontSize: 11, fontWeight: '700', fontFamily: 'monospace', color: catColor }}>{code.alphaCode}</Text>
            </View>
          )}
        </>
      ) : (
        <Text style={{ fontSize: 14, fontWeight: '700', fontFamily: 'monospace', color: ahm732 ? accent : palette.textTertiary, width: 60, marginRight: 10 }}>
          {ahm732 || '---'}
        </Text>
      )}
