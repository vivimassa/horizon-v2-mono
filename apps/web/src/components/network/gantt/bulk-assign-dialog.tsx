...
export function BulkAssignDialog({ open, onClose }: BulkAssignDialogProps) {
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        if (regDropdownOpen) { setRegDropdownOpen(false); return }
        onClose()
      }
    }
  ...
  const filteredAircraft = useMemo(() => {
    ...
    return sortedAircraft.filter(ac =>
      ac.registration.toUpperCase().includes(q) ||
      (ac.aircraftTypeIcao ?? '').toUpperCase().includes(q)
    )
    ...
    try {
      if (mode === 'assign') {
        ...
        if (res.verified !== undefined && res.verified < totalAssigned) {
          setResult(`Warning: ${res.verified}/${totalAssigned} verified. ${totalAssigned - res.verified} may have failed.`)
        }
        ...
        const updated = useGanttStore.getState().flights.map(f =>
          regMap.has(f.id) ? { ...f, aircraftReg: regMap.get(f.id)! } : f
        )
  ...
  const inputCls = 'h-10 rounded-lg text-[14px] outline-none focus:ring-2 focus:ring-module-accent focus:border-transparent transition-colors'
    ...
    <div data-gantt-overlay className="fixed inset-0 z-[9998] flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.4)' }}>
      <div
        ...
        style={{
          ...
          boxShadow: isDark
            ...
            : '0 16px 48px rgba(96,97,112,0.14)',
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 py-4" style={{ borderBottom: `1px solid ${palette.border}` }}>
          ...
          <span className="text-[15px] font-semibold flex-1" style={{ color: palette.text }}>Bulk Assign Aircraft</span>
        ...
        <div className="px-5 py-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[12px] font-medium mb-1.5 block" style={{ color: palette.textSecondary }}>From</label>
            ...
            <div>
              <label className="text-[12px] font-medium mb-1.5 block" style={{ color: palette.textSecondary }}>To</label>
              <div className="flex gap-1.5">
                <input
                  type="date"
                  value={toDate}
          ...
          <div>
            <label className="text-[12px] font-medium mb-1.5 block" style={{ color: palette.textSecondary }}>Action</label>
          ...
          <div>
            <label className="text-[12px] font-medium mb-1.5 block" style={{ color: palette.textSecondary }}>Aircraft Registration</label>
            <button
              ref={triggerRef}
            ...
            >
              ...
              <ChevronDown size={14} style={{ color: palette.textTertiary, transform: regDropdownOpen ? 'rotate(180deg)' : undefined, transition: 'transform 150ms' }} />
            </button>
          </div>
          ...
          {/* Result feedback */}
          {result && (
            <div className="text-[12px] font-medium text-center py-1" style={{
        ...
        {/* Footer */}
        <div className="flex items-center justify-end gap-2.5 px-5 py-3" style={{ borderTop: `1px solid ${palette.border}` }}>
          <button
            onClick={onClose}
          ...
          >
            ...
            {mode === 'assign' ? 'Assign' : 'Deassign'} {matchingFlights.length > 0 ? `(${matchingFlights.length})` : ''}
          </button>
        </div>
      </div>
        ...
        <div
          ...
          style={{
            ...
            boxShadow: isDark
              ...
              : '0 8px 32px rgba(96,97,112,0.10)',
            maxHeight: 300,
          }}
        >
          {/* Search */}
          ...
          {!regSearch && (
            ...
            >
              ...
              <span className="text-[12px] ml-auto" style={{ color: palette.textTertiary }}>{allRegs.size}</span>
            </button>
          )}
              ...
              return (
                <button
                  ...
                  style={{
                    color: palette.text,
                    background: checked
                      ? (isDark ? 'rgba(62,123,250,0.08)' : 'rgba(30,64,175,0.04)')
                      : undefined,
                  }}
                  onMouseEnter={e => { e.currentTarget.style.background = palette.backgroundHover }}
                  onMouseLeave={e => { e.currentTarget.style.background = checked ? (isDark ? 'rgba(62,123,250,0.08)' : 'rgba(30,64,175,0.04)') : 'transparent' }}
                >
                  <span className="w-4 h-4 rounded flex items-center justify-center shrink-0" style={{
                  ...
                  <span className="text-[12px]" style={{ color: palette.textTertiary }}>{ac.aircraftTypeIcao}</span>
                </button>
              )
            })}
            {filteredAircraft.length === 0 && (
              <div className="px-3 py-4 text-[12px] text-center" style={{ color: palette.textTertiary }}>No aircraft found</div>
            )}
          </div>
        </div>
      )}
