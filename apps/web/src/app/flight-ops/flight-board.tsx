...
function KpiStrip({ flights }: { flights: Flight[] }) {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
      <KpiPill label="On Time" value={onTime} colorClass="bg-hz-on-time-bg text-hz-on-time" />
      <KpiPill label="Delayed" value={delayed} colorClass="bg-hz-delayed-bg text-hz-delayed" />
      <KpiPill label="Cancelled" value={cancelled} colorClass="bg-hz-cancelled-bg text-hz-cancelled" />
      <KpiPill label="OTP" value={`${otp}%`} colorClass="bg-hz-accent-light text-hz-accent" />
    </div>
  )
}
      ...
      <div className="flex-1 p-4">
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <span className="text-xs text-hz-text-secondary ml-2">{flight.tail.icaoType ?? ''}</span>
          </div>
          <span className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-full ${s.badge}`}>{s.badgeText}</span>
        </div>
        ...
        <div className="flex items-center gap-3 mb-3">
          <div className="text-center">
            ...
            <div className={`text-xs text-hz-text-secondary ${isCancelled ? 'line-through' : ''}`}>
              {formatTime(flight.schedule.stdUtc)}
            </div>
          </div>
          <div className="flex-1 flex items-center">
            <div className="flex-1 border-t border-hz-border" />
            <svg className="h-3 w-3 text-hz-text-secondary -ml-px" viewBox="0 0 12 12" fill="currentColor">
        ...
        <div className="flex items-center justify-between text-xs text-hz-text-secondary">
          ...
          {flight.delays.length > 0 && (
            <span className="text-hz-delayed">+{flight.delays.reduce((s, d) => s + d.minutes, 0)}min</span>
          )}
        </div>
      </div>
    </div>
...
export function FlightBoard() {
  ...
  if (loading) {
    return <div className="text-sm text-hz-text-secondary animate-pulse">Loading flights…</div>
  }
  ...
  return (
    <div>
      ...
      <p className="text-[11px] font-semibold uppercase tracking-wider text-hz-text-secondary mb-3">Active flights</p>
