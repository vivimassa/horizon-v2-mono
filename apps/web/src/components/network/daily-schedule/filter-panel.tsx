...
export function FilterPanel({ onGo, loading }: FilterPanelProps) {
  ...
  const {
    ...
    timeFrom, timeTo, setTimeRange,
    acTypes, setAcTypes,
    acRegs, setAcRegs,
    domInt, setDomInt,
    statuses, setStatuses,
    showCancelled, setShowCancelled,
    timeModes, toggleTimeMode,
    sortSequence, setSortSequence,
  } = store
  ...
  const acTypeItems = refAcTypes
    .filter(t => t.isActive)
    .map(t => ({ key: t.icaoType, label: t.icaoType }))
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4 min-h-0">
          ...
          <FilterSection label="Time Range (UTC)">
            <div className="flex items-center gap-2">
              <TimeInput value={timeFrom} onChange={v => setTimeRange(v, timeTo)} placeholder="From" isDark={isDark} inputBg={inputBg} inputBorder={inputBorder} />
              <span className="text-[13px] text-hz-text-tertiary">-</span>
              <TimeInput value={timeTo} onChange={v => setTimeRange(timeFrom, v)} placeholder="To" isDark={isDark} inputBg={inputBg} inputBorder={inputBorder} />
            </div>
          </FilterSection>

          {/* AC Type */}
          <FilterSection label="Aircraft Type">
            <MultiDropdown items={acTypeItems} value={acTypes} onChange={setAcTypes} allLabel="All Types" isDark={isDark} inputBg={inputBg} inputBorder={inputBorder} />
          </FilterSection>

          {/* AC Reg */}
          <FilterSection label="Aircraft Registration">
            <MultiDropdown items={regItems} value={acRegs} onChange={setAcRegs} allLabel="All Registrations" isDark={isDark} inputBg={inputBg} inputBorder={inputBorder} />
          </FilterSection>
          ...
          <FilterSection label="Status">
            <MultiDropdown items={FLIGHT_STATUSES} value={statuses} onChange={setStatuses} allLabel="All Statuses" isDark={isDark} inputBg={inputBg} inputBorder={inputBorder} />
          </FilterSection>
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              style={{ background: showCancelled ? "var(--module-accent, #1e40af)" : (isDark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.15)") }}
            >
              <div className="absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform" style={{ left: showCancelled ? 18 : 2 }} />
            </button>
          </div>
              ...
              {TIME_MODES.map(m => {
                ...
                return (
                  <button
                    ...
                    style={{
                      background: active ? "var(--module-accent, #1e40af)" : (isDark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.04)"),
...
function TimeInput({ value, onChange, placeholder, isDark, inputBg, inputBorder }: {
  value: string; onChange: (v: string) => void; placeholder: string; isDark: boolean; inputBg: string; inputBorder: string
}) {
  return (
    <input
      type="text"
...
function SegmentedToggle({ options, value, onChange, isDark }: {
  options: { key: string; label: string }[]; value: string; onChange: (v: string) => void; isDark: boolean
}) {
  return (
    <div className="flex rounded-xl overflow-hidden" style={{ border: `1px solid ${isDark ? "rgba(255,255,255,0.10)" : "rgba(0,0,0,0.10)"}` }}>
      {options.map(o => (
        <button
          key={o.key}
...
function MultiDropdown({ items, value, onChange, allLabel, isDark, inputBg, inputBorder }: {
  ...
  allLabel: string; isDark: boolean; inputBg: string; inputBorder: string
}) {
