  ...
  const handleDelete = useCallback(() => {
    ...
    Alert.alert('Delete Pattern', `Delete pattern "${pattern.code}"?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          try {
            await api.deleteDutyPattern(pattern._id)
            router.back()
          } catch (err: any) {
  ...
  return (
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={['top']}>
      ...
      >
        ...
        {/* Stats */}
        <View
          className="flex-row items-center justify-around mb-4 py-3 rounded-lg"
        ...
        >
          ...
          <View className="items-center">
            <Text style={{ fontSize: 22, fontWeight: '800', fontFamily: 'monospace', color: OFF_COLOR }}>
              {offDays}
            </Text>
            <Text style={{ fontSize: 12, color: palette.textTertiary }}>Off Days</Text>
          </View>
          <View className="items-center">
            <Text style={{ fontSize: 22, fontWeight: '800', fontFamily: 'monospace', color: palette.text }}>
              {ratio}%
            </Text>
        ...
        <View className="flex-row flex-wrap mb-4" style={{ gap: 8 }}>
          ...
          {editing && draftSeq && (
            <View className="flex-row items-center" style={{ gap: 6 }}>
              <Pressable
                onPress={() => setDraftSeq((prev) => (prev ? [...prev, 1, 1] : [1, 1]))}
              ...
              {draftSeq.length > 2 && (
                <Pressable
                  onPress={() => setDraftSeq((prev) => (prev ? prev.slice(0, -2) : null))}
                  className="items-center justify-center rounded-lg active:opacity-60"
        ...
        {editing ? (
            ...
            <View className="flex-row flex-wrap" style={{ gap: 6 }}>
              {OFF_CODES.map((code) => {
                ...
                return (
                  <Pressable
                    key={code}
                    onPress={() => handleFieldChange('offCode', code)}
          ...
          <View style={{ paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: palette.border }}>
            ...
            <Text style={{ fontSize: 15, fontWeight: '600', fontFamily: 'monospace', color: palette.text }}>
              {pattern.offCode}
            </Text>
          </View>
        )}
