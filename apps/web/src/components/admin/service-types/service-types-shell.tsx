...
import {
  Search, Plus, Info, Pencil, Save, X, Trash2,
} from "lucide-react";
...
export function ServiceTypesShell() {
  ...
  const handleSave = useCallback(async (id: string, data: Partial<FlightServiceTypeRef>) => {
  ...
  const handleCreate = useCallback(async (data: Partial<FlightServiceTypeRef>) => {
...
function ServiceTypeList({ types, selected, onSelect, search, onSearchChange, loading, onCreateClick }: {
...
function ServiceTypeDetail({ serviceType, onSave, onDelete, onCreate, onCancelCreate }: {
  ...
  const handleSave = useCallback(async () => {
    if (!onSave || !serviceType || Object.keys(draft).length === 0) { setEditing(false); return; }
    setSaving(true); setErrorMsg("");
    try { await onSave(serviceType._id, draft); setEditing(false); setDraft({}); }
  ...
  const handleDelete = useCallback(async () => {
    ...
    setSaving(true); setErrorMsg("");
    try { await onDelete(serviceType._id); }
    catch (err: any) { setErrorMsg(friendlyError(err)); }
    ...
    return (
      <div className="flex flex-col h-full overflow-hidden">
        ...
        <div className="px-6 py-4 space-y-3 overflow-y-auto">
          ...
          <MiniInput label="Description" value={createForm.description}
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="px-6 pt-3 pb-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8">
            <FieldRow label="Code" value={<span className="font-bold font-mono">{serviceType.code}</span>}
              editing={editing} fieldKey="code" editValue={getVal("code")} onChange={handleFieldChange} />
            <FieldRow label="Name" value={serviceType.name}
              editing={editing} fieldKey="name" editValue={getVal("name")} onChange={handleFieldChange} />
            <FieldRow label="Description" value={serviceType.description}
            ...
            <div className="py-2.5 border-b border-hz-border/50">
              ...
              {editing ? (
                ...
                <div className="flex items-center gap-2">
                  <span className="w-5 h-5 rounded-full border border-hz-border/50" style={{ backgroundColor: serviceType.color || "#6b7280" }} />
