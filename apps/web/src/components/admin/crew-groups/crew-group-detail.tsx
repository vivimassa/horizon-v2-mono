...
import { Plus, Pencil, Save, X, Trash2, Users, Hash, AlertTriangle } from 'lucide-react'
...
export function CrewGroupDetail({ group, onCreate, onSave, onDelete, onCancel }: DetailProps) {
  ...
  return <ViewEditPanel group={group} onSave={onSave!} onDelete={onDelete!} />
}
...
function CreatePanel({
  ...
  onCreate: (d: Partial<CrewGroupRef>) => Promise<void>
  onCancel: () => void
}) {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
          <div className="py-2.5 border-b border-hz-border/50 col-span-2">
            ...
            <input
              type="text"
              value={form.name}
              placeholder="e.g. Management Pilots - AFS"
...
function ViewEditPanel({
  ...
  const getVal = (key: keyof CrewGroupRef) => (key in draft ? (draft as any)[key] : group[key])
      ...
      <div className="flex-1 overflow-y-auto px-6 pt-4 pb-6">
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
            ...
            <FieldRow
              label="Sort Order"
              value={<span className="font-mono">{group.sortOrder}</span>}
              editing={editing}
              fieldKey="sortOrder"
            ...
            <FieldRow
              label="Description"
              value={group.description || <span className="text-hz-text-secondary">No description</span>}
              editing={editing}
