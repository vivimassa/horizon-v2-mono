...
export default function DelayCodeAddScreen() {
  ...
  const [form, setForm] = useState({
    code: '', alphaCode: '', name: '', category: 'Airline Internal',
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      ...
      try { const m = msg.match(/API (\d+): (.+)/); if (m) { const p = JSON.parse(m[2]); msg = Number(m[1]) === 409 ? 'This delay code already exists.' : p.error || msg } } catch {}
      setError(msg)
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 100 }} keyboardShouldPersistTaps="handled">
        <View className="flex-row" style={{ gap: 12, marginBottom: 12 }}>
          <FormField label="Code *" value={form.code} flex={0.4}
            onChangeText={(v) => setForm(p => ({ ...p, code: v }))} palette={palette} mono maxLength={3} />
          <FormField label="Alpha Sub-Code" value={form.alphaCode} flex={0.4}
            onChangeText={(v) => setForm(p => ({ ...p, alphaCode: v.toUpperCase() }))} palette={palette} mono maxLength={2} />
          <FormField label="Name *" value={form.name} flex={1}
            onChangeText={(v) => setForm(p => ({ ...p, name: v }))} palette={palette} />
        </View>
        ...
        <View className="flex-row flex-wrap" style={{ gap: 6, marginBottom: 12 }}>
          {CATEGORIES.map(cat => {
            ...
            return (
              <Pressable key={cat} onPress={() => setForm(p => ({ ...p, category: cat, color: catColor }))}
                className="flex-row items-center px-2.5 py-1.5 rounded-lg"
                style={{ backgroundColor: active ? accentTint(catColor, isDark ? 0.15 : 0.08) : 'transparent', borderWidth: 1, borderColor: active ? catColor : palette.cardBorder, gap: 4 }}>
                <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: catColor }} />
                <Text style={{ fontSize: 11, fontWeight: active ? '600' : '400', color: active ? catColor : palette.text }}>{cat}</Text>
              </Pressable>
            )
          })}
        </View>
        ...
        <View style={{ marginBottom: 16 }}>
          ...
          <TextInput value={form.description} onChangeText={(v) => setForm(p => ({ ...p, description: v }))}
            placeholder="Optional description..." placeholderTextColor={palette.textTertiary} multiline
        ...
        <Pressable onPress={handleCreate} disabled={creating} className="items-center py-3.5 rounded-xl active:opacity-70"
          ...
          <Text style={{ fontSize: 15, fontWeight: '600', color: '#fff' }}>{creating ? 'Creating...' : 'Add Delay Code'}</Text>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
  )
...
function FormField({ label, value, onChangeText, palette, flex = 1, mono, maxLength }: {
  label: string; value: string; onChangeText: (v: string) => void; palette: any; flex?: number; mono?: boolean; maxLength?: number
}) {
  return (
    <View style={{ flex }}>
      ...
      <TextInput value={value} onChangeText={onChangeText} maxLength={maxLength} autoCapitalize={mono ? 'characters' : 'sentences'}
