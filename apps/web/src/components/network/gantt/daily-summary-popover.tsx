...
export function DailySummaryPopover() {
  ...
  const stats = useMemo(() => {
    ...
    const activityByType = [...byType.entries()]
      .map(([type, d]) => {
    ...
    const overnightStations = [...overnightMap.entries()]
      ...
      .map(([station, count]) => ({
        station,
        count,
        pct: acInService > 0 ? Math.round((count / acInService) * 100) : 0,
      }))
  ...
  return createPortal(
    ...
    >
      ...
      <div className="mx-4 mb-3 rounded-lg p-3" style={{ background: cardBg, border: `1px solid ${cardBorder}` }}>
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="text-center">
            ...
            <div className="text-[10px] font-medium uppercase tracking-wider" style={{ color: textMuted }}>
              Flights
            </div>
          </div>
          <div className="text-center">
            <div className="text-[22px] font-bold" style={{ color: text }}>
              {(stats?.totalBlockHrs ?? 0).toFixed(1)}
            </div>
            <div className="text-[10px] font-medium uppercase tracking-wider" style={{ color: textMuted }}>
              Block Hrs
            </div>
          ...
          <div className="text-center">
            <div className="text-[22px] font-bold" style={{ color: text }}>
              {stats?.acInService ?? 0}
              <span className="text-[14px] font-normal" style={{ color: textMuted }}>
                /{stats?.fleetSize ?? 0}
              </span>
            </div>
            <div className="text-[10px] font-medium uppercase tracking-wider" style={{ color: textMuted }}>
      ...
      {stats && stats.activityByType.length > 0 && (
        <div className="mx-4 mb-3 rounded-lg p-3" style={{ background: cardBg, border: `1px solid ${cardBorder}` }}>
          ...
          <div className="grid grid-cols-4 gap-2 mb-1">
            <span className="text-[13px] font-bold" style={{ color: textMuted }}>
              Type
            </span>
      ...
      {stats && stats.activityByType.length > 0 && (
        <div className="mx-4 mb-3 rounded-lg p-3" style={{ background: cardBg, border: `1px solid ${cardBorder}` }}>
          <div className="text-[13px] font-bold uppercase tracking-wider" style={{ color: textMuted }}>
            Utilization
          </div>
          <div className="text-[11px] mb-3" style={{ color: textMuted }}>
            ...
            return (
              <div key={row.type} className="mb-3">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[12px] font-bold w-[36px]" style={{ color: text }}>
                    {row.type}
                  </span>
                  <span className="text-[12px] font-semibold" style={{ color: '#06C270' }}>
                    {row.avgActive.toFixed(1)}h
                  </span>
                  <span className="text-[10px]" style={{ color: textMuted }}>
                    active
                  </span>
                  <span className="text-[12px] font-semibold ml-auto" style={{ color: textSec }}>
                    {row.avgFleet.toFixed(1)}h
                  </span>
                  <span className="text-[10px]" style={{ color: textMuted }}>
                    fleet
                  </span>
                ...
                <div className="relative h-[6px] rounded-full" style={{ background: cardBorder }}>
                  ...
                  <div
                    className="absolute inset-y-0 left-0 rounded-full"
                    style={{ width: `${activePct}%`, background: row.color }}
                  />
                  {/* Target line */}
                  <div
                    className="absolute"
                ...
                <div
                  ...
                  style={{ color: textMuted, marginLeft: `calc(${targetPct}% - 16px)` }}
                >
                  {row.target}h target
                </div>
              </div>
