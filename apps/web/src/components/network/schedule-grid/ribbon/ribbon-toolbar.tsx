...
export function RibbonToolbar({
  ...
  const collapsedItems: { icon: typeof Save; label: string; tooltip: string; onClick?: () => void; disabled?: boolean }[] = [
    // Flight
  ...
  return (
    ...
    >
      {isCollapsed ? (
        ...
        <div className="flex items-center gap-0.5 px-2 w-full" style={{ height: 52, animation: "bc-dropdown-in 150ms ease-out" }}>
          ...
          <Tooltip content="Expand toolbar">
            <button
              onClick={() => { setCollapsed(false); setAutoCollapsed(false); }}
        ...
        <div className="flex items-stretch gap-0 w-full overflow-x-auto" style={{ minHeight: 100, animation: "bc-dropdown-in 150ms ease-out" }}>
...
function CollapsedFontControls({ isDark, hasSelection }: { isDark: boolean; hasSelection: boolean }) {
  ...
  const applyToCell = useCallback((key: string, value: unknown) => {
  ...
  return (
    <div className="flex items-center gap-1.5 ml-1.5">
      <Dropdown
        ...
        value={fontFamily}
        onChange={(v) => { setFontFamily(v); applyToCell("fontFamily", v); }}
        size="sm"
      ...
      <Dropdown
        ...
        value={fontSize}
        onChange={(v) => { setFontSize(v); applyToCell("fontSize", Number(v)); }}
        size="sm"
        disabled={!hasSelection}
        className="w-[70px]"
      />
