              ...
              {state.selectedFlight ? (
                <>
                  <View className="flex-row items-center justify-between mb-1">
                    <Text style={{ fontSize: 16, fontWeight: '700', color: palette.text }}>{state.selectedFlight.id}</Text>
                    <Text
                      style={{ fontSize: 10, fontWeight: '600', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 10, overflow: 'hidden', backgroundColor: `${accent}20`, color: accent }}
                    >
                      {state.selectedFlight.status.charAt(0).toUpperCase() + state.selectedFlight.status.slice(1)}
                    </Text>
                  </View>
                  <View className="flex-row items-center mb-1.5">
                    <Text style={{ fontSize: 15, fontWeight: '600', color: palette.text }}>{state.selectedFlight.dep}</Text>
                    <View className="flex-1 mx-2" style={{ height: 1, backgroundColor: isDark ? 'rgba(255,255,255,0.12)' : 'rgba(0,0,0,0.1)' }} />
                    <PlaneTakeoff size={13} color={palette.textTertiary} style={{ marginHorizontal: 4 }} />
                    <View className="flex-1 mx-2" style={{ height: 1, backgroundColor: isDark ? 'rgba(255,255,255,0.12)' : 'rgba(0,0,0,0.1)' }} />
                    <Text style={{ fontSize: 15, fontWeight: '600', color: palette.text }}>{state.selectedFlight.arr}</Text>
                  </View>
                  <View className="flex-row items-center justify-between">
                    <Text style={{ fontSize: 11, color: palette.textTertiary }}>{state.selectedFlight.std} — {state.selectedFlight.sta}</Text>
                    <Text style={{ fontSize: 11, color: palette.textTertiary }}>{state.selectedFlight.aircraftType} · {state.selectedFlight.tailNumber}</Text>
                  </View>
                </>
              ) : (
