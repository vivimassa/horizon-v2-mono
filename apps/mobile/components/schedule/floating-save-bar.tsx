...
export const FloatingSaveBar = memo(function FloatingSaveBar({
  ...
  return (
    <View
      className="absolute bottom-6 left-4 right-4 flex-row items-center rounded-xl px-4 py-3"
      style={{
    ...
    >
      <View className="flex-1">
        <Text style={{ fontSize: 13, fontWeight: '600', color: palette.text }}>{parts.join(' \u00B7 ')}</Text>
      </View>
      <Pressable
        onPress={onDiscard}
        className="px-3 py-2 rounded-lg mr-2 active:opacity-70"
