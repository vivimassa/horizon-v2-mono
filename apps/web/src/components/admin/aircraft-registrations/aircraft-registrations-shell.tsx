...
export function AircraftRegistrationsShell() {
  ...
  const fetchData = useCallback(() => {
    setLoading(true)
    Promise.all([api.getAircraftRegistrations(), api.getAircraftTypes()])
      .then(([regs, types]) => {
  ...
  const handleSave = useCallback(async (id: string, data: Partial<AircraftRegistrationRef>) => {
    ...
    setRegistrations((prev) => prev.map((r) => (r._id === id ? { ...r, ...data } : r)))
    setSelected((prev) => (prev && prev._id === id ? { ...prev, ...data } : prev))
  }, [])
  ...
  const handleCreate = useCallback(
    async (data: Partial<AircraftRegistrationRef>) => {
      ...
      center={
        selected ? (
          ...
          <AircraftRegistrationDetail
            ...
            onCreate={handleCreate}
            onCancelCreate={() => {
              if (registrations.length > 0) setSelected(registrations[0])
            }}
          />
        )
      }
