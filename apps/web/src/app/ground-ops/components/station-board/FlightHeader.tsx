...
import {
  Plane, AlertTriangle, Clock, PlaneLanding,
  Package, Users, DoorOpen,
} from "lucide-react";
...
export function FlightHeader({ flight, accent, accentDark, isDark }: FlightHeaderProps) {
  ...
  const cargoPct = flight.cargo.capacity > 0
    ? Math.round((flight.cargo.loaded / flight.cargo.capacity) * 100)
    : 0;
      ...
      <div className="relative flex items-center justify-between w-full mb-3" style={{ zIndex: 1 }}>
        ...
        <div className="flex flex-col items-start">
          <div className="flex items-baseline gap-1.5">
            ...
            <span style={{ fontSize: 13, color: isDark ? "rgba(255,255,255,0.3)" : "rgba(0,0,0,0.25)", fontFamily: "monospace" }}>
              UTC+7
            </span>
          </div>
          <span
            ...
            >
              ...
              >
                <Icon
                  size={14}
                  strokeWidth={2}
                  style={{ color: isDelay ? "#dc2626" : accent }}
                />
              </div>
