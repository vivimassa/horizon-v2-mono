...
export default function CrewGroupAddScreen() {
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      ...
      try { const m = msg.match(/API (\d+): (.+)/); if (m) { const p = JSON.parse(m[2]); msg = Number(m[1]) === 409 ? 'A group with this name already exists.' : p.error || msg } } catch {}
      setError(msg)
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 100 }} keyboardShouldPersistTaps="handled">
        <View className={isTablet ? 'flex-row' : ''} style={{ gap: 12, marginBottom: 12 }}>
          <FormField label="Name *" value={form.name} flex={1}
            onChangeText={(v) => setForm(p => ({ ...p, name: v }))} palette={palette} placeholder="e.g. 50/50 Cockpit Crew" />
          <FormField label="Sort Order" value={form.sortOrder} flex={isTablet ? 0.4 : 1}
            onChangeText={(v) => setForm(p => ({ ...p, sortOrder: v }))} palette={palette} numeric placeholder="e.g. 10" />
        </View>
...
function FormField({ label, value, onChangeText, palette, flex = 1, numeric, placeholder, multiline }: {
