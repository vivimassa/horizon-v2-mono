...
import {
  api,
  setApiBaseUrl,
  type MppLeadTimeGroupRef,
  type MppLeadTimeItemRef,
} from "@skyhub/api";
...
export function MppLeadTimesShell() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [g, it] = await Promise.all([
        api.getMppLeadTimeGroups(),
        api.getMppLeadTimeItems(),
      ]);
      setGroups(g);
      setItems(it);
      setSelected((prev) => {
  ...
  const handleSaveGroup = useCallback(async (id: string, data: Partial<MppLeadTimeGroupRef>) => {
  ...
  const handleCreateGroup = useCallback(async (data: Partial<MppLeadTimeGroupRef>) => {
    const created = await api.createMppLeadTimeGroup(data);
    fetchData();
    setTimeout(() => setSelected(created), 300);
  }, [fetchData]);

  const handleCreateItem = useCallback(async (data: Partial<MppLeadTimeItemRef>) => {
    await api.createMppLeadTimeItem(data);
    fetchData();
  }, [fetchData]);

  const handleUpdateItem = useCallback(async (id: string, data: Partial<MppLeadTimeItemRef>) => {
