...
import { Plus, Search, X, Users, Package, Building2, Handshake, Heart, Moon, Trophy, FileText } from 'lucide-react'
...
export function ContractSidebar({ contracts, selectedId, onSelect, onNewContract, isDark }: ContractSidebarProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return contracts.filter((c) => c.contractNumber.toLowerCase().includes(q) || c.clientName.toLowerCase().includes(q))
  }, [contracts, search])
  ...
  return (
    <div className="shrink-0 flex flex-col h-full" style={{ width: 280, borderRight: `1px solid ${sectionBorder}` }}>
      ...
      <div
        ...
        style={{ minHeight: 44, borderBottom: `1px solid ${sectionBorder}` }}
      >
        <div className="flex items-center gap-2">
          <span className="text-[14px] font-bold">Contracts</span>
          <span
            className="px-1.5 py-0.5 rounded-full text-[13px] font-bold"
            style={{
...
function ContractCard({
  ...
  return (
    <button
      ...
      style={{
        background: isSelected ? (isDark ? 'rgba(62,123,250,0.12)' : 'rgba(30,64,175,0.08)') : 'transparent',
      }}
    >
      {/* Type icon */}
      <div
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-1.5">
          ...
          <span
            className="shrink-0 px-1.5 py-0.5 rounded text-[11px] font-semibold capitalize"
        ...
        <div className="text-[13px] text-hz-text-tertiary">
          {formatDateRange(contract.contractStart, contract.contractEnd)}
        </div>
      </div>
    </button>
  )
