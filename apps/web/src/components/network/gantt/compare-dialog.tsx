...
import {
  BarChart3, X, Trash2, Check, Clock, Loader2,
  Link2, Unlink2, TriangleAlert, Timer, Fuel, Plane,
} from 'lucide-react'
...
import {
  listOptimizerRuns, getOptimizerRun, deleteOptimizerRun, bulkAssignFlights,
  ...
  return createPortal(
    <div data-gantt-overlay className="fixed inset-0 z-[9998] flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.45)' }}>
      <div
        ...
        style={{
          width: 1060, maxWidth: '95vw', maxHeight: '90vh',
      ...
      >
        ...
        <div className="flex items-center gap-3 px-6 py-4">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: isDark ? 'rgba(62,123,250,0.06)' : 'rgba(30,64,175,0.03)' }}>
            <BarChart3 size={18} className="text-module-accent" />
          </div>
          <div className="flex-1">
            <div className="text-[15px] font-bold" style={{ color: palette.text }}>Scenario Comparison</div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4" style={{ borderTop: `1px solid ${palette.border}` }}>
          ...
          {!loading && runs.length === 0 && (
            <div className="flex flex-col items-center justify-center py-12" style={{ color: palette.textTertiary }}>
              ...
              <div className="text-[13px] mt-3 text-center">
                No saved runs for this period.<br />
          ...
          {!loading && runs.length > 0 && (
            <div className={`grid gap-4 ${runs.length === 1 ? 'grid-cols-1 max-w-[480px] mx-auto' : runs.length === 2 ? 'grid-cols-2' : runs.length === 3 ? 'grid-cols-3' : 'grid-cols-4'}`}>
              {runs.map(run => {
                ...
                const isBestFuel = (run.stats.fuelSavingsPercent ?? 0) === bestFuelSavings && bestFuelSavings > 0 && runs.length > 1
                const coveragePct = run.stats.totalFlights > 0 ? Math.round((run.stats.assigned / run.stats.totalFlights) * 100) : 0
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: selected ? (isDark ? '0 0 0 2px rgba(62,123,250,0.15)' : '0 0 0 2px rgba(30,64,175,0.08)') : undefined,
                    }}
                  >
                    {/* Delete button */}
                    ...
                    {/* Method badge */}
                    <div className="mb-2">
                      <span className="px-2.5 py-0.5 rounded-md text-[11px] font-semibold uppercase" style={{
                        background: run.config.method === 'fuel' ? 'rgba(255,136,0,0.12)' : run.config.method === 'balance' ? 'rgba(124,58,237,0.12)' : 'rgba(62,123,250,0.12)',
                        color: run.config.method === 'fuel' ? '#FF8800' : run.config.method === 'balance' ? '#7C3AED' : '#3E7BFA',
                      }}>{run.config.method === 'fuel' ? 'Fuel Efficient' : run.config.method === 'balance' ? 'Balance Fleet' : 'Minimize Gaps'}</span>
                    </div>
                    ...
                    <div className="text-[11px] font-medium mb-3" style={{ color: palette.textTertiary }}>
                      {run.createdAt ? new Date(run.createdAt).toLocaleString(undefined, { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : ''}
                    </div>
                    ...
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <div>
                        <div className="text-[18px] font-bold" style={{ color: '#06C270' }}>{run.stats.assigned.toLocaleString()}</div>
                        <div className="text-[11px] font-medium" style={{ color: palette.textTertiary }}>assigned</div>
                      </div>
                    </div>

                    {/* Metric grid */}
                    <div className="space-y-2">
                      <MetricRow icon={Link2} label="Assigned" value={run.stats.assigned.toLocaleString()}
                        isBest={isBestAssigned} palette={palette} isDark={isDark} />
                      <MetricRow icon={Unlink2} label="Overflow" value={String(run.stats.overflow)}
                        isBest={isBestOverflow} palette={palette} isDark={isDark} />
                      <MetricRow icon={TriangleAlert} label="Chain Breaks" value={String(run.stats.chainBreaks)}
                        isBest={isBestChainBreaks} palette={palette} isDark={isDark} />
                      <MetricRow icon={Timer} label="Time" value={`${(run.elapsedMs / 1000).toFixed(1)}s`}
                        isBest={false} palette={palette} isDark={isDark} />
                      {run.stats.fuelSavingsPercent != null && run.stats.fuelSavingsPercent > 0 && (
                        <MetricRow icon={Fuel} label="Fuel Savings" value={`${run.stats.fuelSavingsPercent}%`}
                          isBest={isBestFuel} palette={palette} isDark={isDark} />
                      )}
                    </div>
                        ...
                        {run.typeBreakdown.slice(0, 3).map(tb => {
                          ...
                          return (
                            <div key={tb.icaoType} className="flex items-center gap-2">
                              <span className="text-[11px] font-bold w-9 shrink-0" style={{ color: palette.textSecondary }}>{tb.icaoType}</span>
                              ...
                              <span className="text-[11px] font-mono font-medium" style={{ color: palette.textTertiary }}>{tb.avgBhPerDay}h/d</span>
                            </div>
                          )
                        })}
                      </div>
        ...
        <div className="flex items-center justify-between px-6 py-3.5" style={{ borderTop: `1px solid ${palette.border}` }}>
          <div className="text-[11px]" style={{ color: palette.textTertiary }}>
            {selectedId ? 'Click "Apply Selected" to assign flights from this run' : runs.length > 0 ? 'Select a run to apply' : ''}
          </div>
          <div className="flex items-center gap-2.5">
            ...
            {selectedId && (
              <button onClick={handleApply} disabled={applying}
                ...
                style={{ background: '#06C270', opacity: applying ? 0.5 : 1, boxShadow: !applying ? '0 2px 12px rgba(6,194,112,0.3)' : undefined }}>
                {applying ? <><Loader2 size={14} className="animate-spin" /> Applying...</> : <><Check size={14} /> Apply Selected</>}
              </button>
            )}
          </div>
        </div>
  ...
  return (
    <div className="relative shrink-0" style={{ width: 44, height: 44 }}>
      ...
      <div className="absolute rounded-full flex items-center justify-center" style={{
        inset: 5, background: isDark ? '#191921' : '#fff',
      }}>
        <span className="text-[11px] font-bold" style={{ color: '#06C270' }}>{percent}%</span>
      </div>
    </div>
  )
}

function MetricRow({ icon: Icon, label, value, isBest, palette }: {
  icon: typeof Plane; label: string; value: string; isBest: boolean
  palette: typeof colors.dark; isDark?: boolean
}) {
  return (
    <div className="flex items-center gap-2.5 py-0.5">
      <Icon size={13} style={{ color: palette.textTertiary, opacity: 0.6 }} />
      <span className="text-[12px] font-medium flex-1" style={{ color: palette.textSecondary }}>{label}</span>
      <span className="text-[13px] font-bold font-mono" style={{
        color: isBest ? '#06C270' : palette.text,
      }}>
        {value}
        {isBest && <span className="ml-1 text-[11px] font-semibold" style={{ color: '#06C270' }}>BEST</span>}
      </span>
    </div>
  )
}
