...
import { JumpseaterDialog } from './flight-information/jumpseater-dialog'

// Wrapper that lets Gantt-level callers (right-click menu) open the Jumpseater
// dialog by flight id. Fetches FlightDetail on mount for the Crew + jumpSeaters
// data the dialog needs.

export function GanttJumpseaterDialog() {
  const flightId = useGanttStore((s) => s.jumpseaterDialogFlightId)
  const close = useGanttStore((s) => s.closeJumpseaterDialog)
