...
import {
  Search, ChevronLeft, ChevronRight, ChevronDown, Users, Plus, RefreshCw, Sparkles, Lock, Pencil,
...
export default function CrewComplementsList() {
  ...
  const fetchData = useCallback(() => {
    ...
    setError(null)
    Promise.all([
      api.getCrewComplements(operatorId),
  ...
  const { sections, filteredCount } = useMemo(() => {
    ...
    const filtered = q
      ? complements.filter(c => c.aircraftTypeIcao.toLowerCase().includes(q) || c.templateKey.toLowerCase().includes(q))
      : complements
    ...
    const sections: TypeSection[] = Array.from(map.entries())
      ...
      .map(([icao, data]) => {
        data.sort((a, b) => {
          ...
          const ai = order.indexOf(a.templateKey), bi = order.indexOf(b.templateKey)
      ...
      <View className="px-4 pt-2 pb-3" style={{ borderBottomWidth: 1, borderBottomColor: palette.border }}>
        <View className="flex-row items-center mb-3">
          ...
          <View className="flex-1">
            <Text style={{ fontSize: 20, fontWeight: '700', color: palette.text }}>Crew Complements</Text>
            <Text style={{ fontSize: 15, color: palette.textSecondary }}>
              {filteredCount === complements.length
                ? `${complements.length} template${complements.length !== 1 ? 's' : ''}`
                : `${filteredCount} / ${complements.length} templates`}
            </Text>
          </View>
          <Pressable onPress={() => router.push('/(tabs)/settings/crew-complement-add' as any)}
        ...
        <View className="flex-row items-center rounded-xl" style={{
          ...
          <TextInput className="flex-1 py-2.5 ml-2" style={{ fontSize: 15, color: palette.text }}
            placeholder="Search aircraft type, template..." placeholderTextColor={palette.textTertiary}
        ...
        <SectionList
          ...
          ListEmptyComponent={
            <View className="flex-1 justify-center items-center px-8 pt-16">
              ...
              <View style={{ gap: 10 }}>
                <Pressable onPress={handleSeedAll} disabled={seeding}
          ...
          renderSectionHeader={({ section }) => {
            ...
            return (
              <Pressable className="flex-row items-center active:opacity-70" style={{
                ...
                <Text style={{ fontSize: 15, fontWeight: '700', fontFamily: 'monospace', color: accent, marginRight: 6 }}>{s.icao}</Text>
                <Text style={{ fontSize: 15, fontWeight: '600', color: palette.textSecondary, flex: 1 }} numberOfLines={1}>
          ...
          renderItem={({ item }) => (
            <ComplementAccordion
              ...
              onEdit={() => router.push({ pathname: '/(tabs)/settings/crew-complement-detail' as any, params: { id: item._id } })}
...
const ComplementAccordion = memo(function ComplementAccordion({
  comp, positions, isExpanded, onToggle, onEdit, palette, accent, isDark,
}: {
  comp: CrewComplementRef; positions: CrewPositionRef[];
  ...
  return (
    <View style={{ borderBottomWidth: 1, borderBottomColor: palette.border, opacity: comp.isActive ? 1 : 0.4 }}>
      ...
      <Pressable onPress={onToggle} className="flex-row items-center active:opacity-70"
        ...
        <View className="items-center rounded mr-3" style={{ backgroundColor: `${badgeColor}20`, width: 72, paddingVertical: 3 }}>
        ...
        <View style={{ paddingBottom: 12 }}>
          {cockpit.length <= 4 && cabin.length <= 4 ? (
            ...
            <View style={{ marginHorizontal: 16, marginBottom: 8 }}>
              <View className="flex-row rounded-lg overflow-hidden" style={{
                borderWidth: 1, borderColor: palette.cardBorder, backgroundColor: palette.card,
              }}>
                {positions.map((p, i) => (
                  <View key={p._id} className="items-center flex-1 py-2.5" style={{
                    borderLeftWidth: i > 0 ? 1 : 0, borderLeftColor: palette.cardBorder,
                  }}>
                    <Text style={{ fontSize: 12, fontWeight: '700', fontFamily: 'monospace', color: p.color ?? accent, marginBottom: 2 }}>{p.code}</Text>
                    <Text style={{ fontSize: 18, fontWeight: '800', fontFamily: 'monospace', color: accent }}>{comp.counts[p.code] ?? 0}</Text>
                  </View>
                ))}
              </View>
            </View>
            ...
            <>
              {cockpit.length > 0 && (
                <View style={{ marginHorizontal: 16, marginBottom: 8 }}>
                  <Text style={{ fontSize: 11, fontWeight: '600', color: '#3b82f6', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 6 }}>Flight Deck</Text>
                  <View className="flex-row rounded-lg overflow-hidden" style={{
                    borderWidth: 1, borderColor: palette.cardBorder, backgroundColor: palette.card,
                  }}>
                    {cockpit.map((p, i) => (
                      <View key={p._id} className="items-center flex-1 py-2.5" style={{
                        ...
                        <Text style={{ fontSize: 12, fontWeight: '700', fontFamily: 'monospace', color: p.color ?? '#3b82f6', marginBottom: 2 }}>{p.code}</Text>
                        <Text style={{ fontSize: 18, fontWeight: '800', fontFamily: 'monospace', color: accent }}>{comp.counts[p.code] ?? 0}</Text>
                      </View>
                    ))}
                  </View>
              ...
              {cabin.length > 0 && (
                <View style={{ marginHorizontal: 16, marginBottom: 8 }}>
                  <Text style={{ fontSize: 11, fontWeight: '600', color: '#f59e0b', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 6 }}>Cabin Crew</Text>
                  <View className="flex-row rounded-lg overflow-hidden" style={{
                    borderWidth: 1, borderColor: palette.cardBorder, backgroundColor: palette.card,
                  }}>
                    {cabin.map((p, i) => (
                      <View key={p._id} className="items-center flex-1 py-2.5" style={{
                        ...
                        <Text style={{ fontSize: 12, fontWeight: '700', fontFamily: 'monospace', color: p.color ?? '#f59e0b', marginBottom: 2 }}>{p.code}</Text>
                        <Text style={{ fontSize: 18, fontWeight: '800', fontFamily: 'monospace', color: accent }}>{comp.counts[p.code] ?? 0}</Text>
                      </View>
                    ))}
                  </View>
