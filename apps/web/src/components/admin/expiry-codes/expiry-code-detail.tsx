...
export function ExpiryCodeDetail({
  code,
  categories,
  onCreate,
  onSave,
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="px-6 py-4 border-b border-hz-border shrink-0">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onCancel} className="text-[13px] text-hz-text-secondary hover:text-hz-text font-medium">
            Cancel
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 pt-4 pb-6 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
          <FormField
            label="Linked Training Code"
            value={form.linkedTrainingCode}
            placeholder="e.g. LPC"
            onChange={(v) => set('linkedTrainingCode', v)}
          />
          <FormField label="Notes" value={form.notes} placeholder="Internal notes" onChange={(v) => set('notes', v)} />
        </div>
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-hz-border shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span
              className="text-[24px] font-extrabold font-mono px-3 py-1 rounded-lg"
              style={{
                backgroundColor: accentTint(category?.color ?? ACCENT, isDark ? 0.15 : 0.1),
      ...
      <div className="flex-1 overflow-y-auto px-6 pt-4 pb-6 space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
            <FieldRow
              label="Code"
              value={<span className="font-mono font-bold">{code.code}</span>}
              editing={editing}
              fieldKey="code"
                ...
                value={
                  category ? (
                    <span
                      className="text-[13px] font-semibold px-2 py-0.5 rounded-full"
                      style={{
                        backgroundColor: accentTint(category.color, isDark ? 0.12 : 0.08),
            ...
            <FieldRow
              label="Description"
              value={code.description || <span className="text-hz-text-secondary">No description</span>}
              editing={editing}
            ...
            <FieldRow
              label="Warning Days"
              value={
                code.warningDays != null ? (
                  <span className="font-mono">{code.warningDays} days</span>
                ) : (
                  <span className="text-hz-text-secondary">Not set</span>
                )
              }
              editing={editing}
              fieldKey="warningDays"
              editValue={getVal('warningDays')}
              onChange={(k, v) => setField(k, v === '' || v == null ? null : Number(v))}
              inputType="number"
            />
          </div>
        </div>
        ...
        {editing ? (
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
              <FieldRow
                label="Formula Type"
                value={<span className="font-medium">{formulaDef?.label ?? code.formula}</span>}
              />
              {formulaDef &&
              ...
              <FieldRow
                label="AC Type Scope"
                value={
                  code.acTypeScope === 'none' ? (
                    <span className="text-hz-text-secondary">Not type-specific</span>
                  ) : (
            ...
            {code.applicablePositions.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-2">
                {code.applicablePositions.map((pos) => (
                  <span
                    key={pos}
                    className="text-[13px] font-bold font-mono px-2.5 py-1 rounded-lg bg-gray-100 text-gray-700 dark:bg-gray-500/10 dark:text-gray-300"
                  >
                    {pos}
                  </span>
                ))}
        ...
        {editing && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
            ...
            <FieldRow
              ...
              editing={editing}
              fieldKey="linkedTrainingCode"
              editValue={getVal('linkedTrainingCode') ?? ''}
              onChange={(k, v) => setField(k, v === '' ? null : v)}
            />
          </div>
        )}
        ...
        {!editing && (
          <div className="rounded-2xl p-5 border border-red-200 dark:border-red-500/20">
            ...
            <p className="text-[13px] text-hz-text-secondary mb-3">
              Deleting an expiry code removes it permanently. If crew members have active records for this code,
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-[14px] font-bold uppercase tracking-wider" style={{ color: isDark ? '#e0e0e0' : ACCENT }}>
  ...
  return (
    <div
      ...
      style={{
        ...
        border: `1px solid ${isDark ? 'rgba(239,68,68,0.12)' : 'rgba(239,68,68,0.12)'}`,
      }}
    >
      <div className="flex items-center gap-2 mb-4">
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0">
                <div
                  className={`text-[13px] font-semibold ${active ? (def.isDestructive ? 'text-red-700 dark:text-red-400' : 'text-blue-700 dark:text-blue-400') : 'text-hz-text'}`}
                >
                  {def.label}
                </div>
...
function PositionsSection({
  ...
  return (
    <div
      className="rounded-2xl p-5"
      style={{
        background: isDark ? accentTint('#7c3aed', 0.04) : accentTint('#7c3aed', 0.03),
        border: `1px solid ${accentTint('#7c3aed', isDark ? 0.12 : 0.12)}`,
      }}
    >
...
function ErrorBanner({ msg, onDismiss }: { msg: string; onDismiss: () => void }) {
  return (
    <div className="mt-3 flex items-start gap-2 px-3 py-2.5 rounded-lg border border-red-200 bg-red-50 dark:border-red-500/20 dark:bg-red-500/10">
      ...
      <button onClick={onDismiss} className="text-red-400 hover:text-red-600 shrink-0 ml-auto">
