    ...
    const handleKeyDown = (e: KeyboardEvent) => {
        ...
        if (key === 'd') {
          ...
          if (selectedCell && selectedCell.rowIdx > 0) {
            ...
            if (aboveRow && currentRow && col?.editable) {
              ...
              const valueAbove =
                dirtyAbove !== undefined ? String(dirtyAbove) : ((aboveRow as any)[selectedCell.colKey] ?? '')
      ...
      if (e.shiftKey && ['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(e.key)) {
        ...
        if (
          ...
          selectionRange.endCol >= GRID_COLUMNS.length - 1 &&
      ...
      switch (e.key) {
        ...
        case 'Tab': {
          ...
          if (e.shiftKey) {
            if (curIdx > 0) state.startEditing({ rowIdx, colKey: editableCols[curIdx - 1].key })
              ...
              state.startEditing({ rowIdx: rowIdx - 1, colKey: editableCols[editableCols.length - 1].key })
          } else {
        ...
        case 'End':
          ...
          if (mod)
            state.selectCell({
              rowIdx: Math.max(0, allRows.length - 1),
              colKey: GRID_COLUMNS[GRID_COLUMNS.length - 1].key,
            })
