...
import {
  ...
  Info, LayoutGrid,
  Ruler, MoveHorizontal, Armchair, Zap, Monitor, Star,
...
export default function CabinClassDetailScreen() {
  ...
  useEffect(() => {
    ...
    Promise.all([api.getCabinClass(id), api.getLopaConfigs()])
      .then(([cc, configs]) => { setCabinClass(cc); setLopaConfigs(configs) })
      .catch(console.error)
  ...
  const handleSave = useCallback(async () => {
    if (!cabinClass || Object.keys(draft).length === 0) { setEditing(false); return }
    setSaving(true)
    try {
  ...
  const handleDelete = useCallback(() => {
    ...
    Alert.alert(
      'Delete Cabin Class',
      `Delete ${cabinClass.code} — ${cabinClass.name}?`,
      [
  ...
  return (
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={['top']}>
      ...
      <View className="px-4 pt-4 pb-4" style={{ borderBottomWidth: 1, borderBottomColor: palette.border }}>
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center flex-1 mr-2">
            ...
            <View style={{ width: 20, height: 20, borderRadius: 10, backgroundColor: classColor, marginRight: 8, borderWidth: 1, borderColor: palette.cardBorder }} />
      ...
      {cabinClass.seatLayout && (
        <View style={{ paddingVertical: 16, paddingHorizontal: 24, borderBottomWidth: 1, borderBottomColor: palette.border, alignItems: 'center' }}>
          <SeatRowPreview
      ...
      >
        {cabinClass.seatPitchIn != null && (
          <MetricCard icon={Ruler} label="Pitch" value={`${cabinClass.seatPitchIn}"`} color={classColor} palette={palette} />
        )}
        {cabinClass.seatWidthIn != null && (
          <MetricCard icon={MoveHorizontal} label="Width" value={`${cabinClass.seatWidthIn}"`} color={classColor} palette={palette} />
        )}
        {cabinClass.seatType && (
          <MetricCard icon={Armchair} label="Type" value={SEAT_TYPE_LABELS[cabinClass.seatType] || cabinClass.seatType} color={classColor} palette={palette} />
        )}
        <MetricCard icon={Zap} label="Power" value={cabinClass.hasPower ? 'Yes' : 'No'} color={cabinClass.hasPower ? classColor : '#9ca3af'} palette={palette} />
        <MetricCard icon={Monitor} label="IFE" value={cabinClass.hasIfe ? 'Yes' : 'No'} color={cabinClass.hasIfe ? classColor : '#9ca3af'} palette={palette} />
      </ScrollView>
        ...
        {TABS.map(tab => {
          ...
          return (
            <Pressable key={tab.key} onPress={() => setActiveTab(tab.key)}
              ...
              <Text style={{ fontSize: 14, fontWeight: active ? '600' : '500', color: active ? accent : palette.textSecondary }}>{tab.label}</Text>
            </Pressable>
          )
        })}
      </View>
      ...
      >
        {activeTab === 'specs' && (
          <>
            ...
            {/* Seat Type */}
            <View style={{ paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: palette.border }}>
              <Text style={{ fontSize: 12, color: palette.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5, fontWeight: '600', marginBottom: 6 }}>Seat Type</Text>
              {editing ? (
                <View className="flex-row flex-wrap" style={{ gap: 6 }}>
                  {SEAT_TYPES.map(st => {
                    const active = get('seatType') === st
                    return (
                      <Pressable key={st} onPress={() => handleFieldChange('seatType', st)}
          ...
          <>
            {usage.length === 0 ? (
              ...
              <>
                {usage.map(lc => {
                  ...
                  return (
                    <View key={lc._id}
                      ...
                      style={{ borderWidth: 1, borderColor: palette.cardBorder, paddingHorizontal: 12, paddingVertical: 10, marginBottom: 8, gap: 10 }}
                    >
                      <Text style={{ fontSize: 13, fontWeight: '700', fontFamily: 'monospace', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 6, backgroundColor: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.04)', color: palette.text, overflow: 'hidden' }}>
                        {lc.aircraftType}
                      </Text>
                      <View className="flex-1">
                        <View className="flex-row items-center" style={{ gap: 4 }}>
                          <Text style={{ fontSize: 13, fontWeight: '500', color: palette.text }} numberOfLines={1}>{lc.configName}</Text>
                ...
                <View className="flex-row items-center justify-between" style={{ paddingTop: 8, borderTopWidth: 1, borderTopColor: palette.border }}>
...
// ── Metric card ──
function MetricCard({ icon: Icon, label, value, color, palette }: {
  ...
  return (
    <View className="flex-row items-center rounded-xl"
      ...
      <View>
        <Text style={{ fontSize: 11, color: palette.textTertiary, textTransform: 'uppercase', letterSpacing: 0.5, fontWeight: '600' }}>{label}</Text>
