...
/**
 * 4.1.8.2 Transport Email — SMTP delivery worker.
 *
 * Same shape as hotel-email-deliver: scans pending|partial outbound docs,
 * drains pending|retrying delivery rows via the SMTP adapter, rolls up email
 * status. The adapter is shared with HotelEmail because the transport (plain
 * SMTP) is identical — only the source collection differs.
 *
 * Disable in dev/CI with: ENABLE_TRANSPORT_EMAIL_DELIVERY=false
 */
...
async function processEmail(emailId: string): Promise<void> {
  const doc = (await TransportEmail.findById(emailId).lean()) as Record<string, unknown> | null
  if (!doc) return
...
export async function runTransportEmailDeliveryTick(): Promise<void> {
  const open = await TransportEmail.find(
    {
      direction: 'outbound',
  ...
  for (const m of open) {
    try {
      ...
      console.error(`[transport-email-deliver] email ${m._id} failed:`, e instanceof Error ? e.message : e)
    }
  }
}

export function startTransportEmailDeliveryScheduler(): void {
  if (timer) return
  if (process.env.ENABLE_TRANSPORT_EMAIL_DELIVERY === 'false') {
    console.log('[transport-email-deliver] disabled via ENABLE_TRANSPORT_EMAIL_DELIVERY=false')
    return
  }

  const tick = () => {
    runTransportEmailDeliveryTick().catch((e) =>
      console.error('[transport-email-deliver] master tick error:', e instanceof Error ? e.message : e),
    )
  }

  void tick()
  timer = setInterval(tick, MASTER_TICK_MS)
  console.log('[transport-email-deliver] started (master tick every 60 s)')
}

export function stopTransportEmailDeliveryScheduler(): void {
  if (timer) {
    clearInterval(timer)
    timer = null
  }
