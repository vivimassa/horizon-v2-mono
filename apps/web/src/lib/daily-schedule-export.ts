...
function buildCellValue(flight: Flight, colId: ColumnId, ctx: ExportContext): string {
  switch (colId) {
    ...
    case "sta": {
      ...
      return ctx.activeModes.map(mode => {
        const { time, dayShift } = mode === "utc" ? { time: utcHhmm, dayShift: 0 }
          : mode === "localBase" ? applyOffset(utcHhmm, ctx.homeBaseOffset)
    ...
    case "acType": {
      const acType = flight.tail.icaoType || (flight.tail.registration ? ctx.regToTypeMap[flight.tail.registration] : null)
      return acType ?? ""
    }
    case "acReg":
...
export async function exportXlsx(ctx: ExportContext): Promise<void> {
  ...
  downloadBlob(buf, `daily-schedule-${ctx.periodFrom}-${ctx.periodTo}.xlsx`, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
}
...
export async function exportPdf(ctx: ExportContext): Promise<void> {
  ...
  const didDrawPage = () => {
    ...
    doc.text(`${formatDate(ctx.periodFrom, ctx.dateFormat)} - ${formatDate(ctx.periodTo, ctx.dateFormat)}  |  ${ctx.operatorName}`, pageW / 2, 20, { align: "center" })
