...
const CREW_TYPES: Array<{ key: string; label: string }> = [
  { key: 'cockpit', label: 'Cockpit' }, { key: 'cabin', label: 'Cabin' }, { key: 'other', label: 'Other' },
]
...
export default function MppLeadTimeDetailScreen() {
  ...
  const handleDeleteGroup = useCallback(() => {
    ...
    Alert.alert('Delete Group', `Delete "${group.label}" and all its items?`, [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
        try { await api.deleteMppLeadTimeGroup(group._id); router.back() }
  ...
  const handleDeleteItem = useCallback((item: MppLeadTimeItemRef) => {
  ...
  const handleUpdateItemMonths = useCallback(async (item: MppLeadTimeItemRef, val: string) => {
    ...
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={['top']}>
        ...
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center flex-1 mr-2">
            ...
            <View className="px-2 py-0.5 rounded mr-2" style={{ backgroundColor: `${group.color}20` }}>
              <Text style={{ fontSize: 15, fontWeight: '700', fontFamily: 'monospace', color: group.color }}>{group.code}</Text>
            </View>
            <Text style={{ fontSize: 18, fontWeight: '600', color: palette.text, flex: 1 }} numberOfLines={1}>{group.label}</Text>
          </View>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 100 }} keyboardShouldPersistTaps="handled">
        ...
        <View className={isTablet ? 'flex-row flex-wrap' : ''}>
          <Field label="Code" value={group.code} editing={editing} fieldKey="code" editValue={get('code')}
            onChange={handleFieldChange} palette={palette} mono maxLength={6} half={isTablet} />
          <Field label="Label" value={group.label} editing={editing} fieldKey="label" editValue={get('label')}
            onChange={handleFieldChange} palette={palette} half={isTablet} maxLength={80} />
        </View>
        {editing && (
          <View style={{ paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: palette.border }}>
            <Text style={{ fontSize: 12, color: palette.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5, fontWeight: '600', marginBottom: 6 }}>Crew Type</Text>
            <View className="flex-row" style={{ gap: 6 }}>
              {CREW_TYPES.map(ct => {
                const active = (get('crewType') ?? group.crewType) === ct.key
                return (
                  <Pressable key={ct.key} onPress={() => handleFieldChange('crewType', ct.key)}
                    ...
                    <Text style={{ fontSize: 13, fontWeight: active ? '600' : '400', color: active ? accent : palette.text }}>{ct.label}</Text>
                  </Pressable>
                )
              })}
            </View>
        ...
        {!editing && (
          <Field label="Crew Type" value={CREW_TYPES.find(c => c.key === group.crewType)?.label ?? group.crewType}
            editing={false} fieldKey="" editValue="" onChange={() => {}} palette={palette} />
        )}
        ...
        {addingItem && (
          <View className="mb-3 p-3 rounded-xl" style={{ backgroundColor: palette.card, borderWidth: 1, borderColor: palette.cardBorder }}>
            <View className={isTablet ? 'flex-row' : ''} style={{ gap: 8, marginBottom: 8 }}>
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 11, color: palette.textSecondary, fontWeight: '600', marginBottom: 2 }}>LABEL *</Text>
                <TextInput value={newItem.label} onChangeText={v => setNewItem(p => ({ ...p, label: v }))}
                  placeholder="e.g. FO External Hire" placeholderTextColor={palette.textTertiary}
                  style={{ fontSize: 14, color: palette.text, borderBottomWidth: 1, borderBottomColor: palette.border, paddingVertical: 4 }} />
              </View>
              <View style={{ width: isTablet ? 100 : undefined }}>
                <Text style={{ fontSize: 11, color: palette.textSecondary, fontWeight: '600', marginBottom: 2 }}>MONTHS *</Text>
                <TextInput value={newItem.valueMonths} onChangeText={v => setNewItem(p => ({ ...p, valueMonths: v }))}
                  keyboardType="numeric" placeholder="e.g. 5" placeholderTextColor={palette.textTertiary}
                  style={{ fontSize: 14, color: palette.text, borderBottomWidth: 1, borderBottomColor: palette.border, paddingVertical: 4 }} />
              </View>
            </View>
            <View style={{ marginBottom: 8 }}>
              <Text style={{ fontSize: 11, color: palette.textSecondary, fontWeight: '600', marginBottom: 2 }}>CONSUMED BY</Text>
              <TextInput value={newItem.consumedBy} onChangeText={v => setNewItem(p => ({ ...p, consumedBy: v }))}
                placeholder="e.g. MPP · AOC FO event" placeholderTextColor={palette.textTertiary}
                style={{ fontSize: 14, color: palette.text, borderBottomWidth: 1, borderBottomColor: palette.border, paddingVertical: 4 }} />
            </View>
        ...
        {items.length > 0 ? (
          <View className="rounded-lg overflow-hidden" style={{ borderWidth: 1, borderColor: palette.cardBorder, backgroundColor: palette.card }}>
            {items.map((item, i) => (
              <ItemRow key={item._id} item={item} groupColor={group.color} palette={palette} accent={accent} isDark={isDark}
                isFirst={i === 0} onDelete={() => handleDeleteItem(item)} onUpdateMonths={(v) => handleUpdateItemMonths(item, v)} />
            ))}
          </View>
        ) : !addingItem ? (
          <Text style={{ fontSize: 14, color: palette.textTertiary, fontStyle: 'italic', paddingVertical: 8 }}>No items yet</Text>
        ) : null}
      </ScrollView>
    </SafeAreaView>
  )
...
function ItemRow({ item, groupColor, palette, accent, isDark, isFirst, onDelete, onUpdateMonths }: {
  item: MppLeadTimeItemRef; groupColor: string; palette: Palette; accent: string; isDark: boolean;
  isFirst: boolean; onDelete: () => void; onUpdateMonths: (v: string) => void
}) {
  const [editingMonths, setEditingMonths] = useState(false)
  const [draftMonths, setDraftMonths] = useState(String(item.valueMonths))

  return (
    <View className="flex-row items-center px-3 py-3" style={{ borderTopWidth: isFirst ? 0 : 1, borderTopColor: palette.cardBorder }}>
      ...
      {editingMonths ? (
        <TextInput value={draftMonths} onChangeText={setDraftMonths}
          onBlur={() => { onUpdateMonths(draftMonths); setEditingMonths(false) }}
          onSubmitEditing={() => { onUpdateMonths(draftMonths); setEditingMonths(false) }}
          autoFocus keyboardType="numeric" textAlign="center"
          style={{ fontSize: 16, fontWeight: '700', fontFamily: 'monospace', color: groupColor, width: 50, borderWidth: 1, borderColor: groupColor, borderRadius: 6, paddingVertical: 2 }} />
      ) : (
        <Pressable onPress={() => { setDraftMonths(String(item.valueMonths)); setEditingMonths(true) }}
          className="px-2.5 py-1 rounded" style={{ backgroundColor: accentTint(groupColor, isDark ? 0.15 : 0.08) }}>
          <Text style={{ fontSize: 16, fontWeight: '700', fontFamily: 'monospace', color: groupColor }}>{item.valueMonths}m</Text>
        </Pressable>
      )}
    ...
    return (
      <View style={{ paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: palette.border, ...halfStyle }}>
        ...
        <TextInput value={editValue != null ? String(editValue) : ''}
          onChangeText={(v) => { if (mono) v = v.toUpperCase(); onChange(fieldKey, v) }}
