...
export function DelayCodesShell() {
    ...
    api
      ...
      .then((data) => {
        ...
        setSelected((prev: DelayCodeRef | null) => {
          if (prev) {
            const f = data.find((d) => d._id === prev._id)
            if (f) return f
          }
  ...
  const handleSave = useCallback(
    async (id: string, data: Partial<DelayCodeRef>) => {
  ...
  const handleCreate = useCallback(
    async (data: Partial<DelayCodeRef>) => {
    ...
    <MasterDetailLayout
      ...
      center={
        showHelp ? (
          ...
          <DelayCodeDetail delayCode={selected} onSave={handleSave} onDelete={handleDelete} onCreate={handleCreate} />
        ) : (
          <DelayCodeDetail
            delayCode={null}
                ...
                {!collapsed.has(category) && (
                  <div className="space-y-0.5">
                    {items.map((c) => {
                      ...
                      const ahm732 =
                        c.ahm732Process && c.ahm732Reason && c.ahm732Stakeholder
                          ? `${c.ahm732Process}-${c.ahm732Reason}-${c.ahm732Stakeholder}`
                          : null
                      return (
                        <button
...
function DelayCodeDetail({
  ...
  const friendlyError = useCallback((err: any) => {
    ...
    try {
      ...
      if (m) {
        ...
        return p.error || p.details?.join(', ') || msg
      }
    } catch {}
    return msg
  }, [])
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      await onCreate({
        ...
        alphaCode: createForm.alphaCode || null,
  ...
  const handleSave = useCallback(async () => {
    if (!onSave || !delayCode || Object.keys(draft).length === 0) {
      setEditing(false)
      return
    }
    setSaving(true)
    setErrorMsg('')
    try {
      await onSave(delayCode._id, draft)
      setEditing(false)
      setDraft({})
    } catch (err: any) {
  ...
  const getVal = (key: keyof DelayCodeRef) => (delayCode ? (key in draft ? (draft as any)[key] : delayCode[key]) : null)
        ...
        <div className="px-6 py-4 space-y-3 overflow-y-auto">
          <div className="flex gap-3">
            ...
            <MiniInput
              ...
              value={createForm.alphaCode}
              maxLength={2}
              onChange={(v) => setCreateForm((p) => ({ ...p, alphaCode: v.toUpperCase() }))}
              mono
            />
            <MiniInput
              label="Name *"
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-hz-border shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="text-[24px] font-extrabold font-mono px-3 py-1 rounded-lg bg-hz-border/30">
              {delayCode.code}
            </span>
            {delayCode.alphaCode && (
              <span
                className="text-[14px] font-bold font-mono px-2.5 py-1 rounded-lg"
                style={{ backgroundColor: catColor + '18', color: catColor }}
              >
                {delayCode.alphaCode}
              </span>
            ...
            {editing ? (
              <>
                ...
                <button
                  onClick={handleSave}
                  disabled={saving}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[13px] font-medium text-white bg-module-accent"
                >
                  ...
                  (confirmDelete ? (
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        disabled={saving}
                        className="px-2.5 py-1 rounded-lg text-[12px] font-semibold text-white"
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 rounded-lg text-[12px] font-medium text-hz-text-secondary hover:bg-hz-border/30"
                      >
                        No
                      </button>
                    </div>
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg text-hz-text-secondary/50 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10"
                      title="Delete"
                    >
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="px-6 pt-4 pb-6">
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              <span
                ...
                style={{ color: isDark ? '#e0e0e0' : accent }}
              >
                AHM 732 Equivalent
              </span>
            </div>
            {has732 ? (
              <>
                <div className="flex gap-3 items-stretch">
                  <TripleBox
                    label="Process"
                    letter={delayCode.ahm732Process}
                    desc=""
                    editing={editing}
                    fieldKey="ahm732Process"
                  ...
                  <div
                    ...
                    style={{ color: accentTint(accent, isDark ? 0.5 : 0.3) }}
                  >
                    –
                  </div>
                  <TripleBox
                    label="Reason"
                    letter={delayCode.ahm732Reason}
                    desc=""
                    editing={editing}
                    fieldKey="ahm732Reason"
                    onChange={handleFieldChange}
                    getVal={getVal}
                    isDark={isDark}
                    accent={accent}
                  ...
                  <TripleBox
                    label="Stakeholder"
                    letter={delayCode.ahm732Stakeholder}
                    desc=""
                    editing={editing}
                    fieldKey="ahm732Stakeholder"
                    onChange={handleFieldChange}
                    getVal={getVal}
                    isDark={isDark}
                    accent={accent}
                ...
                <div
                  ...
                  style={{ borderTop: `1px solid ${accentTint(accent, isDark ? 0.15 : 0.2)}` }}
                >
                  <span
                    className="text-[20px] font-extrabold font-mono tracking-[4px]"
                    style={{ color: isDark ? '#f0f0f0' : accent }}
                  >
                    {getVal('ahm732Process') || '?'} – {getVal('ahm732Reason') || '?'} –{' '}
                    {getVal('ahm732Stakeholder') || '?'}
                  </span>
          ...
          {/* Legacy fields */}
          <div className="text-[12px] font-bold uppercase tracking-wider text-hz-text-secondary mb-3">
            Legacy Code (AHM 730/731)
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8">
            <FieldRow
              label="Numeric Code"
              value={<span className="font-bold font-mono">{delayCode.code}</span>}
              editing={editing}
              fieldKey="code"
            ...
            <FieldRow
              label="Alpha Sub-Code"
              value={delayCode.alphaCode ? <span className="font-bold font-mono">{delayCode.alphaCode}</span> : null}
              editing={editing}
              fieldKey="alphaCode"
              editValue={getVal('alphaCode')}
              onChange={handleFieldChange}
            />
            <FieldRow
              label="Name"
              value={delayCode.name}
              editing={editing}
              fieldKey="name"
            ...
            <div className="py-2.5 border-b border-hz-border/50">
              ...
              {editing ? (
                ...
                <div className="flex items-center gap-2">
                  <span
                    className="w-4 h-4 rounded-full border border-hz-border/50"
                    style={{ backgroundColor: delayCode.color || '#6b7280' }}
                  />
...
function TripleBox({
  ...
  return (
    <div
      className="flex-1 rounded-xl p-4 text-center transition-all duration-200 hover:scale-[1.01] cursor-default"
      style={{
        background: isDark ? accentTint(accent, 0.08) : accentTint(accent, 0.04),
        border: `1px solid ${accentTint(accent, isDark ? 0.15 : 0.15)}`,
      }}
      onMouseEnter={(e) => {
        e.currentTarget.style.boxShadow = `0 0 10px ${accentTint(accent, isDark ? 0.25 : 0.2)}`
      }}
      onMouseLeave={(e) => {
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="px-6 py-4 border-b border-hz-border shrink-0">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-[13px] text-hz-text-secondary hover:text-hz-text transition-colors">
            Close
          </button>
        </div>
      </div>
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
        ...
        <div className="space-y-3 text-[13px] text-hz-text-secondary leading-relaxed">
          <div>
            ...
            <p className="mt-1">
              The current IATA standard using 2-digit numeric codes (00–99) organized into 10 categories, with optional
              2-letter alpha sub-codes. Used by the majority of airlines worldwide.
            </p>
          </div>
          <div>
            <span className="font-bold text-hz-text text-[14px]">AHM 732 (Triple-A Framework)</span>
            <p className="mt-1">
              The new IATA standard introduced in the 42nd edition of AHM. Replaces numeric codes with 3-letter
              combinations that provide richer context about each delay:
            </p>
          </div>
        </div>
            ...
            <div className="space-y-1.5 text-[12px]">
              {[
                ...
                <div key={k} className="flex items-center gap-2">
                  <span className="w-5 h-5 rounded bg-module-accent/15 text-module-accent text-[11px] font-bold flex items-center justify-center shrink-0 font-mono">
                    {k}
                  </span>
        ...
        <div className="rounded-xl border border-hz-border/50 p-4">
          ...
          <div className="text-[12px] text-hz-text-secondary mt-1">
            Legacy equivalent: <span className="font-mono font-bold">41</span>{' '}
            <span className="font-mono font-bold text-module-accent">TD</span> (Aircraft Defects)
          </div>
        </div>
        ...
        <div className="rounded-xl bg-hz-border/10 border border-hz-border/30 p-4">
          ...
          <div className="space-y-2 text-[12px] text-hz-text-secondary">
            <div className="flex gap-3">
              <span className="font-mono font-bold text-hz-text shrink-0">42nd ed.</span> AHM 732 introduced — coexists
              with legacy 730/731
            </div>
            <div className="flex gap-3">
              <span className="font-mono font-bold text-hz-text shrink-0">43rd ed.</span> Last edition containing AHM
              730/731
            </div>
            <div className="flex gap-3">
              <span className="font-mono font-bold text-hz-text shrink-0">44th ed.</span> AHM 732 becomes the sole
              standard (~2027)
            </div>
          </div>
        </div>
