...
export default function ExpiryCodeAddScreen() {
  ...
  const [form, setForm] = useState({
    code: '', name: '', categoryId: '', crewCategory: 'both',
    description: '', formula: 'fixed_validity', formulaParams: {} as Record<string, any>,
    warningDays: '30', severity: ['block_auto_assign', 'include_in_reports', 'show_validation_warning'] as string[],
    notes: '',
  })

  useEffect(() => {
    if (!operatorId) return
    api.getExpiryCodeCategories(operatorId).then(cats => {
      setCategories(cats)
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      ...
      try { const m = msg.match(/API (\d+): (.+)/); if (m) { const p = JSON.parse(m[2]); msg = Number(m[1]) === 409 ? 'This expiry code already exists.' : p.error || msg } } catch {}
      setError(msg)
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 100 }} keyboardShouldPersistTaps="handled">
        ...
        <View className={isTablet ? 'flex-row' : ''} style={{ gap: 12, marginBottom: 12 }}>
          <FormField label="Code *" value={form.code} flex={isTablet ? 0.4 : 1}
            onChangeText={(v) => setForm(p => ({ ...p, code: v.toUpperCase() }))} palette={palette} mono maxLength={10} placeholder="e.g. MED1" />
          <FormField label="Name *" value={form.name} flex={1}
            onChangeText={(v) => setForm(p => ({ ...p, name: v }))} palette={palette} placeholder="e.g. Medical Class 1" />
        </View>
        ...
        <View className="flex-row flex-wrap" style={{ gap: 6, marginBottom: 12 }}>
          {categories.map(cat => {
            ...
            return (
              <Pressable key={cat._id} onPress={() => setForm(p => ({ ...p, categoryId: cat._id }))}
                className="flex-row items-center px-2.5 py-1.5 rounded-lg"
                style={{ backgroundColor: active ? accentTint(cat.color, isDark ? 0.15 : 0.08) : 'transparent', borderWidth: 1, borderColor: active ? cat.color : palette.cardBorder, gap: 4 }}>
                <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: cat.color }} />
                <Text style={{ fontSize: 12, fontWeight: active ? '600' : '400', color: active ? cat.color : palette.text }}>{cat.label}</Text>
              </Pressable>
            )
          })}
        </View>
        ...
        {/* Crew Category */}
        <Text style={{ fontSize: 12, color: palette.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5, fontWeight: '600', marginBottom: 6 }}>Crew Category</Text>
        <View className="flex-row" style={{ gap: 8, marginBottom: 12 }}>
          {CREW_CATS.map(c => {
            const active = form.crewCategory === c.key
            return (
              <Pressable key={c.key} onPress={() => setForm(p => ({ ...p, crewCategory: c.key }))}
                ...
                <Text style={{ fontSize: 13, fontWeight: active ? '600' : '400', color: active ? accent : palette.text }}>{c.label}</Text>
              </Pressable>
            )
          })}
        </View>
        ...
        {/* Formula */}
        <Text style={{ fontSize: 12, color: palette.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5, fontWeight: '600', marginBottom: 6 }}>Formula</Text>
        <View className="flex-row flex-wrap" style={{ gap: 6, marginBottom: 12 }}>
          {EXPIRY_FORMULAS.map(f => {
            const active = form.formula === f.id
            return (
              <Pressable key={f.id} onPress={() => setForm(p => ({ ...p, formula: f.id, formulaParams: {} }))}
                className="px-2.5 py-1.5 rounded-lg"
                style={{ backgroundColor: active ? accentTint(accent, isDark ? 0.15 : 0.08) : 'transparent', borderWidth: 1, borderColor: active ? accent : palette.cardBorder }}>
                <Text style={{ fontSize: 12, fontWeight: active ? '600' : '400', color: active ? accent : palette.text }}>{f.label}</Text>
              </Pressable>
            )
          })}
        </View>
        ...
        {formulaDef && formulaDef.fields.length > 0 && (
          <View className={isTablet ? 'flex-row flex-wrap' : ''} style={{ gap: 12, marginBottom: 12 }}>
            {formulaDef.fields.map(f => (
              <FormField key={f.key} label={`${f.label}${f.unit ? ` (${f.unit})` : ''}`}
                value={form.formulaParams[f.key] != null ? String(form.formulaParams[f.key]) : ''}
                onChangeText={(v) => setForm(p => ({ ...p, formulaParams: { ...p.formulaParams, [f.key]: f.type === 'number' ? (v === '' ? undefined : Number(v)) : v } }))}
                palette={palette} numeric={f.type === 'number'} flex={isTablet ? 0.5 : 1} placeholder={f.placeholder} />
            ))}
          </View>
        )}
        ...
        {/* Warning days */}
        <View className={isTablet ? 'flex-row' : ''} style={{ gap: 12, marginBottom: 12 }}>
          <FormField label="Warning Days" value={form.warningDays}
            onChangeText={(v) => setForm(p => ({ ...p, warningDays: v }))} palette={palette} numeric flex={isTablet ? 0.4 : 1} placeholder="e.g. 30" />
        </View>

        {/* Severity */}
        <Text style={{ fontSize: 12, color: palette.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5, fontWeight: '600', marginBottom: 6 }}>Enforcement Rules</Text>
        <View style={{ gap: 6, marginBottom: 12 }}>
          {SEVERITY_DEFINITIONS.map(sev => {
            const active = form.severity.includes(sev.key)
            return (
              <Pressable key={sev.key} onPress={() => setForm(p => {
                const s = [...p.severity]; const i = s.indexOf(sev.key)
                if (i >= 0) s.splice(i, 1); else s.push(sev.key)
                return { ...p, severity: s }
                ...
                <View className="flex-1">
                  <Text style={{ fontSize: 13, fontWeight: '600', color: sev.isDestructive && active ? (isDark ? '#f87171' : '#dc2626') : palette.text }}>{sev.label}</Text>
        ...
        <FormField label="Description" value={form.description} multiline
          onChangeText={(v) => setForm(p => ({ ...p, description: v }))} palette={palette} placeholder="Optional description..." />
        <View style={{ height: 8 }} />
        <FormField label="Notes" value={form.notes} multiline
          onChangeText={(v) => setForm(p => ({ ...p, notes: v }))} palette={palette} placeholder="Optional notes..." />
        ...
        <Pressable onPress={handleCreate} disabled={creating}
          ...
          <Text style={{ fontSize: 15, fontWeight: '600', color: '#fff' }}>{creating ? 'Creating...' : 'Add Expiry Code'}</Text>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
  )
...
function FormField({ label, value, onChangeText, palette, flex = 1, mono, maxLength, numeric, placeholder, multiline }: {
  label: string; value: string; onChangeText: (v: string) => void; palette: any; flex?: number;
  mono?: boolean; maxLength?: number; numeric?: boolean; placeholder?: string; multiline?: boolean
}) {
  return (
