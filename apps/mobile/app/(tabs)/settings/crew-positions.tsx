...
import { Text, View, SectionList, Pressable, Alert } from 'react-native'
import { SafeAreaView } from 'react-native-safe-area-context'
import { useRouter } from 'expo-router'
import { api, type CrewPositionRef } from '@skyhub/api'
import { ChevronRight, UserRound, Plus, RefreshCw, Sparkles } from 'lucide-react-native'
      ...
      <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={['top']}>
        <View style={{ borderBottomWidth: 1, borderBottomColor: palette.border, paddingBottom: 12, paddingTop: 4 }}>
          <ListScreenHeader
            ...
            countLabel="position"
            onAdd={() => router.push('/(tabs)/settings/crew-position-add' as any)}
          />
          ...
          <SectionList
            ...
            renderItem={({ item }) => (
              <PositionRow
                ...
                isDark={isDark}
                onPress={() =>
                  router.push({ pathname: '/(tabs)/settings/crew-position-detail' as any, params: { id: item._id } })
                }
              />
            )}
          />
