...
export function CrewPositionsShell() {
  ...
  const fetchData = useCallback(() => {
    setLoading(true)
    api
      .getCrewPositions(getOperatorId(), true)
      .then((data) => {
        setPositions(data)
        setSelected((prev) => {
          if (prev) {
            const f = data.find((p) => p._id === prev._id)
            if (f) return f
          }
  ...
  const handleCreate = useCallback(
    async (data: Partial<CrewPositionRef>) => {
  ...
  const handleSave = useCallback(
    async (id: string, data: Partial<CrewPositionRef>) => {
      ...
      center={
        showCreate ? (
          <CrewPositionDetail
            ...
            onCancel={() => {
              setShowCreate(false)
              if (positions.length > 0) setSelected(positions[0])
            }}
          />
        ) : selected ? (
          <CrewPositionDetail
    ...
    <div className="flex flex-col items-center justify-center h-full gap-4 px-6">
      ...
      <div className="text-center">
        ...
        <p className="text-[13px] text-hz-text-secondary max-w-sm">
          Crew positions define the rank hierarchy for flight deck and cabin crew. Seed defaults for a quick start, or
