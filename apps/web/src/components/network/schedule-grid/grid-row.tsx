...
export const GridRow = React.memo(function GridRow({
  ...
  const handleFieldWire = useCallback(
  ...
  function handleNavigate(dir: 'up' | 'down' | 'left' | 'right') {
    ...
    if (dir === 'down') selectCell({ rowIdx: rowIdx + 1, colKey: selectedCell.colKey })
      ...
      if (prev) startEditing({ rowIdx, colKey: prev.key })
        ...
        startEditing({ rowIdx: rowIdx - 1, colKey: editableCols[editableCols.length - 1]?.key ?? selectedCell.colKey })
    }
  }
  ...
  return (
    <tr className="group transition-colors" style={{ height: rowHeight ?? ROW_HEIGHT, backgroundColor: rowBg }}>
      {/* Row number */}
      <td
      ...
      {columns.map((col, colIndex) => {
        ...
        const isInRange =
          ...
          rowIdx >= normRange.r1 &&
          rowIdx <= normRange.r2 &&
          colIndex >= normRange.c1 &&
          colIndex <= normRange.c2
        const isClipboardSource =
          clipboard !== null && clipboard.rowIds.includes(row._id) && clipboard.colKeys.includes(col.key)
