...
import { ChevronLeft, Pencil, Save, X, Trash2, Tag } from 'lucide-react-native'
...
export default function ServiceTypeDetailScreen() {
  ...
  const handleSave = useCallback(async () => {
    if (!serviceType || Object.keys(draft).length === 0) {
      setEditing(false)
      return
    }
    setSaving(true)
  ...
  const handleDelete = useCallback(() => {
    ...
    Alert.alert('Delete Service Type', `Delete ${serviceType.code} — ${serviceType.name}?`, [
