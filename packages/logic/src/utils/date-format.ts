...
export const DATE_FORMAT_OPTIONS: Array<{ value: DateFormatType; label: string; example: string }> = [
  { value: 'DD-MMM-YY', label: 'DD-MMM-YY', example: '01-Apr-26' },
  { value: 'DD/MM/YYYY', label: 'DD/MM/YYYY', example: '01/04/2026' },
  { value: 'MM/DD/YYYY', label: 'MM/DD/YYYY', example: '04/01/2026' },
  { value: 'YYYY-MM-DD', label: 'YYYY-MM-DD', example: '2026-04-01' },
  { value: 'DD.MM.YYYY', label: 'DD.MM.YYYY', example: '01.04.2026' },
]
 ...
 */
export function formatDate(isoDate: string | null | undefined, format: DateFormatType): string {
  if (!isoDate) return ''
...
/** Returns a short placeholder example for the selected format. */
export function datePlaceholder(format: DateFormatType): string {
  return DATE_FORMAT_OPTIONS.find((o) => o.value === format)?.example ?? 'YYYY-MM-DD'
}
