...
import {
  Search, ChevronLeft, ChevronRight, FileCheck, Plus, RefreshCw,
...
export default function ExpiryCodesList() {
  ...
  const fetchData = useCallback(() => {
    ...
    Promise.all([
      ...
      .then(([c, cats]) => { setCodes(c); setCategories(cats) })
    ...
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={[]}>
      <View className="px-4 pt-2 pb-3" style={{ borderBottomWidth: 1, borderBottomColor: palette.border }}>
        <View className="flex-row items-center mb-3">
          ...
          <View className="flex-1">
            <Text style={{ fontSize: 20, fontWeight: '700', color: palette.text }}>Expiry Codes</Text>
          ...
          <Pressable onPress={() => router.push('/(tabs)/settings/expiry-code-add' as any)}
        ...
        <View className="flex-row items-center rounded-xl" style={{
          ...
          <TextInput className="flex-1 py-2.5 ml-2" style={{ fontSize: 15, color: palette.text }}
            placeholder="Search code, name, formula..." placeholderTextColor={palette.textTertiary}
      ...
      {loading ? (
        ...
        <SectionList
          ...
          renderItem={({ item }) => (
            <ExpiryCodeRow code={item} palette={palette} accent={accent} isDark={isDark}
              catColor={catMap.get(item.categoryId)?.color ?? accent}
              onPress={() => router.push({ pathname: '/(tabs)/settings/expiry-code-detail' as any, params: { id: item._id } })} />
          )}
        />
      )}
    </SafeAreaView>
...
const ExpiryCodeRow = memo(function ExpiryCodeRow({
  code, palette, accent, isDark, catColor, onPress,
}: {
  code: ExpiryCodeRef; palette: Palette; accent: string; isDark: boolean; catColor: string; onPress: () => void
}) {
  ...
  return (
    <Pressable onPress={onPress} className="flex-row items-center active:opacity-70" style={{
      ...
      <View className="flex-1 ml-2">
        <Text style={{ fontSize: 15, fontWeight: '500', color: palette.text }} numberOfLines={1}>{code.name}</Text>
        <Text style={{ fontSize: 12, color: palette.textTertiary, marginTop: 1 }}>{formulaDef?.label ?? code.formula}</Text>
      </View>
      <View className="flex-row items-center" style={{ gap: 6 }}>
        <View className="px-1.5 py-0.5 rounded" style={{
          backgroundColor: code.crewCategory === 'cockpit' ? (isDark ? 'rgba(59,130,246,0.15)' : '#eff6ff')
            : code.crewCategory === 'cabin' ? (isDark ? 'rgba(139,92,246,0.15)' : '#f5f3ff')
            : (isDark ? 'rgba(255,255,255,0.06)' : '#f3f4f6'),
        }}>
          <Text style={{ fontSize: 11, fontWeight: '600',
            color: code.crewCategory === 'cockpit' ? (isDark ? '#93c5fd' : '#1d4ed8')
              : code.crewCategory === 'cabin' ? (isDark ? '#c4b5fd' : '#6d28d9')
              : palette.textSecondary,
          }}>{CREW_CAT_LABELS[code.crewCategory] ?? 'All'}</Text>
        </View>
