...
import {
  Search, ChevronLeft, ChevronDown, Timer, Plus, RefreshCw, Sparkles, Pencil,
...
export default function DutyPatternsList() {
  ...
  const fetchData = useCallback(() => {
    ...
    api.getDutyPatterns(operatorId)
      .then(list => { setPatterns(list); setExpanded(new Set(list.map(p => p._id))) })
    ...
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={[]}>
      <View className="px-4 pt-2 pb-3" style={{ borderBottomWidth: 1, borderBottomColor: palette.border }}>
        <View className="flex-row items-center mb-3">
          ...
          <Pressable onPress={() => router.push('/(tabs)/settings/duty-pattern-add' as any)}
        ...
        <View className="flex-row items-center rounded-xl" style={{
          ...
          <TextInput className="flex-1 py-2.5 ml-2" style={{ fontSize: 15, color: palette.text }}
            placeholder="Search code, description..." placeholderTextColor={palette.textTertiary}
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <PatternAccordion pattern={item} palette={palette} accent={accent} isDark={isDark}
              ...
              onEdit={() => router.push({ pathname: '/(tabs)/settings/duty-pattern-detail' as any, params: { id: item._id } })} />
          )}
        />
      )}
    </SafeAreaView>
...
const PatternAccordion = memo(function PatternAccordion({
  pattern, palette, accent, isDark, isExpanded, onToggle, onEdit,
}: {
  pattern: DutyPatternRef; palette: Palette; accent: string; isDark: boolean;
    ...
    <View style={{ borderBottomWidth: 1, borderBottomColor: palette.border, opacity: pattern.isActive ? 1 : 0.4 }}>
      ...
      {isExpanded && (
        <View style={{ paddingHorizontal: 16, paddingBottom: 12 }}>
          <View className="rounded-lg p-3" style={{ backgroundColor: palette.card, borderWidth: 1, borderColor: palette.cardBorder }}>
            {/* Description */}
            {pattern.description && (
            ...
            <View className="flex-row flex-wrap mb-3" style={{ gap: 4 }}>
              {pattern.sequence.map((days, i) => (
                <View key={i} className="flex-row items-center px-2 py-1 rounded" style={{
            ...
            <View className="flex-row items-center" style={{ gap: 12, marginBottom: 8 }}>
              ...
              <Text style={{ fontSize: 13, fontFamily: 'monospace', color: palette.textTertiary }}>Off: {pattern.offCode}</Text>
            </View>
