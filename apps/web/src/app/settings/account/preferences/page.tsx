...
import { ArrowLeft, SlidersHorizontal, Languages, Clock, Calendar, Hash, Ruler, Save, Check } from 'lucide-react'
          ...
          <div className="px-5 py-4">
            ...
            >
              ...
              <p style={{ fontSize: F.min, color: palette.textSecondary, marginTop: 4 }}>
                Distance: {prefs.units === 'metric' ? '1,250 km' : '776 mi'} &middot; Weight:{' '}
                {prefs.units === 'metric' ? '78,500 kg' : '173,063 lb'}
              </p>
              <p style={{ fontSize: F.min, color: palette.textSecondary, marginTop: 2 }}>
                Fuel: {currentNumber === 'comma' ? '12,450.00' : currentNumber === 'dot' ? '12.450,00' : '12 450.00'}{' '}
                {prefs.units === 'metric' ? 'L' : 'gal'}
              </p>
            </div>
          </div>
        </aside>
...
function SelectField({
  ...
  return (
    <div className="py-3" style={{ borderBottom: `0.5px solid ${palette.border}` }}>
      <label className="block mb-2" style={{ fontSize: F.min, color: palette.textTertiary }}>
        {label}
      </label>
      <select
        value={value}
        ...
        {options.map((opt) => {
          ...
          return (
            ...
            >
              ...
              <span style={{ fontSize: F.min, color: palette.textSecondary, marginTop: 2 }}>{opt.example}</span>
            </button>
          )
        })}
      </div>
