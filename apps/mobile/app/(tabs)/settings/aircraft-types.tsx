...
import {
  Search, ChevronLeft, ChevronRight, Plane, Plus,
...
const CATEGORY_ORDER: Record<string, number> = {
  narrow_body: 0, wide_body: 1, regional: 2, turboprop: 3,
}
const CATEGORY_LABELS: Record<string, string> = {
  narrow_body: 'Narrow Body', wide_body: 'Wide Body', regional: 'Regional', turboprop: 'Turboprop',
}

interface TypeSection { title: string; data: AircraftTypeRef[] }
    ...
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={[]}>
      ...
      {loading ? (
        ...
        <SectionList
          ...
          renderItem={({ item }) => (
            <TypeRow type={item} palette={palette} accent={accent}
              onPress={() => router.push({ pathname: '/(tabs)/settings/aircraft-type-detail' as any, params: { id: item._id } })} />
          )}
        />
      )}
    </SafeAreaView>
...
const TypeRow = memo(function TypeRow({ type, palette, accent, onPress }: {
  type: AircraftTypeRef; palette: Palette; accent: string; onPress: () => void
}) {
  return (
    <Pressable onPress={onPress} className="flex-row items-center active:opacity-70"
      ...
      <View className="flex-1 ml-2">
        <Text style={{ fontSize: 15, fontWeight: '500', color: palette.text }} numberOfLines={1}>{type.name}</Text>
        {type.manufacturer && (
