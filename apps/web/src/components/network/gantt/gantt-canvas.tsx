...
export function GanttCanvas() {
  ...
  const draw = useCallback(() => {
    ...
    if (df?.active && df.flightIds.size > 0) {
      ...
      const validity = df.targetRowIdx >= 0
        ? (df.targetReg === df.sourceReg ? 'invalid' as const
          : df.targetType && df.sourceType && df.targetType !== df.sourceType ? 'cross-type' as const
          : 'valid' as const)
        : 'invalid' as const
  ...
  const dragRef = useRef<{ startX: number; startY: number; dragging: boolean; ctrlKey: boolean; hitBarId: string | null } | null>(null)
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => { if (e.key === 'Escape') cancelDragFlight() }
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    if (hitBarId) {
      ...
      dragFlightRef.current = {
        active: false, timerId, flightIds: new Set(), sourceReg, sourceType,
        startY: y, currentY: y, targetRowIdx: -1, targetReg: null, targetType: null,
      }
    }
  }, [layout])
  ...
  const handleMouseUp = useCallback((e: React.MouseEvent) => {
    ...
    if (dragFlightRef.current?.active) {
      ...
      if (targetRow?.registration && targetRow.registration !== df.sourceReg) {
        ...
        useGanttStore.getState().rearrangeVirtualPlacements(
          flightIds, df.sourceReg, targetFlightIds, targetRow.registration
        )
      }
  ...
  const handleContextMenu = useCallback((e: React.MouseEvent) => {
    ...
    if (dragFlightRef.current?.active) { cancelDragFlight(); return }
    if (!layout) return
    const rect = (e.currentTarget as HTMLElement).getBoundingClientRect()
    const x = e.clientX - rect.left + scrollState.current.left
    const y = e.clientY - rect.top + scrollState.current.top
  ...
  // Cursor
  useEffect(() => {
    if (scrollRef.current) scrollRef.current.style.cursor = dragActive ? 'grabbing' : swapMode ? 'crosshair' : hoveredFlightId ? 'grab' : 'default'
  }, [hoveredFlightId])
      ...
      <div className="flex-1 min-h-0 flex overflow-hidden">
            ...
            {rows.map((row, i) => {
              ...
              return (
                <div key={`a${i}`} className="flex flex-col justify-center px-3 cursor-pointer"
                  ...
                  <span className="font-mono font-bold leading-tight" style={{ color: palette.text, fontSize: ROW_HEIGHT_LEVELS[rowHeightLevel].fontSize + 2 }}>{row.registration}</span>
                  <span className="font-mono leading-tight" style={{ color: palette.textTertiary, fontSize: ROW_HEIGHT_LEVELS[rowHeightLevel].fontSize }}>
        ...
        <div ref={containerRef} className="flex-1 min-h-0 min-w-0 overflow-hidden relative">
          ...
          <div ref={scrollRef} className="gantt-scroll absolute inset-0" style={{ overflow: 'scroll', zIndex: 1 }}
            onScroll={handleScroll} onMouseDown={handleMouseDown} onMouseMove={handleMouseMoveDrag} onMouseUp={handleMouseUp} onMouseLeave={() => useGanttStore.getState().setHovered(null)} onDoubleClick={handleDoubleClick} onContextMenu={handleContextMenu}>
