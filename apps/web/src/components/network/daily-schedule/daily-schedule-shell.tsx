...
import { useDailyScheduleStore, getFilteredFlights, getSortedFlights, getSummaryStats, type TimeMode } from "@/stores/use-daily-schedule-store"
...
export function DailyScheduleShell() {
  ...
  const handleGo = useCallback(async (dateFrom: string, dateTo: string) => {
    ...
    try {
      ...
      }, "Loading flights...", "Flights loaded")
      if (data) {
  ...
  const handleExport = useCallback((format: "csv" | "xlsx" | "pdf") => {
    const { columnOrder, hiddenColumns, timeModes, airportMap: am, homeBaseOffset, regToTypeMap, dateFrom, dateTo } = useDailyScheduleStore.getState()
