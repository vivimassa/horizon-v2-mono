...
export const ScheduleGrid = memo(function ScheduleGrid({
  flights, onLongPress, palette, accent, isDark, sortKey, sortDir, onSort,
}: ScheduleGridProps) {
  ...
  const handleCellTap = useCallback((cell: CellAddress) => {
  ...
  const handleCellDoubleTap = useCallback((cell: CellAddress) => {
  ...
  const renderRow = useCallback(({ item: flight, index }: { item: ScheduledFlightRef; index: number }) => (
    <GridRow
      flight={flight}
      rowIdx={index}
      ...
      isDark={isDark}
    />
  ), [store.dirtyMap, store.selectedCell, store.editingCell, store.editValue,
      store.selectionRange, store.clipboard, store.cellFormats,
      store.newRowIds, store.deletedIds, store.separatorAfter,
      handleCellTap, handleCellDoubleTap, handleEditChange, handleCommitEdit, handleCancelEdit,
      onLongPress, palette, accent, isDark])
            ...
            return (
              <Pressable key={col.key} onPress={() => onSort(col.key)}
                style={{
                  ...
                  borderRightWidth: 1, borderRightColor: palette.cardBorder,
