...
import {
  ArrowLeft, Lock, ShieldCheck, ScanFace,
  Smartphone, Monitor, Globe, Eye, EyeOff,
  Clock, KeyRound, LogOut, AlertTriangle, Check,
  type LucideIcon,
...
export default function SecurityScreen() {
  ...
  const formSection = (
    <>
      <SectionCard title="Change Password" icon={Lock} iconColor="#dc2626" palette={palette} isDark={isDark} fonts={fonts}>
        ...
        <Pressable
          ...
          style={{ paddingVertical: 12, backgroundColor: canSubmit ? ACCENT : palette.textTertiary, opacity: canSubmit ? 1 : 0.5 }}
        >
  ...
  const sessionsSection = (
    <>
      <SectionCard title="Active Sessions" icon={Globe} iconColor={ACCENT} palette={palette} isDark={isDark} fonts={fonts}>
        {MOCK_SESSIONS.map((session, i) => {
          const SIcon = session.icon
          return (
            <React.Fragment key={i}>
              <View className="flex-row items-center" style={{ paddingVertical: 12 }}>
                <View className="items-center justify-center rounded-xl mr-3"
                  style={{ width: 40, height: 40, backgroundColor: accentTint(session.current ? ACCENT : palette.textTertiary, isDark ? 0.12 : 0.06) }}>
                  ...
                  <View className="flex-row items-center" style={{ gap: 6 }}>
                    ...
                    {session.current && (
                      <View className="px-2 py-0.5 rounded-full"
                        ...
                        <Text style={{ fontSize: 11, fontWeight: '600', color: isDark ? '#4ade80' : '#16a34a' }}>This device</Text>
                      </View>
                    )}
                  </View>
                ...
                <View className="items-end" style={{ gap: 4 }}>
                  ...
                  {!session.current && (
                    <Pressable className="flex-row items-center rounded-lg px-2.5 py-1 active:opacity-70"
                      style={{ backgroundColor: isDark ? 'rgba(220,38,38,0.1)' : 'rgba(220,38,38,0.06)' }}>
                      <LogOut size={12} color="#dc2626" strokeWidth={2} />
                      <Text style={{ fontSize: fonts.xs, fontWeight: '600', color: '#dc2626', marginLeft: 4 }}>Revoke</Text>
                    </Pressable>
                  )}
                </View>
              </View>
      ...
      <SectionCard title="Security Activity" icon={Clock} iconColor={ACCENT} palette={palette} isDark={isDark} fonts={fonts}>
        <View style={{ gap: 14 }}>
          {MOCK_ACTIVITY.map((item, i) => {
            ...
            return (
              <View key={i} className="flex-row items-start" style={{ gap: 10 }}>
                <View className="items-center justify-center rounded-full mt-0.5"
    ...
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={['top']}>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 100 }} showsVerticalScrollIndicator={false}>
        {isTablet ? (
          ...
          <View className="px-4">{formSection}{sessionsSection}</View>
        )}
      </ScrollView>
    </SafeAreaView>
  )
...
function AuthToggleCard({ icon: Icon, title, subtitle, on, onToggle, color, palette, isDark, fonts }: {
  icon: LucideIcon; title: string; subtitle: string; on: boolean; onToggle: () => void;
  color: string; palette: Palette; isDark: boolean; fonts: any;
}) {
  return (
    <Pressable onPress={onToggle}
      className="flex-row items-center rounded-2xl border mb-4 active:opacity-80"
      style={{ padding: 16, backgroundColor: on ? accentTint(color, isDark ? 0.1 : 0.05) : palette.card, borderColor: on ? accentTint(color, 0.2) : palette.cardBorder }}>
      ...
      <View className="rounded-full" style={{ width: 44, height: 24, backgroundColor: on ? color : isDark ? '#444' : '#ddd', justifyContent: 'center' }}>
...
function PasswordField({ label, value, onChange, show, onToggleShow, palette, isDark, fonts, hint, error, success }: {
  label: string; value: string; onChange: (v: string) => void; show: boolean; onToggleShow: () => void;
  palette: Palette; isDark: boolean; fonts: any; hint?: string; error?: string; success?: string;
}) {
  return (
    <View style={{ marginBottom: 14 }}>
      ...
      <View className="flex-row items-center rounded-xl"
        style={{ backgroundColor: isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.03)', borderWidth: 1, borderColor: error ? '#dc2626' : isDark ? 'rgba(255,255,255,0.12)' : 'rgba(0,0,0,0.1)', paddingHorizontal: 14 }}>
        <TextInput value={value} onChangeText={onChange} secureTextEntry={!show} className="flex-1"
          style={{ fontSize: fonts.sm, color: palette.text, paddingVertical: 12 }} placeholderTextColor={palette.textTertiary} />
        <Pressable onPress={onToggleShow} className="pl-2">
          {show ? <EyeOff size={18} color={palette.textTertiary} strokeWidth={1.8} /> : <Eye size={18} color={palette.textTertiary} strokeWidth={1.8} />}
        </Pressable>
      </View>
