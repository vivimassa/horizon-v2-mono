...
export function ConnectorLine({ containerRef, dotRef, dockRef, accent }: ConnectorLineProps) {
  ...
  return (
    ...
    >
      {/* Soft glow */}
      <path d={path} fill="none" stroke={accent} strokeWidth={8} strokeOpacity={0.06} strokeLinecap="round" />

      {/* Main line */}
      <path d={path} fill="none" stroke={accent} strokeWidth={2} strokeOpacity={0.45} strokeLinecap="round" />

      {/* Endpoint dot */}
      <circle cx={to.x} cy={to.y} r={3} fill={accent} fillOpacity={0.3} />
    </svg>
  )
}
