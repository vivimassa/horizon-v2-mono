...
export function FlightFormDialog({ open, onOpenChange, contract, operatorCode, onCreated, isDark }: FlightFormDialogProps) {
  ...
  async function handleSubmit() {
    if (!form.flightNumber || !form.flightDate || !form.departureIata || !form.arrivalIata || !form.stdUtc || !form.staUtc) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.5)' }}
      ...
      <div className="w-[580px] max-h-[85vh] rounded-2xl overflow-hidden flex flex-col"
        ...
        <div className="flex items-center justify-between px-5 py-4 shrink-0" style={{ borderBottom: `1px solid ${glassBorder}` }}>
          <div>
            <h2 className="text-[16px] font-semibold" style={{ color: palette.text }}>Add charter flight</h2>
            <p className="text-[13px]" style={{ color: palette.textSecondary }}>Add a flight to this charter contract.</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          <Fieldset label="Flight">
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="block text-[13px] font-medium mb-1" style={{ color: palette.textSecondary }}>Flight number</label>
                <div className="flex items-center gap-1">
                  <span className="text-[13px] font-mono font-bold shrink-0" style={{ color: palette.textSecondary }}>{operatorCode}</span>
                  <input
                    value={form.flightNumber} onChange={e => update('flightNumber', e.target.value)}
                    className="flex-1 h-10 px-3 rounded-lg text-[13px] font-mono outline-none"
                    style={inputStyle}
                  />
                </div>
              </div>
              <Field label="Date *" value={form.flightDate} onChange={v => update('flightDate', v)} type="date" style={inputStyle} palette={palette} />
              <SelectField label="Leg type" value={form.legType} onChange={v => update('legType', v)}
                options={[{ value: 'revenue', label: 'Revenue' }, { value: 'positioning', label: 'Positioning' }, { value: 'technical', label: 'Technical' }]}
                style={inputStyle} palette={palette} />
            </div>
          </Fieldset>
          ...
          <Fieldset label="Route &amp; Times (UTC)">
            <div className="grid grid-cols-4 gap-3">
              <Field label="Departure *" value={form.departureIata} onChange={v => update('departureIata', v.toUpperCase())} placeholder="SGN" mono maxLength={4} style={inputStyle} palette={palette} />
              <Field label="Arrival *" value={form.arrivalIata} onChange={v => update('arrivalIata', v.toUpperCase())} placeholder="CXR" mono maxLength={4} style={inputStyle} palette={palette} />
              <Field label="STD (UTC) *" value={form.stdUtc} onChange={v => update('stdUtc', v)} type="time" style={inputStyle} palette={palette} />
              <Field label="STA (UTC) *" value={form.staUtc} onChange={v => update('staUtc', v)} type="time" style={inputStyle} palette={palette} />
            </div>
            <div className="grid grid-cols-4 gap-3 mt-3">
              <SelectField label="Day offset" value={form.arrivalDayOffset} onChange={v => update('arrivalDayOffset', v)}
                options={[{ value: '0', label: 'Same day' }, { value: '1', label: '+1 day' }]}
                style={inputStyle} palette={palette} />
            </div>
          </Fieldset>
          ...
          <Fieldset label="Aircraft &amp; Payload">
            <div className="grid grid-cols-3 gap-3">
              ...
              <Field label="Tail" value={form.aircraftRegistration} onChange={v => update('aircraftRegistration', v.toUpperCase())} placeholder="VN-A123" mono style={inputStyle} palette={palette} />
              <Field label="Pax booked" value={form.paxBooked} onChange={v => update('paxBooked', v)} type="number" style={inputStyle} palette={palette} />
            </div>
          </Fieldset>
        </div>
