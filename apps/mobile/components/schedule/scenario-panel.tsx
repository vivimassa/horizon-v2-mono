...
const STATUS_COLORS: Record<string, string> = {
  draft: '#6b7280', review: '#f59e0b', published: '#16a34a', archived: '#8b5cf6',
}
...
export const ScenarioPanel = memo(function ScenarioPanel({
  visible, onClose, scenarios, activeScenarioId, onSelect, onRefresh,
  operatorId, palette, accent, isDark,
}: ScenarioPanelProps) {
  ...
  const handleClone = useCallback(async (scenario: ScenarioRef) => {
    setBusy(true)
    try {
  ...
  const handlePublish = useCallback(async (scenario: ScenarioRef) => {
  ...
  const handleDelete = useCallback(async (scenario: ScenarioRef) => {
      ...
      <Pressable className="flex-1" style={{ backgroundColor: 'rgba(0,0,0,0.4)' }} onPress={onClose}>
        <View className="flex-1 mt-16 rounded-t-2xl" style={{ backgroundColor: palette.background }}>
          <Pressable onPress={() => {}}>
            ...
            {/* Create form */}
            {creating && (
              <View className="flex-row items-center px-4 py-3" style={{ borderBottomWidth: 1, borderBottomColor: palette.border, gap: 8 }}>
                <TextInput value={newName} onChangeText={setNewName} placeholder="Scenario name..."
                  placeholderTextColor={palette.textTertiary} autoFocus
                  style={{ flex: 1, fontSize: 15, color: palette.text, borderWidth: 1, borderColor: palette.cardBorder,
                    borderRadius: 8, paddingHorizontal: 12, paddingVertical: 8, backgroundColor: palette.card }} />
                <Pressable onPress={handleCreate} disabled={busy || !newName.trim()}
                ...
                <Pressable onPress={() => { setCreating(false); setNewName('') }} className="px-2 py-2 active:opacity-60">
            ...
            <Pressable onPress={() => { onSelect(null); onClose() }}
              ...
              style={{
                ...
                borderLeftWidth: activeScenarioId === null ? 3 : 0, borderLeftColor: accent,
              }}>
            ...
            {/* Scenario list */}
            <FlatList data={scenarios} keyExtractor={s => s._id}
              ...
              renderItem={({ item: scenario }) => {
                ...
                return (
                  <Pressable onPress={() => { onSelect(scenario._id); onClose() }}
                    className="flex-row items-center px-4 py-3 active:opacity-70"
                    style={{
                      borderBottomWidth: 1, borderBottomColor: palette.border,
                      backgroundColor: isActive ? accentTint(accent, isDark ? 0.1 : 0.04) : undefined,
                      borderLeftWidth: isActive ? 3 : 0, borderLeftColor: accent,
                    }}>
                    <View className="flex-1">
                      <View className="flex-row items-center" style={{ gap: 6 }}>
                        ...
                        <View className="px-1.5 py-0.5 rounded" style={{ backgroundColor: `${statusColor}20` }}>
                          <Text style={{ fontSize: 10, fontWeight: '600', color: statusColor, textTransform: 'uppercase' }}>{scenario.status}</Text>
                        </View>
                      </View>
                      {scenario.description && (
                        <Text style={{ fontSize: 13, color: palette.textTertiary, marginTop: 2 }} numberOfLines={1}>{scenario.description}</Text>
                      )}
                    </View>
                    <View className="flex-row items-center" style={{ gap: 6 }}>
