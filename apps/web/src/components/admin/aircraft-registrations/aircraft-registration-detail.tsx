...
export function AircraftRegistrationDetail({
  ...
  useEffect(() => {
    ...
    if (acType) {
      api
        .getLopaConfigs(getOperatorId(), acType.icaoType)
        .then(setLopaConfigs)
        .catch(() => {})
    }
    api
  ...
  const handleCreate = useCallback(async () => {
    ...
    if (!createForm.registration || !createForm.aircraftTypeId) {
      setCreateError('Registration and aircraft type are required')
      return
    }
    setCreating(true)
    setCreateError('')
    try {
      ...
      setCreateForm({
        registration: '',
        aircraftTypeId: aircraftTypes[0]?._id || '',
        serialNumber: '',
        variant: '',
        status: 'active',
        homeBaseIcao: '',
  ...
  const handleSave = useCallback(async () => {
    if (!onSave || !registration || Object.keys(draft).length === 0) {
      setEditing(false)
      return
    }
    setSaving(true)
  ...
  const handleDelete = useCallback(async () => {
    ...
    setErrorMsg('')
    try {
      await onDelete(registration._id)
    } catch (err: any) {
      setErrorMsg(friendlyError(err))
    } finally {
      setSaving(false)
        ...
        <div className="px-6 py-4 space-y-3 overflow-y-auto">
          <div className="flex gap-3">
            <MiniInput
              ...
              maxLength={10}
              onChange={(v) => setCreateForm((p) => ({ ...p, registration: v.toUpperCase() }))}
              mono
            />
            <div className="flex-1">
              ...
              <select
                value={createForm.aircraftTypeId}
          ...
          <div className="flex gap-3">
            <MiniInput
              ...
              onChange={(v) => setCreateForm((p) => ({ ...p, serialNumber: v }))}
              mono
            />
            <MiniInput
              label="Variant"
          ...
          <div className="flex gap-3">
            ...
            <MiniInput
              ...
              value={createForm.homeBaseIcao}
              maxLength={4}
              onChange={(v) => setCreateForm((p) => ({ ...p, homeBaseIcao: v.toUpperCase() }))}
              mono
            />
          </div>
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="px-6 py-4 border-b border-hz-border shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span
              className={`text-[13px] font-semibold px-2.5 py-0.5 rounded-full capitalize ${statusStyle.bg} ${statusStyle.text}`}
            >
      ...
      {acType && <ImageDropZone registration={registration} acType={acType} onSave={onSave} />}
        ...
        <div className="px-6 pt-3 pb-6">
          {activeTab === 'basic' && (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8">
              <FieldRow
                label="Registration"
                value={<span className="font-bold font-mono">{registration.registration}</span>}
                editing={editing}
                fieldKey="registration"
                editValue={getVal('registration')}
              ...
              <FieldRow
                label="Serial Number (MSN)"
                value={
                  registration.serialNumber ? <span className="font-mono">{registration.serialNumber}</span> : null
                }
                editing={editing}
                fieldKey="serialNumber"
                editValue={getVal('serialNumber')}
                onChange={handleFieldChange}
              />
              <FieldRow
                label="Variant"
                value={registration.variant}
                editing={editing}
                fieldKey="variant"
                editValue={getVal('variant')}
                onChange={handleFieldChange}
              />
              {/* Status */}
              <div className="py-2.5 border-b border-hz-border/50">
                ...
                {editing ? (
                  ...
                  <div
                    className={`text-[13px] font-semibold capitalize ${STATUS_STYLES[registration.status]?.text || ''}`}
                  >
                    {STATUSES[registration.status] || registration.status}
                  </div>
                )}
              ...
              <FieldRow
                label="Home Base (ICAO)"
                value={
                  registration.homeBaseIcao ? <span className="font-mono">{registration.homeBaseIcao}</span> : null
                }
                editing={editing}
                fieldKey="homeBaseIcao"
                editValue={getVal('homeBaseIcao')}
                onChange={handleFieldChange}
              />
              <FieldRow
                label="SELCAL"
                value={registration.selcal ? <span className="font-mono">{registration.selcal}</span> : null}
                editing={editing}
                fieldKey="selcal"
                editValue={getVal('selcal')}
                onChange={handleFieldChange}
              />
              <FieldRow
                label="Date of Manufacture"
                value={registration.dateOfManufacture}
                editing={editing}
                fieldKey="dateOfManufacture"
                editValue={getVal('dateOfManufacture')}
                onChange={handleFieldChange}
              />
              <FieldRow
                label="Date of Delivery"
                value={registration.dateOfDelivery}
                editing={editing}
                fieldKey="dateOfDelivery"
                editValue={getVal('dateOfDelivery')}
                onChange={handleFieldChange}
              />
              ...
              <div className="py-2.5 border-b border-hz-border/50">
                ...
                {editing ? (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {leaseStatus === 'expired' && (
                      <span className="text-[11px] font-semibold" style={{ color: '#E63535' }}>
                        Expired
                      </span>
                    )}
                    {leaseStatus === 'warning' && (
                      <span className="text-[11px] font-semibold" style={{ color: '#E67A00' }}>
                        Expiring soon
                      </span>
                    )}
                  </div>
            ...
            (() => {
              ...
              const handleLopaSelect = async (configId: string) => {
                ...
                try {
                  ...
                  console.error('Failed to update LOPA:', err)
                }
              }
                ...
                <div className="space-y-4">
                  ...
                  {lopaConfigs.length === 0 ? (
                    <div className="text-[13px] text-hz-text-secondary py-4">
                      No LOPA configurations found for {acType?.icaoType || 'this type'}. Create them in the LOPA
                      Database.
                    </div>
                  ) : (
                      ...
                      {lopaConfigs.map((lc) => {
                        ...
                        return (
                          ...
                          >
                            {/* Config header */}
                            <div
                              className={`flex items-center gap-3 px-4 py-2.5 ${isSelected ? 'bg-module-accent/10' : ''}`}
                            >
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                  <span
                                    className={`text-[13px] font-semibold ${isSelected ? 'text-module-accent' : 'text-hz-text-secondary'}`}
                                  >
                                    {lc.configName}
                                  </span>
                              ...
                              <span
                                className={`text-[15px] font-bold tabular-nums ${isSelected ? 'text-module-accent' : ''}`}
                              >
                                {lc.totalSeats}
                              </span>
                            ...
                            <div
                              className="px-2 pb-2 flex items-center justify-center"
                  ...
                  {lopaConfigs.length > 0 && (
                    <div className="px-4 py-3 rounded-xl bg-hz-border/10 border border-hz-border/30">
                      <p className="text-[12px] text-hz-text-secondary">
                        ...
                        Click a configuration to assign it. Click again to unassign. Manage configs in the LOPA
                        Database.
                      </p>
                    </div>
                  )}
                      ...
                      <TypeSection title="Fuel">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
                          <FieldRow
                            ...
                            value={perf.maxFuelCapacityKg?.toLocaleString() ?? null}
                          />
                          <FieldRow
                            label="Fuel Burn Rate (kg/hr)"
                            value={registration.fuelBurnRateKgPerHour?.toLocaleString() ?? null}
                          />
                        </div>
                      </TypeSection>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
                <FieldRow
                  ...
                  value={acType?.cargo?.maxCargoWeightKg?.toLocaleString() ?? null}
                />
                <FieldRow label="Cargo Positions (ULD)" value={acType?.cargo?.cargoPositions ?? null} />
                <FieldRow
                  label="Bulk Hold Capacity (kg)"
                  value={acType?.cargo?.bulkHoldCapacityKg?.toLocaleString() ?? null}
                />
              ...
              <div className="px-4 py-3 rounded-xl bg-hz-border/10 border border-hz-border/30">
                <p className="text-[12px] text-hz-text-secondary">
                  ...
                  Cargo data is inherited from the aircraft type ({acType?.icaoType}). Edit it in the Aircraft Types
                  Database.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="px-4 py-3 rounded-xl bg-hz-border/10 border border-hz-border/30">
                <p className="text-[12px] text-hz-text-secondary">
                  ...
                  Crew rest data is inherited from the aircraft type ({acType?.icaoType}). Edit it in the Aircraft Types
                  Database.
                </p>
              </div>
            </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="px-4 py-3 rounded-xl bg-hz-border/10 border border-hz-border/30">
                    <p className="text-[12px] text-hz-text-secondary">
                      ...
                      {hasOverride ? (
                        <>
                          Weather data is set per-airframe for this registration. Type defaults from {acType?.icaoType}{' '}
                          are used for any missing values.
                        </>
                      ) : (
                        <>
                          Weather data is inherited from the aircraft type ({acType?.icaoType}). Edit it in the Aircraft
                          Types Database.
                        </>
                      )}
                    </p>
          ...
          {activeTab === 'location' && (
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
                <FieldRow
                  label="Current Location (ICAO)"
                  value={
                    registration.currentLocationIcao ? (
                      <span className="font-mono font-bold">{registration.currentLocationIcao}</span>
                    ) : null
                  }
                  editing={editing}
                  fieldKey="currentLocationIcao"
                  editValue={getVal('currentLocationIcao')}
                  onChange={handleFieldChange}
                />
            ...
            <div className="space-y-4">
              <div className="px-4 py-3 rounded-xl bg-hz-border/10 border border-hz-border/30">
                <p className="text-[12px] text-hz-text-secondary">
                  ...
                  Maintenance tracking will be available in the Maintenance module. This tab will show maintenance
                  checks, due dates, and work orders for this aircraft.
                </p>
              </div>
            </div>
          )}
    ...
    >
      ...
      >
        ...
        <div className="relative flex items-center gap-3">
          ...
          />
          <label
            htmlFor={`ac-img-upload-${registration._id}`}
          ...
          >
            ...
            <span className="text-[13px] font-semibold">
              {uploading ? 'Uploading...' : hasCustomImage ? 'Change Photo' : 'Add Photo'}
            </span>
          </label>
          {hasCustomImage && (
            <button
