...
export default function CrewComplementAddScreen() {
  ...
  useEffect(() => {
    api
      .getAircraftTypes()
      .then((t) => setAcTypes(t.filter((a) => a.isActive)))
      .catch(console.error)
  }, [])
  ...
  const filteredTypes = useMemo(() => {
    ...
    const list = q
      ? acTypes.filter((t) => t.icaoType.toLowerCase().includes(q) || t.name.toLowerCase().includes(q))
      : acTypes
    return list.slice(0, 30)
  }, [acTypes, typeSearch])

  const handleCreate = useCallback(async () => {
    if (!form.aircraftTypeIcao) {
    ...
    try {
      ...
      try {
        ...
        if (m) {
          ...
          msg = Number(m[1]) === 409 ? 'This template already exists for this aircraft type.' : p.error || msg
        }
      } catch {}
      setError(msg)
    } finally {
      ...
      >
        ...
        <View className="flex-row flex-wrap" style={{ gap: 6, marginBottom: 16 }}>
          {filteredTypes.map((t) => {
            ...
            return (
              <Pressable
                key={t._id}
                onPress={() => setForm((p) => ({ ...p, aircraftTypeIcao: t.icaoType }))}
        ...
        >
          <Text style={{ fontSize: 15, fontWeight: '600', color: '#fff' }}>
            {creating ? 'Creating...' : 'Add Complement'}
          </Text>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
