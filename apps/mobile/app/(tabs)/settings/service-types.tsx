...
import {
  Search, ChevronLeft, ChevronDown, Tag, Plus, Pencil, X, Trash2,
...
export default function ServiceTypesList() {
  ...
  const toggleExpand = useCallback((id: string) => {
    ...
    setExpandedId(prev => {
      if (prev === id) { setEditingId(null); setDraft({}); return null }
  ...
  const handleSave = useCallback(async (item: FlightServiceTypeRef) => {
  ...
  const handleDelete = useCallback((item: FlightServiceTypeRef) => {
    Alert.alert(
      'Delete Service Type',
  ...
  const get = useCallback((item: FlightServiceTypeRef, key: keyof FlightServiceTypeRef) => {
            ...
            return (
              <View style={{ borderBottomWidth: 1, borderBottomColor: palette.border }}>
                ...
                >
                  ...
                  {!item.isActive && (
                    <View className="px-1.5 py-0.5 rounded-full mr-2" style={{ backgroundColor: isDark ? 'rgba(220,38,38,0.15)' : '#fee2e2' }}>
                ...
                {/* Accordion content */}
                {isExpanded && (
                  <View style={{ paddingHorizontal: 16, paddingBottom: 16, backgroundColor: isDark ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.015)' }}>
                    {/* Action bar */}
                    <View className="flex-row items-center justify-end mb-3" style={{ gap: 8 }}>
                      {isEditing ? (
                        <>
                          ...
                          <Pressable onPress={() => handleSave(item)} disabled={saving}
                    ...
                    <View className={isTablet ? 'flex-row flex-wrap' : ''}>
                      <DetailField label="Code" value={item.code} palette={palette} half={isTablet}
                        editing={isEditing} editValue={get(item, 'code')}
                        onChange={(v) => handleFieldChange('code', v.toUpperCase())} mono />
                      <DetailField label="Name" value={item.name} palette={palette} half={isTablet}
                        editing={isEditing} editValue={get(item, 'name')}
                        onChange={(v) => handleFieldChange('name', v)} />
                      <DetailField label="Description" value={item.description} palette={palette} half={isTablet}
                        editing={isEditing} editValue={get(item, 'description')}
                      ...
                      <View style={{ paddingVertical: 8, ...(isTablet ? { width: '50%', paddingRight: 12 } : {}) }}>
                        ...
                        <ColorSwatchPicker
                          ...
                          palette={palette} isDark={isDark} editing={isEditing}
                        />
                      </View>
                      <View style={{ paddingVertical: 8, ...(isTablet ? { width: '50%', paddingRight: 12 } : {}) }}>
                        <Text style={{ fontSize: 12, color: palette.textSecondary, textTransform: 'uppercase', letterSpacing: 0.5, fontWeight: '600', marginBottom: 4 }}>Active</Text>
                        {isEditing ? (
                          <Pressable onPress={() => handleFieldChange('isActive', !get(item, 'isActive'))}
                            className="self-start px-3 py-1 rounded-lg"
                            style={{ backgroundColor: get(item, 'isActive') ? (isDark ? 'rgba(22,163,74,0.15)' : '#dcfce7') : (isDark ? 'rgba(220,38,38,0.15)' : '#fee2e2') }}>
                            <Text style={{ fontSize: 13, fontWeight: '600', color: get(item, 'isActive') ? (isDark ? '#4ade80' : '#16a34a') : (isDark ? '#f87171' : '#dc2626') }}>
                              {get(item, 'isActive') ? 'Yes' : 'No'}
                            </Text>
                          </Pressable>
                        ) : (
                          <Text style={{ fontSize: 15, fontWeight: '600', color: item.isActive ? (isDark ? '#4ade80' : '#16a34a') : palette.textSecondary }}>
...
function DetailField({ label, value, palette, half, editing, editValue, onChange, mono }: {
  label: string; value: any; palette: Palette; half?: boolean
  editing?: boolean; editValue?: any; onChange?: (v: string) => void; mono?: boolean
}) {
  return (
