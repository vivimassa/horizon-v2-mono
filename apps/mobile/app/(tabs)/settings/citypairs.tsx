...
const ROUTE_TYPE_LABELS: Record<string, string> = {
  domestic: 'Domestic', regional: 'Regional', international: 'International',
  'long-haul': 'Long-Haul', 'ultra-long-haul': 'Ultra Long-Haul', unknown: 'Unclassified',
}
...
export default function CityPairsList() {
  ...
  useFocusEffect(useCallback(() => { fetchCityPairs() }, [fetchCityPairs]))
    ...
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={['top']}>
      ...
      {loading ? (
        ...
        <SectionList
          ...
          renderItem={({ item }) => (
            <CityPairRow cp={item} palette={palette} accent={accent} isDark={isDark}
              onPress={() => router.push({ pathname: '/(tabs)/settings/citypair-detail' as any, params: { id: item._id } })} />
          )}
        />
      )}
    </SafeAreaView>
...
const CityPairRow = memo(function CityPairRow({
  cp, palette, accent, isDark, onPress,
}: {
  cp: CityPairRef; palette: Palette; accent: string; isDark: boolean; onPress: () => void
}) {
