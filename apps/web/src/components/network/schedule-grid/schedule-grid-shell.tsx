...
export function ScheduleGridShell() {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const {
        rows: storeRows,
        newRowIds: storeNewRowIds,
        dirtyMap: storeDirty,
        deletedIds: storeDel,
        cellFormats,
        clearCellFormats,
        separatorAfter,
      } = useScheduleGridStore.getState()
      ...
      for (const [id, changes] of storeDirty.entries()) {
        ...
        existingUpdates.push({
          id,
          changes: { ...changes, formatting: fmt, sortOrder: sortOrderMap.get(id) ?? 0, ...rot },
        })
        updatedIds.add(id)
      }
      ...
      for (const [key] of cellFormats) {
        ...
        existingUpdates.push({
          id: rowId,
          changes: { formatting: fmt, sortOrder: sortOrderMap.get(rowId) ?? 0, ...rot },
        })
      ...
      for (const row of visibleRows) {
        ...
        const rotChanged =
          rot &&
          (row.rotationId !== rot.rotationId ||
            row.rotationSequence !== rot.rotationSequence ||
            row.rotationLabel !== rot.rotationLabel)
      ...
      if (survivingNewRows.length > 0) {
        const mergedNewRows = survivingNewRows
          .map((row) => {
      ...
      try {
        const data = await api.getScheduledFlights({
          operatorId: getOperatorId(),
          scenarioId: activeScenarioId ?? '__production__',
        })
        setRows(data)
        hydrateFormats(data)
      } catch (_) {
        /* grid already cleared, user can re-fetch via Go */
      }
    } catch (e) {
  ...
  const handleAddFlight = useCallback(
    (focusCol = 'aircraftTypeIcao', insertAtIdx?: number) => {
      ...
      if (insertAtIdx != null && insertAtIdx < visibleRows.length) {
        ...
        requestAnimationFrame(() => {
          const newLen = useScheduleGridStore
            .getState()
            .rows.filter((r) => !useScheduleGridStore.getState().deletedIds.has(r._id)).length
          ...
          <>
            ...
            <RibbonToolbar
              ...
              onInsertFlight={() => {
                ...
                if (sel) {
                  const count = range ? Math.abs(range.endRow - range.startRow) + 1 : 1
