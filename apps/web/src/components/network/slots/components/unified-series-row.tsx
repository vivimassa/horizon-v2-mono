...
import { STATUS_CHIP_CLASSES, PRIORITY_LABELS, ACTION_CODE_LABELS, formatSlotTime, formatPeriod } from '../slot-types'
...
export function UnifiedSeriesRow({ series, utilization, onDataChanged, isDark }: UnifiedSeriesRowProps) {
  ...
  useEffect(() => {
    if (expanded && !loadedDetail) {
      Promise.all([api.getSlotDates(series._id), api.getSlotActionLog(series._id)]).then(([d, a]) => {
        setDates(d)
        setActionLog(a)
  ...
  return (
    <div className="mb-2 rounded-xl overflow-hidden transition-all" style={{ border: `1px solid ${glassBorder}` }}>
          ...
          <div className="flex gap-0.5">
            {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((d, i) => {
              ...
              return (
                <span
                  key={i}
                  className="w-[18px] h-[22px] rounded text-[13px] font-semibold flex items-center justify-center"
                  style={{
        ...
        <div className="mx-4 mb-4 mt-1 grid grid-cols-3 gap-3">
          ...
          <GlassCard title="Series Detail" accent={accent} palette={palette} isDark={isDark} glassBorder={glassBorder}>
            <div className="space-y-1.5 text-[13px]">
              <DetailRow
                label="Allocated Arrival Time"
                value={formatSlotTime(series.allocatedArrivalTime)}
                palette={palette}
              />
              <DetailRow
                label="Allocated Departure Time"
              ...
              <DetailRow
                label="Frequency"
                value={series.frequencyRate === 1 ? 'Weekly' : 'Biweekly'}
                palette={palette}
              />
              <DetailRow
                label="Overnight Indicator"
                value={series.overnightIndicator ? 'Yes' : 'No'}
                palette={palette}
              />
              {series.flexibilityArrival && (
                <DetailRow label="Flexibility Arrival" value={series.flexibilityArrival} palette={palette} />
              )}
              {series.flexibilityDeparture && (
                <DetailRow label="Flexibility Departure" value={series.flexibilityDeparture} palette={palette} />
              )}
              {series.minTurnaroundMinutes && (
                <DetailRow label="Min Turnaround" value={`${series.minTurnaroundMinutes} min`} palette={palette} />
              )}
              {series.coordinatorRef && (
                <DetailRow label="Coordinator Reference" value={series.coordinatorRef} palette={palette} />
              )}
              {series.coordinatorReasonArrival && (
                <DetailRow label="Coordinator Reason (Arr)" value={series.coordinatorReasonArrival} palette={palette} />
              )}
              {series.coordinatorReasonDeparture && (
                <DetailRow
                  label="Coordinator Reason (Dep)"
                  value={series.coordinatorReasonDeparture}
                  palette={palette}
                />
              )}
              {series.waitlistPosition && (
                <DetailRow label="Waitlist Position" value={`#${series.waitlistPosition}`} palette={palette} />
              )}
          ...
          {/* Card 2: Dates */}
          <GlassCard
            title="Slot Historical Performance"
            accent={accent}
            palette={palette}
            isDark={isDark}
          ...
          >
            {dates.length > 0 ? (
              ...
              <div className="text-[13px] py-4 text-center" style={{ color: palette.textTertiary }}>
                No dates expanded
              </div>
            )}
          </GlassCard>
              ...
              <div className="space-y-1.5">
                {actionLog.slice(0, 20).map((a) => (
                  <div key={a._id} className="flex items-center gap-2 text-[13px]">
                    ...
                    <span
                      className="text-[13px] px-1 py-0.5 rounded shrink-0"
                      style={{
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 px-3.5 py-2.5" style={{ borderBottom: `1px solid ${glassBorder}` }}>
        ...
        <span className="text-[13px] font-semibold uppercase tracking-wide" style={{ color: palette.text }}>
          {title}
        </span>
      </div>
...
function DateCalendarGrid({
  ...
  for (const d of dates) {
    ...
    if (!seen.has(key)) {
      seen.add(key)
      monthKeys.push({ key, year: dt.getFullYear(), month: dt.getMonth() })
    }
  }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-7 gap-px mb-0.5">
        {DOW_HEADERS.map((d, i) => (
          <div key={i} className="text-center text-[13px] font-medium" style={{ color: palette.textTertiary }}>
            {d}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7 gap-px">
        ...
        {Array.from({ length: daysInMonth }).map((_, i) => {
          ...
          return (
            <div
              key={day}
              className="flex items-center justify-center h-[32px] rounded-md"
              style={{
            ...
            >
              <span
                ...
                style={{ color: textColor, opacity: status ? 1 : 0.25 }}
              >
                {day}
              </span>
            </div>
...
function LegendItem({
  ...
  label: string
  isDark: boolean
  palette: { textTertiary: string }
}) {
  return (
    <div className="flex items-center gap-1.5">
      <div className="w-[14px] h-[14px] rounded-sm" style={{ background: `${color}${isDark ? '26' : '1F'}` }} />
      <span className="text-[13px]" style={{ color: palette.textTertiary }}>
        {label}
      </span>
    </div>
