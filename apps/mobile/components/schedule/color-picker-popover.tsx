...
export const ColorPickerPopover = memo(function ColorPickerPopover({
  visible, title, onPick, onClose, palette, isDark,
}: {
  visible: boolean; title: string;
  onPick: (color: string) => void; onClose: () => void;
    ...
    <Modal visible={visible} transparent animationType="fade">
      <Pressable className="flex-1" style={{ backgroundColor: 'rgba(0,0,0,0.3)' }} onPress={onClose}>
        <View className="flex-1 justify-center items-center">
          <Pressable onPress={() => {}} className="rounded-2xl p-4"
            style={{
              width: 260,
            ...
            <View className="flex-row flex-wrap" style={{ gap: 6 }}>
              {PALETTE.map(color => (
                <Pressable key={color} onPress={() => { onPick(color); onClose() }}
            ...
            <Pressable onPress={() => { onPick(''); onClose() }}
