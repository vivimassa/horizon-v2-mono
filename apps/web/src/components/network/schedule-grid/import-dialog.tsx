  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ backgroundColor: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)" }}>
      <div
        ...
        style={{ backgroundColor: bg, border: `1px solid ${border}`, boxShadow: isDark ? "0 12px 48px rgba(0,0,0,0.5)" : "0 12px 48px rgba(96,97,112,0.18)", fontFamily: "Inter, system-ui, sans-serif" }}
      >
        {/* Header */}
        ...
        {!result ? (
          <>
            ...
            <div
              ...
              onMouseEnter={(e) => { if (!file) e.currentTarget.style.borderColor = "var(--color-module-accent)"; }}
              onMouseLeave={(e) => { if (!file) e.currentTarget.style.borderColor = border; }}
            >
              ...
              {file ? (
                ...
                <>
                  ...
                  <p className="text-[14px] font-medium text-hz-text-secondary">Drop .xlsx file here or click to browse</p>
                  <button
                    type="button"
                    onClick={async (e) => {
                      e.stopPropagation();
            ...
            {/* Error */}
            {error && (
              <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg" style={{ backgroundColor: "rgba(230,53,53,0.08)", border: `1px solid rgba(230,53,53,0.15)` }}>
                <AlertCircle size={14} style={{ color: "#E63535" }} />
                <span className="text-[13px] font-medium" style={{ color: "#E63535" }}>{error}</span>
              </div>
            )}
          ...
          <>
            ...
            {/* Errors list */}
            {result.errors && result.errors.length > 0 && (
              <div className="max-h-32 overflow-y-auto rounded-lg p-2.5 space-y-1" style={{ border: `1px solid ${border}`, backgroundColor: isDark ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.02)" }}>
                {result.errors.map((e, i) => (
                  <p key={i} className="text-[12px] text-hz-text-secondary font-mono">Row {e.row}: {e.message}</p>
                ))}
              </div>
            )}
