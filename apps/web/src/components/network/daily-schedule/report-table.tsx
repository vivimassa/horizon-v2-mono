...
import { useDailyScheduleStore, COLUMN_DEFS, utcMsToHhmm, applyOffset, getGroupKey, type ColumnId, type TimeMode } from "@/stores/use-daily-schedule-store"
...
export function ReportTable({ flights }: ReportTableProps) {
  ...
  const {
    timeModes, sortSequence, columnOrder, hiddenColumns, compactMode,
    airportMap, homeBaseOffset, regToTypeMap, setColumnOrder,
  ...
  const handleDragStart = useCallback((col: ColumnId) => { dragCol.current = col }, [])
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex shrink-0" style={{ background: headerBg, borderBottom: `1px solid ${glassBorder}`, height: 36 }}>
        {visibleCols.map(col => (
          <div
            key={col.id}
            draggable
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        {flights.length === 0 && (
          <div className="flex items-center justify-center h-32 text-[14px] text-hz-text-tertiary">No flights match the current filters</div>
        )}
        {(() => {
          let groupIdx = 0
          let prevKey = ""
                ...
                >
                  {visibleCols.map(col => (
                    <div key={col.id} className="px-3 min-w-0 truncate text-center" style={{ flex: col.flex }}>
                      <CellContent flight={flight} colId={col.id} activeModes={activeModes} airportMap={airportMap} homeBaseOffset={homeBaseOffset} regToTypeMap={regToTypeMap} isDark={isDark} dateFormat={dateFormat} />
                    </div>
                  ))}
                </div>
              </div>
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      <div className="flex gap-2">
        {modes.map(m => (
          <span key={m} className="text-[13px] font-medium normal-case tracking-normal text-hz-text-tertiary">{modeLabels[m]}</span>
        ))}
      </div>
    </div>
  )
...
function GroupSeparator({ flight, sortSequence, isDark }: { flight: Flight; sortSequence: string; isDark: boolean }) {
  ...
  if (sortSequence === "reg-date-std") {
    ...
    label = d.toLocaleDateString("en-US", { weekday: "long", day: "numeric", month: "short", year: "numeric", timeZone: "UTC" }).toUpperCase()
  }
...
function CellContent({ flight, colId, activeModes, airportMap, homeBaseOffset, regToTypeMap, isDark, dateFormat }: {
  flight: Flight; colId: string; activeModes: TimeMode[]
  airportMap: Record<string, { utcOffset: number; countryId: string | null }>
  homeBaseOffset: number; regToTypeMap: Record<string, string>; isDark: boolean; dateFormat: string
}) {
  switch (colId) {
    case "date":
      return <span className="text-[13px] font-medium text-hz-text">{formatDate(flight.operatingDate, dateFormat as any)}</span>
    case "dow": {
      const d = new Date(flight.operatingDate + "T00:00:00Z")
    ...
    case "dep":
      return <span className="text-[13px] font-semibold" style={{ color: "var(--module-accent, #1e40af)" }}>{flight.dep.iata || flight.dep.icao}</span>
    case "arr":
      return <span className="text-[13px] font-semibold" style={{ color: "var(--module-accent, #1e40af)" }}>{flight.arr.iata || flight.arr.icao}</span>
    case "std":
    case "sta": {
      ...
      return <TimeCell utcHhmm={utcHhmm} activeModes={activeModes} homeBaseOffset={homeBaseOffset} stationOffset={stationOffset} />
    }
    case "block": {
      if (!flight.schedule.stdUtc || !flight.schedule.staUtc) return <span className="text-[13px] text-hz-text-tertiary">-</span>
      const mins = Math.round((flight.schedule.staUtc - flight.schedule.stdUtc) / 60000)
      const h = Math.floor(mins / 60)
      const m = mins % 60
      return <span className="text-[13px] text-hz-text">{h}:{String(m).padStart(2, "0")}</span>
    }
    case "acType": {
...
function TimeCell({ utcHhmm, activeModes, homeBaseOffset, stationOffset }: {
  utcHhmm: string; activeModes: TimeMode[]; homeBaseOffset: number; stationOffset: number
}) {
...
function resolveTime(utcHhmm: string, mode: TimeMode, homeBaseOffset: number, stationOffset: number) {
  switch (mode) {
    case "utc": return { time: utcHhmm, dayShift: 0 }
    case "localBase": return applyOffset(utcHhmm, homeBaseOffset)
    case "localStation": return applyOffset(utcHhmm, stationOffset)
  }
}
