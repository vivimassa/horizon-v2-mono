        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-[13px] text-hz-text-secondary mt-1">
              This will permanently remove this cabin class. This action cannot be undone.
            </p>
          </div>
        </div>
...
export function CabinClassDetail({
  cabinClass,
  lopaConfigs = [],
  onSave,
  onDelete,
  ...
  const handleSave = useCallback(async () => {
    if (!onSave || !cabinClass || Object.keys(draft).length === 0) {
      setEditing(false)
      return
    }
    setSaving(true)
    try {
      await onSave(cabinClass._id, draft)
      setEditing(false)
      setDraft({})
    } catch (err) {
      ...
      <div className="flex flex-col h-full overflow-hidden">
        ...
        <div className="px-6 py-4 space-y-3 overflow-y-auto">
          ...
          <div className="flex gap-3">
            <MiniInput
              label="Seat Layout"
              value={createForm.seatLayout}
              onChange={(v) => setCreateForm((p) => ({ ...p, seatLayout: v }))}
              mono
            />
            <MiniInput
              label="Pitch (in)"
              value={createForm.seatPitchIn}
              type="number"
              onChange={(v) => setCreateForm((p) => ({ ...p, seatPitchIn: v }))}
            />
            <MiniInput
              label="Width (in)"
              value={createForm.seatWidthIn}
              type="number"
      ...
      <div className="px-6 py-4 border-b border-hz-border shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span
              className="w-7 h-7 rounded-full border border-hz-border/50 shrink-0"
        ...
        {activeTab === 'specs' && (
          <div className="px-6 pt-3 pb-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8">
              ...
              <FieldRow
                label="Sort Order"
                value={cabinClass.sortOrder}
                editing={editing}
                fieldKey="sortOrder"
                editValue={getVal('sortOrder')}
                onChange={handleFieldChange}
                inputType="number"
              />
              <FieldRow
                label="Seat Layout"
                value={cabinClass.seatLayout ?? null}
                editing={editing}
                fieldKey="seatLayout"
                editValue={getVal('seatLayout')}
                onChange={handleFieldChange}
              />
              <FieldRow
                label="Seat Pitch (in)"
                value={cabinClass.seatPitchIn ? `${cabinClass.seatPitchIn}"` : null}
                editing={editing}
                fieldKey="seatPitchIn"
                editValue={getVal('seatPitchIn')}
                onChange={handleFieldChange}
                inputType="number"
              />
              <FieldRow
                label="Seat Width (in)"
                value={cabinClass.seatWidthIn ? `${cabinClass.seatWidthIn}"` : null}
                editing={editing}
                fieldKey="seatWidthIn"
                editValue={getVal('seatWidthIn')}
                onChange={handleFieldChange}
                inputType="number"
              />
              <div className="py-2.5 border-b border-hz-border/50">
              ...
              <div className="flex flex-col items-center justify-center py-12 gap-3">
                ...
                <div className="text-center">
                  ...
                  <div className="text-[13px] text-hz-text-tertiary mt-0.5">
                    This cabin class is not used in any LOPA configuration.
                  </div>
                </div>
              </div>
            ) : (
              <div className="space-y-2">
                {usage.map((lc) => {
                  ...
                  return (
                    <div
                      key={lc._id}
                      className="flex items-center gap-3 px-4 py-3 rounded-xl border border-hz-border/50"
                    >
                      <span className="text-[13px] font-bold font-mono px-2 py-0.5 rounded-lg bg-hz-border/30">
                        {lc.aircraftType}
                      </span>
...
function MetricCard({
  ...
  return (
    <div className="flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl border border-hz-border/50 shrink-0">
      ...
      <div>
        <div className="text-[13px] text-hz-text-tertiary uppercase tracking-wider font-semibold leading-none">
          {label}
        </div>
        <div className="text-[14px] font-bold mt-0.5 leading-tight" style={{ color }}>
          {value}
        </div>
      </div>
    </div>
