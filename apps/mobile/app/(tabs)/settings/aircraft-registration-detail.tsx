...
import { api, type AircraftRegistrationRef, type AircraftTypeRef, type LopaConfigRef, type CabinClassRef } from '@skyhub/api'
import {
  ChevronLeft, Pencil, Save, X, Trash2,
  Info, Gauge, Armchair, Package, Users, CloudRain, MapPin, Wrench, Star,
...
const STATUS_COLORS: Record<string, string> = { active: '#10b981', maintenance: '#f59e0b', stored: '#6b7280', retired: '#ef4444' }
const STATUS_LABELS: Record<string, string> = { active: 'Active', maintenance: 'Maintenance', stored: 'Stored', retired: 'Retired' }
...
export default function AircraftRegistrationDetailScreen() {
  ...
  const handleSave = useCallback(async () => {
    if (!reg || Object.keys(draft).length === 0) { setEditing(false); return }
    setSaving(true)
    try {
      const updated = await api.updateAircraftRegistration(reg._id, draft)
      setReg(updated); setDraft({}); setEditing(false)
  ...
  const handleDelete = useCallback(() => {
    ...
    Alert.alert('Delete Aircraft', `Delete ${reg.registration}?`, [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
        try { await api.deleteAircraftRegistration(reg._id); router.back() }
        catch (err: any) {
    ...
    return (
      <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={['top']}>
        <View className="flex-1 justify-center items-center">
          <Text style={{ fontSize: 15, color: palette.textTertiary }}>{loading ? 'Loading...' : 'Registration not found'}</Text>
        </View>
      </SafeAreaView>
    )
  }
  ...
  return (
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={['top']}>
      ...
      <View className="px-4 pt-2 pb-3" style={{ borderBottomWidth: 1, borderBottomColor: palette.border }}>
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center flex-1 mr-2">
            ...
            <Text style={{ fontSize: 20, fontWeight: '700', fontFamily: 'monospace', color: palette.text, marginRight: 8 }}>
              {reg.registration}
            </Text>
        ...
        <View className="flex-row items-center mt-2 flex-wrap" style={{ gap: 6, marginLeft: 36 }}>
          ...
          {acType && (
            <View className="px-2 py-0.5 rounded-full" style={{ backgroundColor: isDark ? 'rgba(99,102,241,0.15)' : '#eef2ff' }}>
              <Text style={{ fontSize: 11, fontWeight: '600', fontFamily: 'monospace', color: isDark ? '#818cf8' : '#4338ca' }}>{acType.icaoType}</Text>
            </View>
          )}
          {leaseStatus === 'expired' && (
            <View className="px-2 py-0.5 rounded-full" style={{ backgroundColor: isDark ? 'rgba(220,38,38,0.15)' : '#fee2e2' }}>
              <Text style={{ fontSize: 11, fontWeight: '600', color: isDark ? '#f87171' : '#dc2626' }}>Lease Expired</Text>
            </View>
          )}
          {leaseStatus === 'warning' && (
            <View className="px-2 py-0.5 rounded-full" style={{ backgroundColor: isDark ? 'rgba(245,158,11,0.15)' : '#fef3c7' }}>
              <Text style={{ fontSize: 11, fontWeight: '600', color: isDark ? '#fbbf24' : '#b45309' }}>Lease Expiring</Text>
            </View>
          )}
        </View>
      </View>
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 100 }} keyboardShouldPersistTaps="handled">
        ...
        {activeTab === 'seating' && (
          <>
            <InfoBox text="Click to assign. Manage configs in LOPA Database." palette={palette} isDark={isDark} accent={accent} />
            {lopaConfigs.length === 0 ? (
              <Text style={{ fontSize: 13, color: palette.textSecondary, paddingVertical: 16 }}>No LOPA configurations found.</Text>
            ) : lopaConfigs.map(lc => {
              const isSelected = reg.lopaConfigId === lc._id
              return (
                <Pressable key={lc._id} onPress={() => {
                  if (!editing) return
                  handleFieldChange('lopaConfigId', isSelected ? null : lc._id)
                }}
                  className="rounded-xl mb-3" style={{ borderWidth: isSelected ? 2 : 1, borderColor: isSelected ? accent : palette.cardBorder, padding: 12, backgroundColor: isSelected ? accentTint(accent, isDark ? 0.08 : 0.04) : undefined }}>
                  <View className="flex-row items-center mb-2" style={{ gap: 6 }}>
                    ...
                    {isSelected && <Text style={{ fontSize: 11, fontWeight: '600', color: accent, marginLeft: 4 }}>Assigned</Text>}
        ...
        {activeTab === 'maint' && (
          <View className="flex-1 items-center justify-center py-16">
            ...
            <Text style={{ fontSize: 15, color: palette.textSecondary, marginTop: 12 }}>Coming in Maintenance module</Text>
          </View>
        )}
      </ScrollView>
    </SafeAreaView>
...
function InfoBox({ text, palette, isDark, accent }: { text: string; palette: Palette; isDark: boolean; accent: string }) {
  return (
    <View className="rounded-lg px-3 py-2.5 mb-3" style={{ backgroundColor: accentTint(accent, isDark ? 0.08 : 0.05), borderWidth: 1, borderColor: accentTint(accent, 0.15) }}>
