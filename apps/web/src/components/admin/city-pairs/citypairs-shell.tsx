...
export function CityPairsShell() {
  ...
  useEffect(() => { fetchCityPairs(); }, [fetchCityPairs]);

  const handleSave = useCallback(async (id: string, data: Partial<CityPairRef>) => {
  ...
  const handleCreate = useCallback(async (data: { station1Icao: string; station2Icao: string; standardBlockMinutes?: number }) => {
