  ...
  return (
    <div className="p-5 space-y-4">
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <KpiCard
          label="Positioning"
          value={stats.positioningFlights}
          color={isDark ? '#FDAC42' : '#E67A00'}
          isDark={isDark}
        />
          ...
          <table className="w-full text-left">
            <thead>
              <tr style={{ borderBottom: `1px solid ${cardBorder}` }}>
                {['Date', 'Flight', 'Type', 'Route', 'STD', 'STA', 'Block', 'AC', 'Tail', 'Pax', 'Status', ''].map(
                  (h) => (
                    <th
                      key={h}
                      className="px-3 py-2.5 text-[13px] font-medium uppercase whitespace-nowrap"
              ...
              {flights.map((f, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-[13px] font-mono font-bold whitespace-nowrap">
                      {(operator?.code ?? '') + f.flightNumber}
                    </td>
                    <td className="px-3 py-2">
                      <span
                        className="px-2 py-0.5 rounded-md text-[13px] font-semibold capitalize"
                        style={{
                          background: legStyle.background,
                          color: legStyle.color,
                          border: `1px solid ${legStyle.borderColor}`,
                        }}
                      >
                        {f.legType}
                      </span>
                    </td>
                    <td className="px-3 py-2 text-[13px] font-mono whitespace-nowrap">
                      {f.departureIata} &rarr; {f.arrivalIata}
                    </td>
                    ...
                    <td className="px-3 py-2 text-[13px] font-mono text-hz-text-tertiary">
                      {f.aircraftTypeIcao ?? '\u2014'}
                    </td>
                    <td className="px-3 py-2 text-[13px] font-mono text-hz-text-tertiary">
...
function KpiCard({
  ...
  return (
    ...
    >
      <div className="text-[18px] font-bold font-mono" style={{ color: color || (isDark ? '#F5F2FD' : '#1C1C28') }}>
        {value}
      </div>
