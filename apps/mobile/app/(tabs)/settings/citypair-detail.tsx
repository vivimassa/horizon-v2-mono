...
import { ChevronLeft, Pencil, Save, X, Trash2, Info, Clock } from 'lucide-react-native'
...
function greatCircleCoords(lat1: number, lon1: number, lat2: number, lon2: number, n = 60) {
  ...
  const d =
    2 *
    Math.asin(
      Math.sqrt(Math.sin((la1 - la2) / 2) ** 2 + Math.cos(la1) * Math.cos(la2) * Math.sin((lo1 - lo2) / 2) ** 2),
    )
...
export default function CityPairDetailScreen() {
  ...
  const handleSave = useCallback(async () => {
    if (!cityPair || Object.keys(draft).length === 0) {
      setEditing(false)
      return
    }
    setSaving(true)
  ...
  const handleDelete = useCallback(() => {
    ...
    Alert.alert('Delete City Pair', `Are you sure you want to delete ${label1}–${label2}?`, [
  ...
  const routeTypeColor =
    cityPair.routeType === 'domestic'
      ? '#166534'
      : cityPair.routeType === 'regional'
        ? '#1e40af'
        : cityPair.routeType === 'international'
          ? '#92400e'
          : cityPair.routeType === 'long-haul'
            ? '#9d174d'
            : cityPair.routeType === 'ultra-long-haul'
              ? '#5b21b6'
              : '#374151'
    ...
    <SafeAreaView className="flex-1" style={{ backgroundColor: palette.background }} edges={['top']}>
      ...
      <View className="px-4 pt-4 pb-4" style={{ borderBottomWidth: 1, borderBottomColor: palette.border }}>
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center flex-1 mr-2">
            ...
            <View
              ...
              style={{ backgroundColor: isDark ? `${routeTypeBg}20` : routeTypeBg }}
            >
              <Text style={{ fontSize: 13, fontWeight: '600', color: routeTypeColor, textTransform: 'capitalize' }}>
                {cityPair.routeType}
              </Text>
            </View>
          </View>
        ...
        <View className="flex-row items-center mt-1.5" style={{ marginLeft: 36, gap: 8 }}>
          ...
          {cityPair.isEtops && <Text style={{ fontSize: 13, fontWeight: '600', color: '#d97706' }}>· ETOPS</Text>}
        </View>
      </View>
            ...
            <View className="flex-row" style={{ gap: 16 }}>
              <View style={{ flex: 1 }}>
                ...
                <Field
                  ...
                  editValue={get('standardBlockMinutes')}
                  onChange={handleFieldChange}
                  numeric
                />
                <ToggleField
                  label="ETOPS"
                  value={cityPair.isEtops}
                  palette={palette}
                  isDark={isDark}
                  editing={editing}
                  fieldKey="isEtops"
                  editValue={get('isEtops')}
                  onChange={handleFieldChange}
                />
              </View>
              <View style={{ flex: 1 }}>
                ...
                <ToggleField
                  label="Overwater"
                  value={cityPair.isOverwater}
                  palette={palette}
                  isDark={isDark}
                  editing={editing}
                  fieldKey="isOverwater"
                  editValue={get('isOverwater')}
                  onChange={handleFieldChange}
                />
              </View>
            </View>
              ...
              <View style={{ gap: 8 }}>
                {cityPair.blockHours.map((bh) => (
                  ...
                  >
                    <View className="flex-row items-center justify-between mb-2">
                      <View className="flex-row items-center" style={{ gap: 8 }}>
                        <Text style={{ fontSize: 15, fontWeight: '800', color: palette.text }}>
                          {bh.aircraftTypeIcao ?? 'All'}
                        </Text>
                        <Text style={{ fontSize: 13, color: palette.textTertiary, textTransform: 'capitalize' }}>
                          {bh.seasonType}
                        </Text>
                      </View>
                    </View>
                    <View className="flex-row" style={{ gap: 16 }}>
                      <View style={{ flex: 1 }}>
                        <Text style={{ fontSize: 13, color: palette.textTertiary, fontWeight: '600' }}>
                          Block {label1}→{label2}
                        </Text>
                      ...
                      <View className="flex-row mt-2" style={{ gap: 16 }}>
                        <View style={{ flex: 1 }}>
                          ...
                          <Text style={{ fontSize: 14, color: palette.text }}>
                            {bh.dir1FuelKg?.toLocaleString() ?? '—'} kg
                          </Text>
                        </View>
                        <View style={{ flex: 1 }}>
                          <Text style={{ fontSize: 13, color: palette.textTertiary, fontWeight: '600' }}>
                          ...
                          <Text style={{ fontSize: 14, color: palette.text }}>
                            {bh.dir2FuelKg?.toLocaleString() ?? '—'} kg
                          </Text>
                        </View>
                      </View>
                    )}
