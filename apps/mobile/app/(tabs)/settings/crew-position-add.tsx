...
export default function CrewPositionAddScreen() {
  ...
  const handleCreate = useCallback(async () => {
    ...
    if (!form.rankOrder) {
      setError('Rank order is required')
      return
    }
    setCreating(true)
    setError('')
      ...
      >
          ...
          {CATEGORIES.map((cat) => {
            ...
            return (
              <Pressable
                key={cat.key}
                onPress={() =>
                  setForm((p) => ({ ...p, category: cat.key, isPic: cat.key === 'cabin' ? false : p.isPic }))
                }
        ...
        {form.category === 'cockpit' && (
          <ToggleRow
            ...
            value={form.isPic}
            onToggle={() => setForm((p) => ({ ...p, isPic: !p.isPic }))}
            palette={palette}
            isDark={isDark}
          />
        )}
        <ToggleRow
          ...
          onToggle={() => setForm((p) => ({ ...p, canDownrank: !p.canDownrank }))}
          palette={palette}
          isDark={isDark}
        />
      ...
      >
        <Text
          style={{
            fontSize: 13,
            fontWeight: '600',
            color: value ? (isDark ? '#4ade80' : '#16a34a') : isDark ? '#f87171' : '#dc2626',
          }}
        >
          {value ? 'Yes' : 'No'}
        </Text>
